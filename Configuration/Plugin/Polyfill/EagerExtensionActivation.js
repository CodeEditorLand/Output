...
function EagerExtensionActivation() {
  ...
  function FireActivationEvents() {
    try {
      ...
      for (const EventName of ["onStartupFinished", "*"]) {
        try {
          ...
          if (Result && typeof Result.then === "function") {
            Result.catch((Error2) => {
              console.warn(`[LandFix:EagerActivation] activateByEvent ${EventName} rejected: ${String(Error2)}`);
            });
          }
          FireCount++;
        } catch (Error2) {
          console.warn(`[LandFix:EagerActivation] activateByEvent ${EventName} threw: ${String(Error2)}`);
        }
      }
      if (FireCount > 0) {
      ...
      console.warn(`[LandFix:EagerActivation] FireActivationEvents failed: ${String(Error2)}`);
      return false;
    }
  }
