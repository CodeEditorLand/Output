  ...
  function AdvancePhase() {
    try {
      ...
      Services.invokeFunction((Accessor) => {
        try {
          ...
          if (Brands && Brands["ILifecycleService"]) {
            Lifecycle = Accessor.get(Brands["ILifecycleService"]);
          }
        } catch {
        }
      });
      ...
      try {
        ...
        console.warn(`[LandFix:Lifecycle] phase setter rejected: ${String(Error2)}`);
      }
      return false;
    } catch (Error2) {
      console.warn(`[LandFix:Lifecycle] advance failed: ${String(Error2)}`);
      return false;
    }
  }
...
//# sourceMappingURL=EagerLifecyclePhase.js.map
