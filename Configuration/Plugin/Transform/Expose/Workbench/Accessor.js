...
import { OnWorkbenchReady as __CEL_OnWorkbenchReady } from './CELExposeAccessor.js';`,
	...
	l = {
		...
		Match: ({ Path: n }) =>
			/\/vs\/workbench\/browser\/web\.main\.js$/.test(n) ||
			/\/vs\/workbench\/browser\/web\.factory\.js$/.test(n) ||
		...
		Transform({ Path: n, Source: e }) {
			...
			if (a || o) {
				...
				const i = e.replace(r, r + m).replace(t, E);
			...
			if (p) {
				...
				const r = e.replace(s, s + j).replace(c, h);
