...
const Plugin = {
  ...
  Transform({ Source }) {
    ...
    if (Next.includes(SwThen)) {
      const SwThenLog = ".then(async registration => { DEBUG_WV('SW_REGISTER_OK', { controller: !!navigator.serviceWorker.controller });";
      Next = Next.replace(SwThen, SwThenLog);
    }
    const SwCatch = ".catch(error => {";
    if (Next.includes(SwCatch)) {
      const SwCatchLog = ".catch(error => { DEBUG_WV('SW_REGISTER_FAIL', error.message || String(error));";
      Next = Next.replace(SwCatch, SwCatchLog);
    }
    const SignalReady = "const start = (/** @type {string} */ parentOrigin) => {";
    ...
    const PostReady = "window.parent.postMessage({ target: ID, channel: 'webview-ready'";
    if (Next.includes(PostReady)) {
      const PostReadyLog = "DEBUG_WV('SIGNAL_OK', 'posting webview-ready'); window.parent.postMessage({ target: ID, channel: 'webview-ready'";
      Next = Next.replace(PostReady, PostReadyLog);
    }
    const ThrowMismatch = "throw new Error(`Expected '${parentOriginHash}' as hostname or subdomain!`);";
    ...
    if (Next.includes(RealContentHandler)) {
      const LogRealContent = "DEBUG_WV('REAL_CONTENT', 'content handler fired'); hostMessaging.onMessage('content', async";
    ...
    if (Next.includes(OnMessageContent)) {
      const LogContent = "DEBUG_WV('CONTENT_EVENT', 'content handler invoked (unloadMonitor)'); hostMessaging.onMessage('content'";
      Next = Next.replace(OnMessageContent, LogContent);
    }
    const FakeHtml = "newFrame.src = `./fake.html";
    if (Next.includes(FakeHtml)) {
      const LogFake = "DEBUG_WV('INNER_FRAME_FAKE', 'setting src to fake.html'); newFrame.src = `./fake.html";
    ...
    if (Next.includes(OnFrameLoaded)) {
      const LogFrameLoaded = "function onFrameLoaded(contentDocument) { DEBUG_WV('INNER_FRAME_LOADED', { readyState: contentDocument.readyState });";
    ...
    if (Next.includes(WriteHtml)) {
      const LogWrite = "DEBUG_WV('INNER_WRITE', { htmlLen: newDocument.length }); contentDocument.write(newDocument);";
    ...
    if (Next.includes(CspSetAttribute)) {
      const CspFontFallback = "csp.setAttribute('content', newCsp); /* Land font-src wildcard fallback for WKWebView */ if (!/font-src\\s+\\*/.test(newCsp)) { try { csp.setAttribute('content', newCsp.replace(/(font-srcs+)([^;]+)/, '$1$2 *')); } catch(_) {} }";
