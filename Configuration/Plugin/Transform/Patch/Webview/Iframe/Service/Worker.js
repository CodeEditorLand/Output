...
const ServiceWorkerExpression = "const disableServiceWorker = searchParams.has('disableServiceWorker');";
const ServiceWorkerReplacement = `/* ${Marker} */ const disableServiceWorker = true; void searchParams;`;
const HashThrowExpression = "throw new Error(`Expected '${parentOriginHash}' as hostname or subdomain!`);";
...
const PathnameEndsWithExpression = "if (contentDocument.location.pathname.endsWith('/fake.html') && contentDocument.readyState !== 'loading') {";
const PathnameEndsWithReplacement = "if (contentDocument.readyState !== 'loading') {";
