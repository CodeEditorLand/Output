...
const ReplacementSuffix = `.replace(/vscode-file:\\/\\/vscode-app\\/Static\\/Application\\/out\\//g, (globalThis).location.origin + '/Static/Application/').replace(/vscode-file:\\/\\/vscode-app\\//g, (globalThis).location.origin + '/Extension/')`;
const IconsStyleSheetAnchor = {
  ...
  Search: /(getCSS\(\)\s*\{\s*[\s\S]*?return\s+rules\.join\('\\n'\));(\s*\})/
};
...
const Plugin = {
  ...
  Match: /* @__PURE__ */ __name(({ Path }) => Anchors.some((Anchor) => Anchor.PathRegex.test(Path)), "Match"),
  Transform({ Path, Source }) {
    const Anchor = Anchors.find(
      (Candidate) => Candidate.PathRegex.test(Path)
    );
...
export {
  RewriteIconsStyleSheetURLs_default as default
};
