...
const Plugin = {
  ...
  Enabled: /* @__PURE__ */ __name(() => process.env["Electron"] === "true" && // `Pack` is the space-separated list of bundled-workbench variants
  ...
  Transform({ Source }) {
    ...
    const Lines = [
      ...
      `let _n = 0;`,
      ...Imports.map(
        (Path, Index) => `try{await import('${Path}');_n++;${Index % 10 === 0 ? `console.log("[workbench.desktop.main] "+_n+"/${Imports.length}: ${Path}");` : ""}}catch(_e){console.error("[workbench.desktop.main] FAILED #${Index}: ${Path}",_e)}`
      ),
...
export {
  StaticToDynamicImport_default as default
};
