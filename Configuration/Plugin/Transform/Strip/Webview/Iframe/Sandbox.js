...
const Plugin = {
  ...
  Match: /* @__PURE__ */ __name(({ Path }) => PreIndexPathRegex.test(Path) || WebviewElementPathRegex.test(Path), "Match"),
  Transform({ Path, Source }) {
    if (Source.includes(Marker)) return { Kind: "Unchanged" };
    const Pattern = PreIndexPathRegex.test(Path) ? PreIndexSandboxCall : WebviewElementSandboxCall;
