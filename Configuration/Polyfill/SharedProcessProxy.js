...
function listenToTauri(event, handler) {
	...
	if (typeof window.TAURI?.event?.listen === "function") {
		const unlistenPromise = window.TAURI.event
			.listen(event, ({ payload }) => {
				handler(payload);
			})
			.catch(() => {});
		...
		async healthCheck() {
			try {
				if (service === "extension-host") {
					...
					return await invokeTauri("cocoon_debug_service_health", {});
				} else {
					return await invokeTauri("shared_process_service_health", {
						service,
					});
				}
			} catch {
...
const SearchService = Object.assign(createServiceProxy("search"), {
	/**
	 * Perform search
	 */
...
const DebugService = Object.assign(createServiceProxy("debug"), {
	/**
	 * Start debug session
	 */
...
const StorageService = Object.assign(createServiceProxy("storage"), {
	/**
...
const UpdateService = Object.assign(createServiceProxy("update"), {
	/**
	 * Check for updates
	 */
...
if (typeof window !== "undefined") {
	installSharedProcessProxy().catch((Error2) => {
		globalThis.__LAND_POLYFILL_TELEMETRY__?.On("polyfill.install", Error2, {
