...
const SnapshotViewRegistry = /* @__PURE__ */ __name(() => {
  try {
    ...
    const ContainerIds = Locations.map(
      (Location) => String(Location?.id ?? "<no-id>")
    );
    const ViewIds = [];
    try {
...
const ExposeAccessor = /* @__PURE__ */ __name((InstantiationService) => {
  ...
  try {
    ...
    try {
      ...
      if (ExtensionSvc && typeof ExtensionSvc.activateByEvent === "function" && !ExtensionSvc.__CEL_PATCHED_ONVIEW__) {
        const Original = ExtensionSvc.activateByEvent.bind(ExtensionSvc);
        ExtensionSvc.activateByEvent = function(Event2) {
          if (typeof Event2 === "string" && Event2.indexOf("onView:") === 0) {
          ...
          window.addEventListener(
            ...
            (MessageEvent) => {
              try {
                ...
                if (Data && Data.channel === "webview-ready" && typeof Data.target === "string" && MessageEvent.ports && MessageEvent.ports.length > 0) {
