...
const FetchFromMountain = /* @__PURE__ */ __name(async (Method, ForceBuiltin) => {
  ...
  try {
    ...
    if (Extensions.length === 0 && (Method === "extensions:scanSystemExtensions" || Method === "extensions:scanUserExtensions")) {
      ...
      for (let Retry = 0; Retry < Schedule.length; Retry++) {
        ...
        await new Promise(
          (Resolve) => setTimeout(Resolve, Schedule[Retry])
        );
    ...
    for (let I = 0; I < Extensions.length; I++) {
      ...
      try {
        const Manifest = Extension.manifest && typeof Extension.manifest === "object" ? Extension.manifest : Extension;
        const RawLocation = Extension.location ?? Extension.extensionLocation;
        const Location = RawLocation ? typeof RawLocation === "string" ? URI.parse(RawLocation) : URI.revive(RawLocation) : URI.file("/extensions/" + (Manifest.name || "unknown"));
        const Identifier = Extension.identifier?.id || Extension.identifier?.value || (Manifest.publisher && Manifest.name ? Manifest.publisher + "." + Manifest.name : Manifest.name) || "unknown";
        const ExtType = typeof Extension.type === "number" ? Extension.type : ForceBuiltin ? 0 : 1;
        const IsBuiltin = typeof Extension.isBuiltin === "boolean" ? Extension.isBuiltin : ForceBuiltin;
        Mapped.push({
          type: ExtType,
...
class ExtensionsScannerService {
  ...
  async scanExistingExtension(_ExtensionLocation, _ExtensionType, _ScanOptions) {
    return null;
  }
  async scanOneOrMultipleExtensions(_ExtensionLocation, _ExtensionType, _ScanOptions) {
    return [];
  }
