...
const o = (s, i) => {
	...
	g = async (s, i) => {
		...
		try {
			...
			const u = await l("MountainIPCInvoke", { method: s, params: [] });

			let r = Array.isArray(u) ? u : [];

			if (
				(o("scanner:fetch:result", {
					method: s,
					count: r.length,
					type: typeof u,
			...
			if (
				...
				c(
					...
					a.location?.toString?.()?.slice(0, 80),
				);
			}
...
export { h as ExtensionsScannerService, p as IExtensionsScannerService };
