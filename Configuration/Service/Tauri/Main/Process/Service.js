...
const _TierIPC =
	import.meta.env?.TierIPC ?? globalThis.__LandTiers?.TierIPC ?? "Mountain";
...
__name(_InvokeViaNode, "_InvokeViaNode");
	...
	async call(Command, Arg, _CancellationToken) {
		...
		if (_TierIPC === "NodeDeferred" || _TierIPC === "Node") {
			...
			const NodeParams =
				Arg !== void 0 ? (Array.isArray(Arg) ? Arg : [Arg]) : [];
			try {
				return await _InvokeViaNode(NodeMethod, NodeParams);
			} catch {
				return void 0;
			}
		}
	...
	listen(Event, Arg) {
		...
		if (
			...
			return (Listener) => {
				...
				return { dispose: /* @__PURE__ */ __name(() => {}, "dispose") };
			};
		}
		return () => ({ dispose: /* @__PURE__ */ __name(() => {}, "dispose") });
	}
...
var Service_default = TauriMainProcessService;
export { TauriMainProcessService, Service_default as default };
