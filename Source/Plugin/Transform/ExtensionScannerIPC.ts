/**
 * Step 14: Replace the electron-browser `extensionsScannerService.js` with an
 * IPC-backed override that fetches the scanned extensions from Mountain via
 * `MountainIPCInvoke("extensions:getAll")`.
 *
 ...
 * tree on boot; this shim pipes those results into the workbench DI
 * container so the `@builtin` sidebar, extension activation, and
 * workspaceContains checks all see the real set.
 *
 * Replacement is a full rewrite (the upstream class is irrelevant under
 * Tauri); the body is kept in a single template string for diff-ability
 * against Sky's historical inline version.
 */
