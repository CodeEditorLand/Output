...
let CodeApplication = class extends Disposable {
  ...
  nativeHostMainService;
  configureSession() {
    const isUrlFromWindow = /* @__PURE__ */ __name((requestingUrl) => requestingUrl?.startsWith(
  ...
  async initServices(machineId, sqmId, devDeviceId, sharedProcessReady) {
    ...
    services.set(
      INativeMcpDiscoveryHelperService,
      new SyncDescriptor(NativeMcpDiscoveryHelperService)
    );
    services.set(
      ICSSDevelopmentService,
  ...
  initChannels(accessor, mainProcessElectronServer, sharedProcessClient) {
    ...
    const webContentExtractorChannel = ProxyChannel.fromService(
      accessor.get(IWebContentExtractorService),
      disposables
    );
    mainProcessElectronServer.registerChannel(
      "webContentExtractor",
    ...
    const mcpDiscoveryChannel = ProxyChannel.fromService(
      accessor.get(INativeMcpDiscoveryHelperService),
      disposables
    );
    mainProcessElectronServer.registerChannel(
      NativeMcpDiscoveryHelperChannelName,
