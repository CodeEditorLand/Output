...
let DefaultExtensionsInitializer = class extends Disposable {
  constructor(environmentService, extensionManagementService, storageService, fileService, logService) {
    ...
    if (isWindows && storageService.getBoolean(
  ...
  async initializeDefaultExtensions() {
    ...
    try {
      ...
      if (!stat.children) {
        this.logService.debug(
          "There are no default extensions to initialize",
    ...
    const vsixs = stat.children.filter(
      (child) => child.name.toLowerCase().endsWith(".vsix")
    );
    if (vsixs.length === 0) {
      this.logService.debug(
        "There are no default extensions to initialize",
      ...
      vsixs.map(async (vsix) => {
        ...
        try {
          ...
          this.logService.error(
            "Error installing default extension",
            vsix.resource.toString(),
  ...
  getDefaultExtensionVSIXsLocation() {
    return URI.file(
      join(
        dirname(dirname(this.environmentService.appRoot)),
        "bootstrap",
        "extensions"
      )
    );
