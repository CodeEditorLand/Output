...
let NativeEditContext = class extends AbstractEditContext {
  constructor(ownerID, context, overflowGuardContainer, viewController, _visibleRangeProvider, instantiationService, _accessibilityService) {
    ...
    this._focusTracker = this._register(
      new FocusTracker(this.domNode.domNode, (newFocusValue) => {
        if (newFocusValue) {
    ...
    this._register(
      addDisposableListener(
        ...
        "copy",
        (e) => this._ensureClipboardGetsEditorSelection(e)
      )
    );
    this._register(
    ...
    this._register(
      addDisposableListener(this._textArea.domNode, "paste", (e) => {
        this._screenReaderSupport.setIgnoreSelectionChangeTime(
          "onPaste"
        );
        e.preventDefault();
  ...
  _emitTypeEvent(viewController, e) {
    ...
    if (!this._editContextPrimarySelection.equalsSelection(
  ...
  _updateSelectionAndControlBounds(ctx) {
    ...
    const viewStartPosition = this._context.viewModel.coordinatesConverter.convertModelPositionToViewPosition(
      modelStartPosition
    );
  ...
  _updateCharacterBounds(e) {
    ...
    for (let offset = e.rangeStart; offset < e.rangeEnd; offset++) {
      ...
      const characterViewRange = this._context.viewModel.coordinatesConverter.convertModelRangeToViewRange(
        characterModelRange
      );
      const characterLinesVisibleRanges = this._visibleRangeProvider.linesVisibleRangesForRange(
        characterViewRange,
        true
      ) ?? [];
      const characterVerticalOffset = this._context.viewLayout.getVerticalOffsetForLineNumber(
        characterViewRange.startLineNumber
      );
