...
class ObservableCodeEditor extends Disposable {
  ...
  isTextFocused = observableFromEvent(
    this,
    (e) => {
  ...
  inComposition = observableFromEvent(
    this,
    (e) => {
  ...
  layoutInfoVerticalScrollbarWidth = this.layoutInfo.map(
    (l) => l.verticalScrollbarWidth
  );
  ...
  observeLineOffsetRange(lineRange, store) {
    ...
    return derived((reader) => {
      ...
      const e = range.isEmpty ? s : this.editor.getBottomForLineNumber(
  ...
  observePosition(position, store) {
    ...
    const result = observableValueOpts(
      {
        owner: this,
        debugName: /* @__PURE__ */ __name(() => `topLeftOfPosition${pos?.toString()}`, "debugName"),
    ...
    const w = {
      ...
      afterRender: /* @__PURE__ */ __name((position2, coordinate) => {
        ...
        if (model && pos && pos.lineNumber > model.getLineCount()) {
          ...
          result.set(
            coordinate ? new Point(coordinate.left, coordinate.top) : null,
            void 0
          );
        }
      }, "afterRender")
