...
.monaco-editor .ghost-text {
  ...
  optionPreview = this._editorObs.getOption(EditorOption.suggest).map((v) => v.preview);
  optionPreviewMode = this._editorObs.getOption(EditorOption.suggest).map((v) => v.previewMode);
  optionMode = this._editorObs.getOption(EditorOption.inlineSuggest).map((v) => v.mode);
  optionInlineEditsEnabled = this._editorObs.getOption(EditorOption.inlineSuggest).map((v) => !!v.edits.experimental?.enabled);
