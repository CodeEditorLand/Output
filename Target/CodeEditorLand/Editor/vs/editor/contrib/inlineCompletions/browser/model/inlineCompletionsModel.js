...
let InlineCompletionsModel = class extends Disposable {
  constructor(textModel, _selectedSuggestItem, _textModelVersionId, _positions, _debounceValue, _enabled, _editor, _instantiationService, _commandService, _languageConfigurationService, _accessibilityService) {
    ...
    this._register(
      autorun((reader) => {
        const jumpToReset = this.state.map(
          (s) => !s || s.kind === "inlineEdit" && !s.cursorAtInlineEdit
        ).read(reader);
        if (jumpToReset) {
    ...
    const inlineEditSemanticId = this.inlineEditState.map(
      (s) => s?.inlineCompletion.semanticId
    );
    this._register(
      autorun((reader) => {
  ...
  _didUndoInlineEdits = derivedHandleChanges(
    {
      owner: this,
      createEmptyChangeSummary: /* @__PURE__ */ __name(() => ({ didUndo: false }), "createEmptyChangeSummary"),
  ...
  getIndentationInfo(reader) {
    ...
    if (!!this?._selectedSuggestItem && ghostText && ghostText.parts.length > 0) {
      ...
      const indentationEndColumn = this.textModel.getLineIndentColumn(
    ...
    subtransaction(tx, (tx2) => {
      if (stopReason === "explicitCancel") {
        ...
        if (sourceInlineCompletion && source?.provider.handleRejection) {
          source.provider.handleRejection(
  ...
  _inlineCompletionItems = derivedOpts(
    { owner: this },
    (reader) => {
  ...
  _hasVisiblePeekWidgets = derived(
    this,
    (reader) => this._editorObs.openedPeekWidgets.read(reader) > 0
  );
  state = derivedOpts(
    {
      owner: this,
      equalsFn: /* @__PURE__ */ __name((a, b) => {
        ...
        if (a.kind === "ghostText" && b.kind === "ghostText") {
          return ghostTextsOrReplacementsEqual(
            a.ghostTexts,
            b.ghostTexts
          ) && a.inlineCompletion === b.inlineCompletion && a.suggestItem === b.suggestItem;
    ...
    (reader) => {
      ...
      if (inlineEditResult) {
        ...
        const cursorAtInlineEdit = LineRange.fromRangeInclusive(
          edit.range
        ).addMargin(1, 1).contains(cursorPos.lineNumber);
        const cursorInsideShowRange = cursorAtInlineEdit || (inlineEditResult.inlineCompletion.cursorShowRange?.containsPosition(
          cursorPos
        ) ?? true);
        ...
        const inlineEdit = new InlineEdit(
          edit,
          commands ?? [],
          inlineEditResult.inlineCompletion
        );
        const edits = inlineEditResult.updatedEdit.read(reader);
        const e = edits ? TextEdit.fromOffsetEdit(
          edits,
          new TextModelText(this.textModel)
        ).edits : [edit];
        return {
          kind: "inlineEdit",
          inlineEdit,
  ...
  _tabShouldIndent = derived(this, (reader) => {
    ...
    return selections?.some((s) => {
      if (s.isEmpty()) {
        ...
        return isMultiLine(s) || s.containsRange(
          getNonIndentationRange(
            this.textModel,
            s.startLineNumber
          )
        );
  ...
  async handleInlineEditShown(inlineCompletion) {
    ...
    inlineCompletion.source.provider.handleItemDidShow?.(
      inlineCompletion.source.inlineCompletions,
      inlineCompletion.sourceInlineCompletion,
      inlineCompletion.insertText
    );
    if (inlineCompletion.shownCommand) {
      await this._commandService.executeCommand(
        inlineCompletion.shownCommand.id,
        ...inlineCompletion.shownCommand.arguments || []
      );
    }
  }
};
