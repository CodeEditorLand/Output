...
let InlineEditsGutterIndicator = class extends Disposable {
  constructor(_editorObs, _originalRange, _verticalOffset, _host, _model, _isHoveringOverInlineEdit, _focusIsInMenu, _hoverService, _instantiationService, _storageService, _accessibilityService, themeService) {
    ...
    this._register(
      this._editorObs.editor.onMouseMove((e) => {
        const el = this._iconRef.element;
        const rect = el.getBoundingClientRect();
        const rectangularArea = Rect.fromLeftTopWidthHeight(
          rect.left,
          rect.top,
          rect.width,
          rect.height
        ...
        this._isHoveredOverIcon.set(
          rectangularArea.containsPoint(point),
          void 0
        );
      })
    );
    ...
    this._register(
      runOnChange(
        this._isHoveredOverInlineEditDebounced,
        (isHovering) => {
          if (isHovering) {
    ...
    if (this._newUserType === "active" /* Active */) {
    ...
    this._register(
      autorun((reader) => {
        ...
        if (this._indicator.element) {
          this._editorObs.editor.applyFontInfo(
  ...
  _newUserAnimationDisposable = this._register(
    new MutableDisposable()
  );
  _firstToSecondTimeUserDisposable = this._register(
    new MutableDisposable()
  );
  _secondTimeToActiveUserDisposable = this._register(
    new MutableDisposable()
  );
  get _newUserType() {
  ...
  setupNewUserExperience() {
    ...
    disposableStore.add(
      runOnChange(this._activeCompletionId, async (id) => {
        if (id === void 0) {
          return;
        }
        ...
        switch (userType) {
          case "firstTime" /* FirstTime */: {
            if (++firstTimeUserAnimationCount >= 5 || userHasHoveredOverIcon) {
              this._newUserType = "secondTime" /* SecondTime */;
            }
            break;
          }
          case "secondTime" /* SecondTime */: {
            if (++secondTimeUserAnimationCount >= 5 && inlineEditHasBeenAccepted) {
              this._newUserType = "active" /* Active */;
            }
            break;
          }
        }
    ...
    disposableStore.add(
      runOnChange(this._isHoveredOverIconDebounced, async (isHovered) => {
        if (isHovered) {
  ...
  _state = derived((reader) => {
    ...
    return {
      range,
      lineOffsetRange: this._editorObs.observeLineOffsetRange(
        range,
        this._store
      )
    };
  });
  _stickyScrollController = StickyScrollController.get(
    this._editorObs.editor
  );
  _stickyScrollHeight = this._stickyScrollController ? observableFromEvent(
  ...
  _lineNumberToRender = derived(this, (reader) => {
    ...
    if (lineNumberOptions.renderType === RenderLineNumbersType.Relative) {
      ...
      const relativeLineNumber = Math.abs(
  ...
  _availableWidthForIcon = derived(this, (reader) => {
    ...
    const lineNumberOptions = this._editorObs.getOption(EditorOption.lineNumbers).read(reader);
    if (lineNumberOptions.renderType === RenderLineNumbersType.Relative || lineNumberOptions.renderType === RenderLineNumbersType.Off) {
      return () => gutterWidth;
    }
    ...
    for (let digits = 1; digits <= totalLinesDigits; digits++) {
      ...
      const topOfLineNumber = editor.getTopForLineNumber(
      ...
      const usableWidthLeftOfLineNumber = Math.min(
        ...
        Math.max(
          0,
          rightOfLineNumber - digitsWidth - layout.glyphMarginLeft
        )
      );
      offsetDigits.push({
        firstLineNumberWithDigitCount,
    ...
    return (topOffset) => {
      ...
      throw new BugIndicatingError(
        "Could not find avilable width for icon"
      );
    };
  });
  _layout = derived(this, (reader) => {
    ...
    const gutterViewPortWithoutStickyScroll = gutterViewPortWithStickyScroll.withTop(
      this._stickyScrollHeight.read(reader) + gutterViewPortPadding
    );
    ...
    const iconDocked = derived(
      (reader2) => this._isHoveredOverIconDebounced.read(reader2) || this._isHoveredOverInlineEditDebounced.read(reader2) ? Codicon.check : iconNoneDocked.read(reader2)
    );
    const idealIconWidth = 22;
    const minimalIconWidth = 16;
    const iconWidth = /* @__PURE__ */ __name((pillRect2) => {
      const availableWidth = this._availableWidthForIcon.get()(
        pillRect2.bottom + this._editorObs.editor.getScrollTop()
      ) - gutterViewPortPadding;
      return Math.max(
        Math.min(availableWidth, idealIconWidth),
        minimalIconWidth
      );
    }, "iconWidth");
    ...
    const pillRectMoved = pillFullyDockedRect.moveToBeContainedIn(
      gutterViewPortWithStickyScroll
    );
  ...
  _showHover() {
    ...
    disposableStore.add(
      focusTracker.onDidBlur(
        () => this._focusIsInMenu.set(false, void 0)
      )
    );
    disposableStore.add(
      focusTracker.onDidFocus(
        () => this._focusIsInMenu.set(true, void 0)
      )
    );
    disposableStore.add(
      toDisposable(() => this._focusIsInMenu.set(false, void 0))
    );
      ...
      (layout) => !layout ? [] : [
        n.div({
          style: {
            ...
            borderRadius: "4px",
            ...rectToProps(
              (reader) => layout.read(reader).gutterEditArea
            )
          }
        }),
        n.div(
          {
            ...
            style: {
              ...
              zIndex: "1000",
              position: "absolute",
              backgroundColor: this._gutterIndicatorStyles.map(
                (v) => v.background
              ),
              ["--vscodeIconForeground"]: this._gutterIndicatorStyles.map(
                (v) => v.foreground
              ),
              border: this._gutterIndicatorStyles.map(
                (v) => `1px solid ${v.border}`
              ),
              ...
              ...rectToProps(
                (reader) => layout.read(reader).pillRect
              )
            }
          },
          [
            n.div(
              {
                ...
                style: {
                  lineHeight: layout.map(
                    (l) => l.lineNumberRect ? l.lineNumberRect.height : 0
                  ),
                  display: layout.map(
                    (l) => l.lineNumberRect ? "flex" : "none"
                  ),
                  alignItems: "center",
                  justifyContent: "flex-end",
                  width: layout.map(
                    (l) => l.lineNumberRect ? l.lineNumberRect.width : 0
                  ),
                  height: "100%",
                  color: this._gutterIndicatorStyles.map(
                    (v) => v.foreground
                  )
                }
              },
            ...
            n.div(
              {
                style: {
                  rotate: layout.map(
                    (l) => `${getRotationFromDirection(l.iconDirection)}deg`
                  ),
              ...
              [
                layout.map(
                  (l, reader) => renderIcon(l.icon.read(reader))
                )
              ]
            )
          ]
