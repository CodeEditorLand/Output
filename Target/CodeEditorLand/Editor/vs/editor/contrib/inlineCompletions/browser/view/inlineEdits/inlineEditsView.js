...
let InlineEditsViewAndDiffProducer = class extends Disposable {
  ...
  _differ = new LRUCachedFunction({ getCacheKey: JSON.stringify }, (arg) => {
  ...
  _inlineEditPromise = derived(this, (reader) => {
    ...
    return diffResult.promiseResult.map((p) => {
      ...
      return new InlineEditWithChanges(text, diffEdits, inlineEdit.isCollapsed, true, inlineEdit.commands);
    });
  });
  _inlineEdit = derivedOpts({ owner: this, equalsFn: equalsIfDefined(itemEquals()) }, (reader) => this._inlineEditPromise.read(reader)?.read(reader));
};
InlineEditsViewAndDiffProducer = __decorateClass([
  __decorateParam(3, IInstantiationService),
  __decorateParam(4, IDiffProviderFactoryService),
  __decorateParam(5, IModelService)
], InlineEditsViewAndDiffProducer);
class InlineEditWithChanges {
  constructor(originalText, edit, isCollapsed, showInlineIfPossible, commands) {
  ...
  equals(other) {
    return this.originalText.getValue() === other.originalText.getValue() && this.edit.equals(other.edit) && this.isCollapsed === other.isCollapsed && this.showInlineIfPossible === other.showInlineIfPossible && this.commands === other.commands;
  }
}
...
let InlineEditsView = class extends Disposable {
  ...
  _toolbar = this._register(this._instantiationService.createInstance(CustomizedMenuWorkbenchToolBar, this._elements.toolbar, MenuId.InlineEditsActions, {
  ...
  _extraCommands = derivedOpts({ owner: this, equalsFn: structuralEquals }, (reader) => {
    return this._uiState.read(reader)?.edit.commands ?? [];
  });
  _updateToolbarAutorun = this._register(autorun((reader) => {
  ...
  _previewEditorRootVisibility = derived(this, (reader) => this._uiState.read(reader)?.state === "sideBySide" ? "block" : "none");
  _updatePreviewEditorRootVisibility = derived((reader) => {
  ...
  _updatePreviewEditor = derived((reader) => {
    ...
    this._previewEditor.setHiddenAreas(hiddenAreas, void 0, true);
  ...
  */
  ...
  _inlineDiffView = this._register(new OriginalEditorInlineDiffView(this._editor, this._inlineDiffViewState));
  _indicator = this._register(new InlineEditsIndicator(
    this._editorObs,
    derived((reader) => {
      const state = this._uiState.read(reader);
