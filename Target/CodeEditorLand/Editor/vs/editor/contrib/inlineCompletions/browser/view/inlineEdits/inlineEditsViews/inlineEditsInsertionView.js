...
let InlineEditsInsertionView = class extends Disposable {
    ...
    this._ghostTextView = this._register(
      instantiationService.createInstance(
        ...
        this._editor,
        {
          ...
          targetTextModel: this._editorObs.model.map(
            (model) => model ?? void 0
          ),
  ...
  _trimVertically = derived(this, (reader) => {
  ...
  _maxPrefixTrim = derived((reader) => {
    ...
    const modifiedLines = lines.slice(
      trimVertically.linesTop,
      lines.length - trimVertically.linesBottom
    );
    ...
    const originalRange = new LineRange(
      state.lineNumber,
      state.lineNumber + (trimVertically.linesTop > 0 ? 0 : 1)
    );
    return getPrefixTrim([], originalRange, modifiedLines, this._editor);
  });
  _ghostText = derived((reader) => {
    const state = this._state.read(reader);
    ...
    const inlineDecorations = modifiedLines.map(
      (line, i) => new InlineDecoration(
        new Range(
          i + 1,
          i === 0 ? 1 : prefixTrim.prefixTrim + 1,
          i + 1,
          line.length + 1
        ),
        "modified-background",
  ...
  _editorMaxContentWidthInRange = derived(this, (reader) => {
    ...
    const textModel = this._editor.getModel();
    const eol = textModel.getEOL();
    const textBeforeInsertion = state.text.startsWith(eol) ? "" : textModel.getValueInRange(
      new Range(
        state.lineNumber,
        1,
        state.lineNumber,
  ...
  startLineOffset = this._trimVertically.map(
    (v) => v.topOffset
  );
  originalLines = this._state.map(
    (s) => s ? new LineRange(
      s.lineNumber,
      Math.min(
        s.lineNumber + 2,
        this._editor.getModel().getLineCount() + 1
      )
    ) : void 0
  );
  _overlayLayout = derivedWithStore(
    this,
    (reader, store) => {
      ...
      this._editorObs.observePosition(
        observableValue(
          this,
          new Position(state.lineNumber, state.column)
        ),
        store
      ).read(reader);
  ...
  _modifiedOverlay = n.div(
    ...
    derived((reader) => {
      const overlayLayoutObs = mapOutFalsy(this._overlayLayout).read(
        reader
      );
      if (!overlayLayoutObs) {
        return void 0;
      }
      const overlayHider = overlayLayoutObs.map(
      ...
      const overlayRect = overlayLayoutObs.map(
        (l) => l.overlay.withMargin(
          0,
          BORDER_WIDTH,
          0,
          l.startsAtContentLeft ? 0 : BORDER_WIDTH
        ).intersectHorizontal(
          new OffsetRange(
            overlayHider.left,
      ...
      const underlayRect = overlayRect.map(
        (rect) => rect.withMargin(
      ...
      return [
        ...
        n.div({
          ...
          style: {
            ...
            borderRadius: BORDER_RADIUS,
            border: getModifiedBorderColor(this._tabAction).map(
