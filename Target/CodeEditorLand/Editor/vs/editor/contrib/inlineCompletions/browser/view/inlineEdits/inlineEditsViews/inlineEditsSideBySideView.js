...
let InlineEditsSideBySideView = class extends Disposable {
    ...
    this._register(
      this._editorObs.createOverlayWidget({
        ...
        minContentWidthInPx: derived((reader) => {
          const x = this._previewEditorLayoutInfo.read(
            reader
          )?.maxContentWidth;
          if (x === void 0) {
            return 0;
  ...
  static {
    __name(this, "InlineEditsSideBySideView");
  }
  ...
  isHovered = this._editorContainer.didMouseMoveDuringHover;
  previewEditor = this._register(
  ...
  _cursorPosIfTouchesEdit = derived(this, (reader) => {
  ...
  _originalStartPosition = derived(this, (reader) => {
    const inlineEdit = this._edit.read(reader);
    return inlineEdit ? new Position(inlineEdit.originalLineRange.startLineNumber, 1) : null;
  });
  _originalEndPosition = derived(this, (reader) => {
    const inlineEdit = this._edit.read(reader);
    return inlineEdit ? new Position(
      inlineEdit.originalLineRange.endLineNumberExclusive,
      1
    ) : null;
  });
  _originalVerticalStartPosition = this._editorObs.observePosition(this._originalStartPosition, this._store).map((p) => p?.y);
  _originalVerticalEndPosition = this._editorObs.observePosition(this._originalEndPosition, this._store).map((p) => p?.y);
  _originalDisplayRange = this._uiState.map(
    (s) => s?.originalDisplayRange
  );
  ...
  _previewEditorLayoutInfo = derived(this, (reader) => {
    ...
    const selectionBottom = this._originalVerticalEndPosition.read(reader) ?? this._editor.getBottomForLineNumber(
  ...
  _shouldOverflow = derived((reader) => {
    if (!ENABLE_OVERFLOW) {
      return false;
    }
  ...
  _originalBackgroundColor = observableFromEvent(
    this,
    ...
    [
      n.svgElem("path", {
        ...
        d: derived((reader) => {
          ...
          return new PathBuilder().moveTo(layoutInfo.codeRect.getRightTop()).lineTo(
            layoutInfo.codeRect.getRightTop().deltaX(1e3)
          ).lineTo(
            layoutInfo.codeRect.getRightBottom().deltaX(1e3)
          ).lineTo(layoutInfo.codeRect.getRightBottom()).build();
        }),
        style: {
          fill: asCssVariableWithDefault(
            editorBackground,
            "transparent"
          )
        }
  ...
  _originalOverlay = n.div(
    {
      style: {
        pointerEvents: "none",
        display: this._previewEditorLayoutInfo.map(
          (layoutInfo) => layoutInfo?.isInsertion ? "none" : "block"
        )
      }
    },
    derived((reader) => {
      const layoutInfoObs = mapOutFalsy(
        this._previewEditorLayoutInfo
      ).read(reader);
      if (!layoutInfoObs) {
        return void 0;
      }
      const borderStyling = getOriginalBorderColor(
        this._tabAction
      ).map((bc) => `${BORDER_WIDTH}px solid ${asCssVariable(bc)}`);
      ...
      const overlayHider = layoutInfoObs.map(
        (layoutInfo) => Rect.fromLeftTopRightBottom(
      ...
      const overlayRect = layoutInfoObs.map(
        (layoutInfo) => layoutInfo.codeRect.intersectHorizontal(intersectionLine)
      );
      const separatorRect = overlayRect.map(
        (overlayRect2) => overlayRect2.withMargin(
      ...
      const transitionRect = overlayRect.map(
        (overlayRect2) => Rect.fromLeftTopWidthHeight(
          overlayRect2.right - transitionRectSize + BORDER_WIDTH,
          overlayRect2.bottom - BORDER_WIDTH,
          transitionRectSize,
          transitionRectSize
        ).intersectHorizontal(intersectionLine)
      );
        ...
        n.div(
          {
            ...
            style: {
              pointerEvents: "none",
              display: isModifiedLower.map(
                (isLower) => isLower ? "block" : "none"
              ),
  ...
  _modifiedOverlay = n.div(
    ...
    derived((reader) => {
      ...
      const borderStylingSeparator = `${BORDER_WIDTH + WIDGET_SEPARATOR_WIDTH}px solid ${asCssVariable(editorBackground)}`;
      const overlayRect = layoutInfoObs.map(
        (layoutInfo) => layoutInfo.editRect.withMargin(0, BORDER_WIDTH)
      );
      const separatorRect = overlayRect.map(
        (overlayRect2) => overlayRect2.withMargin(
          WIDGET_SEPARATOR_WIDTH,
      ...
      const insertionRect = derived((reader2) => {
      ...
      return [
        n.div({
          ...
          style: {
            ...
            backgroundColor: getModifiedBorderColor(
              this._tabAction
  ...
  _nonOverflowView = n.div(
    ...
    [
      ...
      derived(
        this,
        (reader) => this._shouldOverflow.read(reader) ? [] : [
