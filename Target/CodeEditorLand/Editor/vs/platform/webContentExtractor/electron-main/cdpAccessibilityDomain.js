import{URI as I}from"../../../base/common/uri.js";function L(t){if(t.length===0)return[];const e=new Map;for(const o of t)e.set(o.nodeId,o);function n(o){const i=e.get(o);if(!i||!i.childIds)return[];const r=[];for(const l of i.childIds){const u=e.get(l);u&&(u.ignored?r.push(...n(l)):r.push(l))}return r}const s=new Map;for(const o of t)o.ignored||s.set(o.nodeId,{node:o,children:[],parent:null});for(const o of t){if(o.ignored)continue;const i=s.get(o.nodeId);if(o.childIds)for(const r of o.childIds){const l=e.get(r);if(l)if(l.ignored){const u=n(r);for(const C of u){const p=s.get(C);p&&(p.parent=i,i.children.push(p))}}else{const u=s.get(r);u&&(u.parent=i,i.children.push(u))}}}const c=[];for(const o of s.values())o.parent||c.push(o);return c}const g=80;function A(t,e){const n=L(e);if(n.length===0)return"";const s=[],c=[];for(const i of n){const r=T(t,i),l=D(i);r.trim().length>0&&s.push(r),c.push(...l)}return s.join(`
