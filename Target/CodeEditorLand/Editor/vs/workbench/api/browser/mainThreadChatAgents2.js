...
import {
  IChatEditingService,
  IChatRelatedFileProviderMetadata
...
let MainThreadChatAgents2 = class extends Disposable {
  ...
  _chatRelatedFilesProviders = this._register(
    new DisposableMap()
  );
  ...
  $transferActiveChatSession(toWorkspace) {
    ...
    this._chatService.transferChatSession(
      { sessionId, inputValue, location, mode },
  ...
  $registerRelatedFilesProvider(handle, metadata) {
    this._chatRelatedFilesProviders.set(
      handle,
      this._chatEditingService.registerRelatedFilesProvider(handle, {
        description: metadata.description,
        provideRelatedFiles: /* @__PURE__ */ __name(async (request, token) => {
          return (await this._proxy.$provideRelatedFiles(
            handle,
            request,
            token
          ))?.map((v) => ({
