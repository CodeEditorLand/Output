...
let ExtHostLanguageModels = class {
    ...
    try {
      if (data.provider.provideLanguageModelResponse2) {
        value = data.provider.provideLanguageModelResponse2(
          messages.value.map(
            typeConvert.LanguageModelChatMessage2.to
          ),
          options,
  ...
  async $acceptResponseDone(requestId, error) {
    ...
    if (error) {
      data.res.reject(
        extHostTypes.LanguageModelError.tryDeserialize(error) ?? transformErrorFromSerialization(error)
      );
    } else {
      data.res.resolve();
    }
  ...
  async _computeTokenLength(languageModelId, value, token) {
    ...
    return this._proxy.$countTokens(
      languageModelId,
      typeof value === "string" ? value : typeConvert.LanguageModelChatMessage2.from(value),
      token
    );
  }
