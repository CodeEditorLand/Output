  ...
  function from(extContext) {
    return {
      ...
      conversation: extContext.conversation?.map((item) => item.type === "request" ? {
        ...
        type: "response",
        message: item.message,
        result: item.result ? ChatAgentResult.from(item.result) : void 0,
...
((ChatPromptReference2) => {
  function to(variable) {
    ...
    return {
      ...
      value: isUriComponents(value) ? URI.revive(value) : value && typeof value === "object" && "uri" in value && "range" in value && isUriComponents(value.uri) ? Location.to(revive(value)) : variable.isImage ? new types.ChatReferenceBinaryData(variable.mimeType ?? "image/png", () => Promise.resolve(new Uint8Array(Object.values(value)))) : value,
...
((LanguageModelToolDescription2) => {
  function to(item) {
    return {
      ...
      parametersSchema: item.inputSchema,
