...
function registerChatActions() {
          ...
          store.add(
            picker.onDidTriggerItemButton(async (context) => {
              if (context.button === openInEditorButton) {
                const options = {
                  target: {
                    sessionId: context.item.chat.sessionId
                  },
                  pinned: true
                };
  ...
  registerAction2(
    class ChatAddAction extends Action2 {
      ...
      async run(accessor, ...args) {
        ...
        const hasAgentOrCommand = extractAgentAndCommand(
          widget.parsedInput
        );
    ...
    class ConfigureCopilotCompletions extends Action2 {
      ...
      async run(accessor) {
        ...
        commandService.executeCommand(
          defaultChat.completionsMenuCommand
        );
      }
    }
  );
  registerAction2(
    class ShowLimitReachedDialogAction extends Action2 {
      static {
        __name(this, "ShowLimitReachedDialogAction");
      }
      constructor() {
        super({
      ...
      async run(accessor) {
        const chatEntitlementService = accessor.get(
          IChatEntitlementService
        );
        const commandService = accessor.get(ICommandService);
        const dialogService = accessor.get(IDialogService);
        ...
        if (chatQuotaExceeded && !completionsQuotaExceeded) {
          message = localize(
            ...
            dateFormatter.format(
              chatEntitlementService.quotas.quotaResetDate
            )
          );
    ...
    constructor() {
      super(
        ...
        localize(
          ...
          "Toggle visibility of the Copilot Controls in title bar"
        ),
        5,
        false,
        ChatContextKeys.supported
...
let CopilotTitleBarMenuRendering = class extends Disposable {
  static {
    __name(this, "CopilotTitleBarMenuRendering");
  }
  static ID = "workbench.contrib.copilotTitleBarMenuRendering";
        ...
        if (!chatExtensionInstalled && (!setupFromDialog || chatHidden)) {
          ...
          if (chatQuotaExceeded && !completionsQuotaExceeded) {
            primaryActionTitle = localize(
              ...
              "Monthly chat messages limit reached. Click for details."
            );
          } else if (completionsQuotaExceeded && !chatQuotaExceeded) {
            primaryActionTitle = localize(
              "completionsQuotaExceededButton",
              "Monthly code completions limit reached. Click for details."
            );
          } else {
            primaryActionTitle = localize(
              "chatAndCompletionsQuotaExceededButton",
              "Copilot Free plan limit reached. Click for details."
            );
          }
...
CopilotTitleBarMenuRendering = __decorateClass([
...
function getEditsViewId(accessor) {
  const chatService = accessor.get(IChatService);
  return chatService.unifiedViewEnabled ? ChatViewId : EditsViewId;
}
__name(getEditsViewId, "getEditsViewId");
...
async function showClearEditingSessionConfirmation(editingSession, dialogService, options) {
  ...
  const undecidedEdits = currentEdits.filter(
    (edit) => edit.state.get() === WorkingSetEntryState.Modified
  );
