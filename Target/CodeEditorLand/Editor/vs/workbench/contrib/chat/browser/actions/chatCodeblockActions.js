...
import {
  ...
  ICodeCompareBlockActionContext
          ...
          class extends MenuEntryActionViewItem {
            getTooltip() {
              ...
              if (isCodeBlockActionContext(context) && context.codemapperUri) {
                const label = labelService.getUriLabel(
...
function registerChatCodeBlockActions() {
  ...
  registerAction2(
    class InsertAtCursorAction extends ChatCodeBlockAction {
      static {
  ...
  function navigateCodeBlocks(accessor, reverse) {
    ...
    const currentResponse = element ?? focusedResponse ?? widget.viewModel?.getItems().reverse().find(
  ...
  registerAction2(
    class ApplyEditsCompareBlockAction extends ChatCompareCodeBlockAction {
      ...
      async runWithContext(accessor, context) {
        ...
        const editorToApply = await editorService.openCodeEditor(
          { resource: item.uri },
          null
        );
        if (editorToApply) {
