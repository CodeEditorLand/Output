  ...
  constructor() {
    const precondition = ContextKeyExpr.and(
      ...
      ContextKeyExpr.or(
        ...
        ChatContextKeys.instructionsAttached
      ),
...
class ToggleChatModeAction extends Action2 {
  ...
  async run(accessor, ...args) {
    ...
    const switchToMode = validateChatMode(arg?.mode) ?? this.getNextMode(
      context.chatWidget,
      requestCount,
      configurationService
    );
    const needToClearEdits = (!chatService.unifiedViewEnabled || !configurationService.getValue(
      ChatConfiguration.Edits2Enabled
    ) && (context.chatWidget.input.currentMode === ChatMode.Edit || switchToMode === ChatMode.Edit)) && requestCount > 0;
...
class SendToChatEditingAction extends Action2 {
  ...
  async run(accessor, ...args) {
    ...
    const dialogService = accessor.get(IDialogService);
    const { widget: editingWidget } = await viewsService.openView(
      EditsViewId
    );
