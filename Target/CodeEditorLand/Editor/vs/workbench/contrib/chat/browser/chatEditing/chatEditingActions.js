...
function getEditingSessionContext(accessor, args) {
  ...
  let chatWidget = context ? chatWidgetService.getWidgetBySessionId(context.sessionId) : void 0;
  if (!chatWidget) {
...
__name(getEditingSessionContext, "getEditingSessionContext");
...
registerAction2(
  class RemoveFileFromWorkingSet extends WorkingSetAction {
    static {
    ...
    async runWorkingSetAction(accessor, currentEditingSession, chatWidget, ...uris) {
      ...
      const pendingEntries = currentEditingSession.entries.get().filter(
        (entry) => uris.includes(entry.modifiedURI) && entry.state.get() === WorkingSetEntryState.Modified
      );
      if (pendingEntries.length > 0) {
        const file = pendingEntries.length > 1 ? localize(
          "chat.editing.removeFile.confirmationmanyFiles",
          "{0} files",
          pendingEntries.length
        ) : basename(pendingEntries[0].modifiedURI);
        const confirmation = await dialogService.confirm({
          title: localize(
            "chat.editing.removeFile.confirmation.title",
            "Remove {0} from working set?",
            file
          ),
          message: localize(
            "chat.editing.removeFile.confirmation.message",
            "This will remove {0} from your working set and undo the edits made to it. Do you want to proceed?",
            file
          ),
          primaryButton: localize(
            "chat.editing.removeFile.confirmation.primaryButton",
            "Yes"
          ),
          type: "info"
        });
      ...
      currentEditingSession.remove(
        WorkingSetEntryRemovalReason.User,
        ...uris
      );
      ...
      for (const uri of uris) {
        ...
        if (editedFile?.state.get() === WorkingSetEntryState.Modified) {
          await editorService.openEditor({
            original: {
              resource: URI.from(editedFile.originalURI, true)
            },
            modified: {
              resource: URI.from(editedFile.modifiedURI, true)
            }
          });
        } else {
...
__name(discardAllEditsWithConfirmation, "discardAllEditsWithConfirmation");
class ChatEditingRemoveAllFilesAction extends EditingSessionAction {
  static {
    __name(this, "ChatEditingRemoveAllFilesAction");
  }
  static ID = "chatEditing.clearWorkingSet";
  constructor() {
    super({
  ...
  async runEditingSessionAction(accessor, editingSession, chatWidget, ...args) {
    const uris = [...editingSession.entries.get()].map(
      (e) => e.modifiedURI
    );
    ...
    const attachmentIdsToRemove = fileAttachments.map(
...
registerAction2(
  class AddFilesToWorkingSetAction extends EditingSessionAction {
    static {
    ...
    constructor() {
      super({
        ...
        title: localize2(
          "workbench.action.chat.addSelectedFilesToWorkingSet.label",
          "Add Selected Files to Working Set"
        ),
        icon: Codicon.attach,
        precondition: ChatContextKeys.location.isEqualTo(
    ...
    async run(accessor, ...args) {
      ...
      if (requestId) {
        ...
        const entriesModifiedInRequestsToRemove = session.entries.get().filter(
          (entry) => requestIdsToRemove.has(
            entry.lastModifyingRequestId
          )
        ) ?? [];
        ...
        if (editsToUndo === 1) {
          if (entriesModifiedInRequestsToRemove.length === 1) {
            message = localize(
              ...
              basename(
                entriesModifiedInRequestsToRemove[0].modifiedURI
              )
            );
          } else {
            message = localize(
...
registerAction2(
  class OpenWorkingSetHistoryAction extends Action2 {
    ...
    static id = "chat.openFileUpdatedBySnapshot";
    constructor() {
      super({
...
registerAction2(
  class OpenWorkingSetHistoryAction2 extends Action2 {
    static {
      __name(this, "OpenWorkingSetHistoryAction");
    }
    ...
    constructor() {
      super({
        id: OpenWorkingSetHistoryAction2.id,
        title: localize(
    ...
    async run(accessor, ...args) {
      ...
      const snapshot = chatEditingService.getEditingSession(chatModel.sessionId)?.getSnapshotUri(
        context.requestId,
        context.uri,
        context.stopId
      );
...
registerAction2(
  class ResolveSymbolsContextAction extends EditingSessionAction {
    static {
      __name(this, "ResolveSymbolsContextAction");
    }
    constructor() {
      super({
    ...
    async runEditingSessionAction(accessor, editingSession, chatWidget, ...args) {
      ...
      for (const reference of [
        ...
        attachments.push(
          chatWidget.attachmentModel.asVariableEntry(reference.uri)
        );
      }
    ...
    async getImplementations(position, textModel, languageFeaturesService) {
      ...
      const implementations = await Promise.all(
        implementationProviders.map(async (implementationProvider) => {
          return await implementationProvider.provideImplementation(
            textModel,
            position,
            CancellationToken.None
          ) ?? [];
