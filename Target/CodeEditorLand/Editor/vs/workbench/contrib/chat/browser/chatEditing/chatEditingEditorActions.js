...
} from "./chatEditingEditorContextKeys.js";
class ChatEditingEditorAction extends Action2 {
  ...
  async run(accessor, ...args) {
    ...
    return instaService.invokeFunction(
      this.runChatEditingCommand.bind(this),
      session,
      entry,
      ctrl,
      ...args
...
class NavigateAction extends ChatEditingEditorAction {
  ...
  async runChatEditingCommand(accessor, session, entry, ctrl) {
    ...
    const didOpenNext = await instaService.invokeFunction(
      openNextOrPreviousChange,
      session,
      entry,
      this.next
    );
  ...
  constructor(id, accept) {
    super({
      ...
      menu: {
        ...
        order: accept ? 0 : 1,
        when: ContextKeyExpr.and(
          !accept ? ctxReviewModeEnabled : void 0,
          ctxHasRequestInProgress.negate()
        )
      }
    });
    ...
    super({
      ...
      toggled: {
        condition: ContextKeyExpr.or(
          EditorContextKeys.inDiffEditor,
          ActiveEditorContext.isEqualTo(TEXT_DIFF_EDITOR_ID)
        ),
...
function registerChatEditorActions() {
  registerAction2(
    class NextAction extends NavigateAction {
      static {
  ...
  registerAction2(
    class PrevAction extends NavigateAction {
      static {
  ...
  registerAction2(
    class extends MultiDiffAcceptDiscardAction {
      constructor() {
        super(true);
      }
    }
