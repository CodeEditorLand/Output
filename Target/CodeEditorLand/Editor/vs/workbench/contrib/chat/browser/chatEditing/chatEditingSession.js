...
import {
  ...
  ISingleOffsetEdit,
  OffsetEdit
} from "../../../../../editor/common/core/offsetEdit.js";
...
let ChatEditingSession = class extends Disposable {
  ...
  _ignoreTrimWhitespaceObservable = observableConfigValue(
    ...
    if (next) {
      if (stopIndex === snap.stops.length - 1) {
        const postEdit = new ResourceMap(
          snap.postEdit || this._createSnapshot(void 0, void 0).entries
        );
  ...
  async _createModifiedFileEntry(resource, telemetryInfo, mustExist = false, initialContent) {
    ...
    try {
      if (this._notebookService.hasSupportedNotebooks(notebookUri) && (this._notebookService.getNotebookTextModel(notebookUri) || ChatEditingModifiedNotebookEntry.canHandleSnapshotContent(
        initialContent
      ))) {
...
let ChatEditingSessionStorage = class {
  ...
  async restoreState() {
    ...
    const getFileContent = /* @__PURE__ */ __name((hash) => {
      ...
      if (!readPromise) {
        readPromise = this._fileService.readFile(
          joinPath(
            storageLocation,
            STORAGE_CONTENTS_FOLDER,
            hash
          )
    ...
    const deserializeResourceMap = /* @__PURE__ */ __name((resourceMap, deserialize, result) => {
    ...
    const deserializeChatEditingStopDTO = /* @__PURE__ */ __name(async (stopDTO) => {
      const entries = await deserializeSnapshotEntriesDTO(
        stopDTO.entries
      );
      const workingSet = deserializeResourceMap(
        stopDTO.workingSet,
        (value) => value,
        new ResourceMap()
      );
      return {
        stopId: "stopId" in stopDTO ? stopDTO.stopId : void 0,
        workingSet,
        entries
      };
    ...
    const deserializeSnapshotEntry = /* @__PURE__ */ __name(async (entry) => {
      return {
        ...
        originalToCurrentEdit: OffsetEdit.fromJson(
          entry.originalToCurrentEdit
        ),
        state: entry.state,
  ...
  async storeState(state) {
    ...
    const contentsFolder = URI.joinPath(
      storageFolder,
      STORAGE_CONTENTS_FOLDER
    );
    ...
    const serializeResourceMap = /* @__PURE__ */ __name((resourceMap, serialize) => {
      return Array.from(resourceMap.entries()).map(
        ([resourceURI, value]) => [
    ...
    const serializeChatEditingSessionStop = /* @__PURE__ */ __name((stop) => {
      return {
        stopId: stop.stopId,
        workingSet: serializeResourceMap(
          stop.workingSet,
          (value) => value
        ),
        entries: Array.from(stop.entries.values()).map(
    ...
    const serializeChatEditingSessionSnapshot = /* @__PURE__ */ __name((snapshot) => {
      return {
        ...
        postEdit: snapshot.postEdit ? Array.from(snapshot.postEdit.values()).map(
          serializeSnapshotEntry
        ) : void 0
      };
    ...
    try {
      const data = {
        ...
        recentSnapshot: serializeChatEditingSessionStop(
      ...
      await this._fileService.writeFile(
        ...
        VSBuffer.fromString(JSON.stringify(data, void 0, 2))
      );
    } catch (e) {
      this._logService.debug(
