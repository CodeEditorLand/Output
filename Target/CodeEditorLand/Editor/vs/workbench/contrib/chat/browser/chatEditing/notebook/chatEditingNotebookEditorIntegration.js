...
import {
  ...
  IDocumentDiff2
} from "../chatEditingCodeEditorIntegration.js";
import { ChatEditingModifiedNotebookEntry } from "../chatEditingModifiedNotebookEntry.js";
import {
  countChanges,
  ICellDiffInfo,
  sortCellChanges
} from "./notebookCellChanges.js";
...
let ChatEditingNotebookEditorWidgetIntegration = class extends Disposable {
  constructor(_entry, notebookEditor, notebookModel, originalModel, cellChanges, instantiationService, _editorService, _chatAgentService, notebookEditorService, accessibilitySignalService) {
    ...
    const onDidChangeVisibleRanges = debouncedObservable(
      observableFromEvent(
    ...
    const shouldBeReadonly = _entry.isCurrentlyBeingModifiedBy.map(
      (value) => !!value
    );
    this._register(
      autorun((r) => {
        const isReadOnly = shouldBeReadonly.read(r);
        const notebookEditor2 = notebookEditorService.retrieveExistingWidgetFromURI(
          _entry.modifiedURI
        )?.value;
        if (!notebookEditor2) {
          return;
    ...
    this._store.add(
      autorun((r) => {
        if (!_entry.isCurrentlyBeingModifiedBy.read(r) && !_entry.isProcessingResponse.read(r) && lastModifyingRequestId !== _entry.lastModifyingRequestId && cellChanges.read(r).some(
          (c) => c.type !== "unchanged" && !c.diff.read(r).identical
        )) {
    ...
    this._register(
      autorun((r) => {
        ...
        const changes = sortedCellChanges.filter(
          (c) => c.type !== "delete"
        );
        ...
        changes.forEach((change) => {
          ...
          if (!editor) {
            if (!this.markupCellListeners.has(cell.handle) && cell.cellKind === CellKind.Markup) {
              const cellModel = this.notebookEditor.getViewModel()?.viewCells.find(
                ...
                const listener = cellModel.onDidChangeEditorAttachState(
                  () => {
                    if (cellModel.editorAttached) {
                      this.mdCellEditorAttached.set(
                        cell.handle,
                        void 0
                      );
                      listener.dispose();
                      this.markupCellListeners.delete(
                        cell.handle
                      );
                    }
                  }
                );
                this.markupCellListeners.set(
                  cell.handle,
                  listener
                );
              }
          ...
          if (currentDiff) {
            ...
            const diff2 = observableValue(
              `diff${cell.handle}`,
              diff
            );
            const integration = this.instantiationService.createInstance(
              ChatEditingCodeEditorIntegration,
              _entry,
              editor,
              diff2
    ...
    const cellsAreVisible = onDidChangeVisibleRanges.map(
      (v) => v.length > 0
    );
    ...
    this._register(
      autorun((r) => {
        ...
        const changes = debouncedChanges.read(r).filter(
          (c) => c.type === "insert" ? !c.diff.read(r).identical : true
        );
      ...
      this.instantiationService.createInstance(
        ...
        this.notebookEditor,
        {
          ...
          argFactory: /* @__PURE__ */ __name((deletedCellIndex) => {
            return {
              accept() {
                const entry = cellChanges.find(
                  (c) => c.type === "delete" && c.originalCellIndex === deletedCellIndex
                );
                if (entry) {
                  return entry.keep(
                    entry.diff.get().changes[0]
                  );
                }
  ...
  reveal(firstOrLast) {
    const changes = sortCellChanges(
      this.cellChanges.get().filter((c) => c.type !== "unchanged")
    );
    if (!changes.length) {
      return void 0;
    }
  ...
  _revealChange(change, indexInCell) {
    switch (change.type) {
      ...
      case "modified": {
        ...
        if (cellViewModel) {
          this.revealChangeInView(
            cellViewModel,
            textChange?.modified
          );
          this._currentChange.set(
            { change, index: indexInCell },
            void 0
          );
        }
        return true;
  ...
  async revealChangeInView(cell, lines) {
    ...
    await this.notebookEditor.revealRangeInCenterAsync(
      cell,
      new Range(
        targetLines.startLineNumber,
        0,
        targetLines.endLineNumberExclusive,
        0
      )
    );
  }
  next(wrap) {
    ...
    switch (currentChange.change.type) {
      case "modified":
        {
          const cellIntegration = this.getCell(
            currentChange.change.modifiedCellIndex
          );
          if (cellIntegration) {
              ...
              this._currentChange.set(
                {
                  ...
                  index: cellIntegration.currentIndex.get()
                },
                void 0
              );
              return true;
  ...
  async toggleDiff(_change) {
    const defaultAgentName = this._chatAgentService.getDefaultAgent(
      ChatAgentLocation.EditingSession
    )?.fullName;
    const diffInput = {
      original: {
        resource: this._entry.originalURI,
        options: { selection: void 0 }
      },
      modified: {
        resource: this._entry.modifiedURI,
        options: { selection: void 0 }
      },
      label: defaultAgentName ? localize(
        "diff.agent",
        "{0} (changes from {1})",
    ...
    this._store.add(
      autorun((r) => {
        ...
        if (numberOfCellChanges.length && index >= 0 && index < numberOfCellChanges.length) {
          const changesSoFar = countChanges(
            numberOfCellChanges.slice(0, index + 1)
          );
