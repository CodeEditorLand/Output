  ...
  constructor(configService, chatEditingService, chatAgentService, textFileService, labelService, dialogService, _fileConfigService) {
    ...
    const update = /* @__PURE__ */ __name(() => {
      ...
      const saveJobs = new class {
        _deferred;
        _soon = new RunOnceScheduler(() => this._prompt(), 0);
        _uris = new ResourceSet();
        add(uri) {
          this._uris.add(uri);
          this._soon.schedule();
        ...
        async _prompt() {
          ...
          const message = this._uris.size === 1 ? localize("message.1", "Do you want to save the changes {0} made in {1}?", agentName, filelabel) : localize("message.2", "Do you want to save the changes {0} made to {1} files?", agentName, this._uris.size);
      ...
      menu: [
        ...
        },
        {
          ...
          when: ContextKeyExpr.and(
            ...
            ContextKeyExpr.notEquals("config.files.autoSave", "off"),
