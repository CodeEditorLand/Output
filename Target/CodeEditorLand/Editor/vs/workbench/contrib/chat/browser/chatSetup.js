...
import {
  ...
  IChatViewsWelcomeContributionRegistry
} from "./viewsWelcome/chatViewsWelcome.js";
const defaultChat = {
    ...
    return instantiationService.invokeFunction((accessor) => {
      ...
      switch (location) {
        ...
        case ChatAgentLocation.EditingSession:
          ...
          description = isToolsAgent ? localize(
            "agentDescription",
...
${baseMessage}`
      ...
      disposable.add(
        chatAgentService.registerAgentImplementation(id, agent)
      );
      return { agent, disposable };
    });
  }
  static SETUP_NEEDED_MESSAGE = new MarkdownString(
    localize(
  ...
  whenLanguageModelReady(languageModelsService) {
    ...
    return Event.toPromise(
      Event.filter(
        ...
        (e) => e.added?.some((added) => added.metadata.isDefault) ?? false
      )
    );
  }
  whenAgentReady(chatAgentService) {
    ...
    return Event.toPromise(
      Event.filter(chatAgentService.onDidChangeAgents, () => {
        const defaultAgent2 = chatAgentService.getDefaultAgent(
          this.location
        );
  ...
  async doInvokeWithSetup(request, progress, chatService, languageModelsService, chatWidgetService, chatAgentService) {
            ...
            progress({
              ...
              content: new MarkdownString(
                localize(
                  ...
                  ) === defaultChat.enterpriseProviderId ? defaultChat.enterpriseProviderName : defaultChat.providerName
                )
              )
            });
            break;
    ...
    let success = void 0;
    try {
      success = await ChatSetup.getInstance(
...
let ChatSetup = class {
            ...
            primaryButtonDropdown: {
              ...
              actions: [
                toAction({
                  ...
                  label: localize(
                    "setupWithProvider",
                    "Sign in with a {0} Account",
                    defaultChat.providerName
                  ),
                  run: /* @__PURE__ */ __name(() => result = 2 /* SetupWithoutEnterpriseProvider */, "run")
                }),
                toAction({
                  ...
                  run: /* @__PURE__ */ __name(() => result = 3 /* SetupWithEnterpriseProvider */, "run")
                })
              ]
            }
          },
  ...
  createDialog(disposables) {
    ...
    const header = localize(
      { key: "headerDialog", comment: ['{Locked="[Copilot]({0})"}'] },
    ...
    element.appendChild(
      $(
        ...
        disposables.add(
          markdown.render(
            new MarkdownString(header, { isTrusted: true })
          )
        ).element
      )
    );
    ...
    element.appendChild(
      $(
        ...
        disposables.add(
          markdown.render(
            new MarkdownString(terms, { isTrusted: true })
          )
        ).element
      )
    );
    if (this.telemetryService.telemetryLevel !== TelemetryLevel.NONE) {
      ...
      element.appendChild(
        $(
          ...
          disposables.add(
            markdown.render(
              new MarkdownString(settings, { isTrusted: true })
            )
          ).element
        )
      );
...
let ChatSetupContribution = class extends Disposable {
  ...
  registerSetupAgents(context, controller) {
    ...
    const updateRegistration = /* @__PURE__ */ __name(() => {
      ...
      if (!disabled && !registration.value) {
        const { agent: panelAgent, disposable: panelDisposable } = SetupChatAgentImplementation.register(
          this.instantiationService,
        ...
        registration.value = combinedDisposable(
          panelDisposable,
          SetupChatAgentImplementation.register(
            this.instantiationService,
      ...
      Event.runAndSubscribe(
        Event.any(
          ...
          Event.filter(
            ...
            (e) => e.affectsConfiguration("chat.setupFromDialog")
          )
        ),
        () => updateRegistration()
      )
  ...
  registerChatWelcome(context, controller) {
    Registry.as(
      ...
      icon: Codicon.copilotLarge,
      content: /* @__PURE__ */ __name((disposables) => disposables.add(
        this.instantiationService.createInstance(
  ...
  registerActions(context, requests, controller) {
    ...
    class ChatSetupTriggerAction extends Action2 {
      ...
      async run(accessor, mode) {
        ...
        const chatWidgetPromise = showCopilotView(
          viewsService,
          layoutService
        );
        if (mode) {
        ...
        const setupFromDialog = configurationService.getValue(
        ...
        statusbarService.updateEntryVisibility(
          "chat.statusBarEntry",
          true
        );
    ...
    class ChatSetupHideAction extends Action2 {
      ...
      async run(accessor) {
        const viewsDescriptorService = accessor.get(
          IViewDescriptorService
        );
        const layoutService = accessor.get(IWorkbenchLayoutService);
        const configurationService = accessor.get(
        ...
        const { confirmed } = await dialogService.confirm({
          message: localize(
            ...
            "Are you sure you want to hide Copilot?"
          ),
          detail: localize(
            "hideChatSetupDetail",
        ...
        if (location === ViewContainerLocation.AuxiliaryBar) {
          const activeContainers = viewsDescriptorService.getViewContainersByLocation(location).filter(
            (container) => viewsDescriptorService.getViewContainerModel(
              container
    ...
    class UpgradePlanAction extends Action2 {
      ...
      async run(accessor, from) {
        ...
        if (entitlement !== ChatEntitlement.Pro) {
          windowFocusListener.value = hostService.onDidChangeFocus(
            (focus) => this.onWindowFocus(focus, commandService)
          );
        }
      }
    ...
    this._register(
      ExtensionUrlHandlerOverrideRegistry.registerHandler({
        canHandleURL: /* @__PURE__ */ __name((url) => {
          return url.scheme === this.productService.urlProtocol && equalsIgnoreCase(
            url.authority,
        ...
        handleURL: /* @__PURE__ */ __name(async (url) => {
          ...
          this.telemetryService.publicLog2("workbenchActionExecuted", {
            id: CHAT_SETUP_ACTION_ID,
            from: "url",
            detail: params.get("referrer") ?? void 0
          });
...
let ChatSetupController = class extends Disposable {
  ...
  registerListeners() {
    this._register(
      this.context.onDidChange(() => this._onDidChange.fire())
    );
  }
  setStep(step) {
    if (this._step === step) {
  ...
  async setup(options) {
    ...
    const badge = this.activityService.showViewContainerActivity(
      preferCopilotEditsView(this.viewsService) ? CHAT_EDITING_SIDEBAR_PANEL_ID : CHAT_SIDEBAR_PANEL_ID,
      {
  ...
  async install(session, entitlement, providerId, options, watch) {
    ...
    try {
      ...
      entitlement !== ChatEntitlement.Unavailable) {
        if (!session) {
          try {
            session = (await this.authenticationService.getSessions(
              providerId
            )).at(0);
          } catch (error) {
          }
  ...
  async handleEnterpriseInstance() {
    ...
    const uri = this.configurationService.getValue(
      defaultChat.providerUriSetting
    );
...
let ChatSetupWelcomeContent = class extends Disposable {
  ...
  create() {
    ...
    {
      const header = localize(
        { key: "header", comment: ['{Locked="[Copilot]({0})"}'] },
    ...
    const free = localize(
      { key: "free", comment: ['{Locked="[]({0})"}'] },
      "$(sparkle-filled) We now offer [Copilot for free]({0}).",
      defaultChat.skusDocumentationUrl
    );
    const freeContainer = this.element.appendChild(
      $(
        "p",
        void 0,
        this._register(
          markdown.render(
            new MarkdownString(free, {
              isTrusted: true,
      ...
      new ButtonWithDropdown(buttonContainer, {
        actions: [
          toAction({
            ...
            run: /* @__PURE__ */ __name(() => this.controller.setupWithProvider({
              useEnterpriseProvider: false
            }), "run")
          }),
          toAction({
    ...
    const settingsContainer = this.element.appendChild(
      $(
        "p",
        void 0,
        this._register(
    ...
    this._register(
      Event.runAndSubscribe(
        ...
        () => this.update(freeContainer, settingsContainer, button)
      )
    );
  }
  update(freeContainer, settingsContainer, button) {
    ...
    switch (this.controller.step) {
      ...
      case 3 /* Installing */:
        buttonLabel = localize(
          "setupChatInstalling",
          "$(loading~spin) Getting Copilot Ready..."
        );
        break;
    }
