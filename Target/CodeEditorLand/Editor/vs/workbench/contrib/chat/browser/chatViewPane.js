...
let ChatViewPane = class extends ViewPane {
  constructor(chatOptions, options, keybindingService, contextMenuService, configurationService, contextKeyService, viewDescriptorService, instantiationService, openerService, themeService, hoverService, storageService, chatService, chatAgentService, logService, layoutService) {
    ...
    if (this.chatService.unifiedViewEnabled && this.chatOptions.location === ChatAgentLocation.Panel && !this.viewState.hasMigratedCurrentSession) {
      ...
      if (lastEditsState.sessionId) {
        ...
        if (!this.chatService.isPersistedSessionEmpty(
          lastEditsState.sessionId
        )) {
      ...
      this.chatAgentService.onDidChangeAgents(() => {
        if (this.chatAgentService.getDefaultAgent(
          ...
          if (!this._widget?.viewModel && !this._restoringSession) {
            ...
            this._restoringSession = (info.sessionId ? this.chatService.getOrRestoreSession(
              info.sessionId
            ) : Promise.resolve(void 0)).then(async (model) => {
  ...
  async updateModel(model, viewState) {
    ...
    model = model ?? (this.chatService.transferredSessionData?.sessionId && this.chatService.transferredSessionData?.location === this.chatOptions.location ? await this.chatService.getOrRestoreSession(
  ...
  shouldShowWelcome() {
    const showSetup = this.contextKeyService.contextMatchesRules(
      ChatContextKeys.SetupViewCondition
    );
          ...
          { viewId: this.id },
          {
            ...
            rendererOptions: {
              renderTextEditsAsSummary: /* @__PURE__ */ __name((uri) => {
                return this.chatService.isEditingLocation(
                  this.chatOptions.location
                );
            ...
            enableImplicitContext: this.chatOptions.location === ChatAgentLocation.Panel || this.chatService.isEditingLocation(
              this.chatOptions.location
            ),
            enableWorkingSet: this.chatService.isEditingLocation(
              this.chatOptions.location
            ) ? "explicit" : void 0,
            supportsChangingModes: this.chatService.isEditingLocation(
              this.chatOptions.location
            )
          },
          {
...
ChatViewPane = __decorateClass([
  ...
  __decorateParam(15, ILayoutService)
], ChatViewPane);
export {
