...
let ChatWidget = class extends Disposable {
      ...
      this.tree.setChildren(null, treeItems, {
        diffIdentityProvider: {
          getId: /* @__PURE__ */ __name((element) => {
            ...
            `_${element.isHidden ? "1" : "0"}` + // Rerender request if we got new content references in the response
  ...
  async _acceptInput(query, options) {
    if (this.viewModel) {
      ...
      const input = !query ? editorValue : "query" in query ? query.query : `${query.prefix} ${editorValue}`;
