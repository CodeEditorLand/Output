...
import {
  ...
  IChatWidget,
  IChatWidgetService,
  IChatWidgetViewContext,
  IChatWidgetViewOptions
} from "./chat.js";
...
let ChatWidget = class extends Disposable {
  constructor(location, _viewContext, viewOptions, styles, codeEditorService, configurationService, contextKeyService, instantiationService, chatService, chatAgentService, chatWidgetService, contextMenuService, chatAccessibilityService, logService, themeService, chatSlashCommandService, chatEditingService, storageService, telemetryService) {
    ...
    ChatContextKeys.inUnifiedChat.bindTo(contextKeyService).set(
      this._location.location === ChatAgentLocation.Panel && !!this.viewOptions.supportsChangingModes && this.configurationService.getValue(
      ...
      bindContextKey(
        ...
        (reader) => {
          ...
          const decidedEntries = entries.filter(
            (entry) => entry.state.read(reader) !== WorkingSetEntryState.Modified
          );
  ...
  get isUnifiedPanelWidget() {
    return this._location.location === ChatAgentLocation.Panel && !!this.viewOptions.supportsChangingModes && this.configurationService.getValue(
      ChatConfiguration.UnifiedChatView
    );
  }
  _lastSelectedAgent;
  ...
  async _acceptInput(query, options) {
    ...
    if (this.viewModel) {
      ...
      if (this.viewOptions.enableWorkingSet !== void 0) {
  ...
  layout(height, width) {
    ...
    if (this.viewOptions.renderFollowups) {
      ...
      extraOffset = Math.max(
        100 - this.inputPart.editSessionWidgetHeight,
        0
      );
    }
...
__decorateClass([
  memoize
], ChatWidget.prototype, "isUnifiedPanelWidget", 1);
