                ...
                (c, i) => {
                  ...
                  return {
                    ...
                    command: c.executeImmediately ? {
                      id: widget.location === ChatAgentLocation.EditingSession ? ChatEditingSessionSubmitAction.ID : ChatSubmitAction.ID,
                      title: withSlash,
                      arguments: [
                        {
                          widget,
                ...
                coalesce(
                  agents.flatMap(
                    (agent) => agent.slashCommands.map((c, i) => {
                      if (agent.isDefault && this.chatAgentService.getDefaultAgent(
                        widget.location,
                        widget.input.currentMode
                      )?.id !== agent.id) {
                        return;
...
let BuiltinDynamicCompletions = class extends Disposable {
  constructor(historyService, workspaceContextService, searchService, labelService, languageFeaturesService, chatWidgetService, _chatEditingService, instantiationService, outlineService, editorService, configurationService, fileService, markerService) {
    ...
    this.registerVariableCompletions(
      "file",
      async ({ widget, range, position, model }, token) => {
        ...
        const range2 = computeCompletionRanges(
          model,
          position,
          new RegExp(`${chatVariableLeader}[^\\s]*`, "g"),
          true
        );
        if (range2) {
    ...
    this.registerVariableCompletions(
      ...
      async ({ widget, range, position, model }, token) => {
        ...
        const afterRange = new Range(
          ...
          range.replace.startColumn,
          position.lineNumber,
          range.replace.startColumn + "#folder:".length
        );
        result.suggestions.push({
    ...
    this.registerVariableCompletions(
      "symbol",
      ({ widget, range, position, model }, token) => {
        if (!widget.supportsFileReferences) {
          return null;
        }
        const result = { suggestions: [] };
        const afterRangeSym = new Range(
          position.lineNumber,
          range.replace.startColumn,
          position.lineNumber,
          range.replace.startColumn + "#sym:".length
        );
        result.suggestions.push({
    ...
    this.registerVariableCompletions(
      SelectAndInsertProblemAction.Name,
      ({ widget, range, position, model }, token) => {
        ...
        const afterTextRange = new Range(
          position.lineNumber,
          range.replace.startColumn,
          position.lineNumber,
          range.replace.startColumn + completedText.length
        );
        result.suggestions.push({
      ...
      this.languageFeaturesService.completionProvider.register(
        ...
        },
        {
          ...
          provideCompletionItems: /* @__PURE__ */ __name(async (model, position, context, token) => {
            ...
            if (range) {
              return provider(
                { model, position, widget, range, context },
                token
              );
            }
            return;
  ...
  async addFileEntries(widget, result, info, token) {
    ...
    if (widget.input.currentMode !== ChatMode.Ask && widget.viewModel && widget.viewModel.model.editingSession) {
      const relatedFiles = await raceTimeout(
        this._chatEditingService.getRelatedFiles(
  ...
  async addFolderEntries(widget, result, info, token) {
    ...
    const makeFolderCompletionItem = /* @__PURE__ */ __name((resource, description) => {
      ...
      const uriLabel = this.labelService.getUriLabel(dirname(resource), {
        relative: true
      });
      const labelDescription = description ? localize(
    ...
    if (info.varWord?.word && info.varWord.word.startsWith(folderLeader)) {
      ...
      for (const folder of await getTopLevelFolders(
        workspaces,
        this.fileService
      )) {
  ...
  addSymbolEntries(widget, result, info, token) {
    ...
    for (const outlineModel of this.outlineService.getCachedModels()) {
      if (pattern) {
        symbolsToAdd.push(
          ...outlineModel.asListOfDocumentSymbols().map((symbol) => ({ symbol, uri: outlineModel.uri }))
        );
      } else {
        symbolsToAdd.push(
          ...outlineModel.getTopLevelSymbols().map((symbol) => ({ symbol, uri: outlineModel.uri }))
        );
      }
    }
        ...
        },
        {
          ...
          provideCompletionItems: /* @__PURE__ */ __name(async (model, position, _context, _token) => {
            ...
            const range = computeCompletionRanges(
              model,
              position,
              ToolCompletions.VariableNameDef,
              true
            );
            if (!range) {
              return null;
            ...
            const toolItems = [];
            toolItems.push(
              ...Array.from(toolsService.getTools()).filter((t) => t.canBeReferencedInPrompt).filter(
                (t) => !usedToolNames.has(
                  t.toolReferenceName ?? ""
                )
              ).map((t) => {
...
Registry.as(
  WorkbenchExtensions.Workbench
).registerWorkbenchContribution(ToolCompletions, LifecyclePhase.Eventually);
export {
