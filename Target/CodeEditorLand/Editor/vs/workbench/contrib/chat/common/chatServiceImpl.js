      ...
      try {
        ...
        if (agentPart || defaultAgent && !commandPart) {
          const prepareChatAgentRequest = /* @__PURE__ */ __name(async (agent2, command2, enableCommandDetection2, chatRequest, isParticipantDetected) => {
            ...
            request = chatRequest ?? model.addRequest(parsedRequest, initVariableData, attempt, agent2, command2, options?.confirmation, options?.locationData, options?.attachedContext, options?.workingSet);
            let variableData;
            let message;
            if (chatRequest) {
