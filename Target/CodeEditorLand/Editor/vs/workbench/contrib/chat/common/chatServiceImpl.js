...
let ChatService = class extends Disposable {
  ...
  getHistory() {
    ...
    const persistedSessionItems = persistedSessions.filter((session) => !session.isImported && session.initialLocation !== ChatAgentLocation.EditingSession).map((session) => {
    ...
    const liveSessionItems = Array.from(this._sessionModels.values()).filter((session) => !session.isImported && session.initialLocation !== ChatAgentLocation.EditingSession).map((session) => {
  ...
  async initializeSession(model, token) {
    try {
      ...
      const welcomeMessage = await defaultAgent.provideWelcomeMessage?.(token) ?? void 0;
      const sampleQuestions = await defaultAgent.provideSampleQuestions?.(model.initialLocation, token) ?? void 0;
    ...
    const sendRequestInternal = /* @__PURE__ */ __name(async () => {
      ...
      try {
        ...
        if (agentPart || defaultAgent && !commandPart) {
          ...
          chatTitlePromise = model.getRequests(false).length === 1 && !model.customTitle ? this.chatAgentService.getChatTitle(defaultAgent.id, this.getHistoryEntriesFromModel(model.getRequests(false), model.sessionId, location, agent.id), CancellationToken.None) : void 0;
        } else if (commandPart && this.chatSlashCommandService.hasCommand(commandPart.slashCommand.command)) {
          request = model.addRequest(parsedRequest, { variables: [] }, attempt);
          completeResponseCreated();
          const history = [];
          for (const request2 of model.getRequests(false)) {
