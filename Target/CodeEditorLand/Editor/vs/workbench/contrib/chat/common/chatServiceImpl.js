...
let ChatService = class extends Disposable {
  constructor(storageService, logService, extensionService, instantiationService, telemetryService, workspaceContextService, chatSlashCommandService, chatVariablesService, chatAgentService, configurationService, experimentService) {
    ...
    if (this.useFileStorage) {
      this._chatSessionStore.migrateDataIfNeeded(
        () => this._persistedSessions
      );
    }
  ...
  _onDidSubmitRequest = this._register(
    new Emitter()
  );
  ...
  saveState() {
    const liveChats = Array.from(this._sessionModels.values()).filter(
      (session) => session.initialLocation === ChatAgentLocation.Panel || session.initialLocation === ChatAgentLocation.EditingSession
    );
    if (this.useFileStorage) {
  ...
  async getHistory() {
    if (this.useFileStorage) {
      const liveSessionItems2 = Array.from(this._sessionModels.values()).filter(
        (session) => !session.isImported && (session.initialLocation !== ChatAgentLocation.EditingSession || this.unifiedViewEnabled)
      ).map((session) => {
      ...
      const entries = Object.values(index).filter(
        (entry) => !this._sessionModels.has(entry.sessionId) && !entry.isImported && !entry.isEmpty
      ).map(
        (entry) => ({
          ...entry,
    ...
    const persistedSessionItems = persistedSessions.filter(
      (session) => !session.isImported && session.initialLocation !== ChatAgentLocation.EditingSession
    ).map((session) => {
    ...
    const liveSessionItems = Array.from(this._sessionModels.values()).filter(
      (session) => !session.isImported && session.initialLocation !== ChatAgentLocation.EditingSession
    ).map((session) => {
      const title = session.title || localize("newChat", "New Chat");
      return {
  ...
  async getOrRestoreSession(sessionId) {
    ...
    if (this.useFileStorage) {
      sessionData = revive(
        await this._chatSessionStore.readSession(sessionId)
      );
    } else {
        ...
        if (agentPart || defaultAgent && !commandPart) {
          const prepareChatAgentRequest = /* @__PURE__ */ __name(async (agent2, command2, enableCommandDetection2, chatRequest, isParticipantDetected) => {
            ...
            if (chatRequest) {
              ...
              const promptTextResult = getPromptText(
                request.message
              );
              variableData = updateRanges(
                variableData,
          ...
          ) !== false && this.chatAgentService.hasChatParticipantDetectionProviders() && !agentPart && !commandPart && !agentSlashCommandPart && enableCommandDetection && options?.mode !== ChatMode.Agent && options?.mode !== ChatMode.Edit) {
  ...
  async checkAgentAllowed(agent) {
    if (agent.isToolsAgent) {
      const enabled = await this.experimentService.getTreatment(
        "chatAgentEnabled"
      );
      if (enabled === false) {
...
__decorateClass([
  memoize
], ChatService.prototype, "unifiedViewEnabled", 1);
__decorateClass([
  memoize
], ChatService.prototype, "useFileStorage", 1);
