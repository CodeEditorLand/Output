...
IMPORTANT: Mark todos completed as soon as they are done. Do not batch completions.`,source:b.Internal,inputSchema:u}}const N=x();let m=class extends h{constructor(t,e,o){super(),this.a=t,this.b=e,this.c=o}async invoke(t,e,o,i){const s=t.parameters;let r=t.context?.sessionResource;if(!r&&s.operation==="read"&&s.chatSessionResource)try{r=I.parse(s.chatSessionResource)}catch(n){this.b.error("ManageTodoListTool: Invalid chatSessionResource URI",n)}if(!r)return{content:[{kind:"text",value:"Error: No session resource available"}]};this.b.debug(`ManageTodoListTool: Invoking with options ${JSON.stringify(s)}`);try{return s.operation==="read"?this.h(r):this.j(s,r)}catch(n){return{content:[{kind:"text",value:`Error: ${n instanceof Error?n.message:"Unknown error"}`}]}}}async prepareToolInvocation(t,e){const o=t.parameters,i=t.chatSessionResource;if(!i)return;const s=this.a.getTodos(i);let r;o.operation==="read"?r=l(8453,null):o.todoList&&(r=this.f(s,o.todoList));const a=(o.todoList??s).map(c=>({id:c.id.toString(),title:c.title,status:c.status}));return{pastTenseMessage:new v(r??l(8454,null)),toolSpecificData:{kind:"todoList",todoList:a}}}f(t,e){if(t.length===0)return e.length===1?l(8455,null):l(8456,null,e.length);const o=new Map(t.map(n=>[n.id,n])),i=e.filter(n=>{const a=o.get(n.id);return a&&a.status!=="in-progress"&&n.status==="in-progress"});if(i.length>0){const n=i[0],a=e.length,c=e.findIndex(d=>d.id===n.id)+1;return l(8457,null,n.title,c,a)}const s=e.filter(n=>{const a=o.get(n.id);return a&&a.status!=="completed"&&n.status==="completed"});if(s.length>0){const n=s[0],a=e.length,c=e.findIndex(d=>d.id===n.id)+1;return l(8458,null,n.title,c,a)}const r=e.filter(n=>!o.has(n.id));return r.length>0?r.length===1?l(8459,null):l(8460,null,r.length):l(8461,null)}g(t,e){return t.length===0?"No todo list found.":`# Todo List
...
`)}).join(`
`)}r(t,e){let o=0;const i=Math.min(t.length,e.length);for(let a=0;a<i;a++){const c=t[a],d=e[a];(c.title!==d.title||c.status!==d.status)&&o++}const s=Math.max(0,e.length-t.length),r=Math.max(0,t.length-e.length);return s+r+o}};m=g([p(0,T),p(1,y),p(2,C)],m);export{M as $j5b,x as $k5b,N as $l5b,m as $m5b};
