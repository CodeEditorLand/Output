    ...
    languageModelToolsExtensionPoint.setHandler((extensions, delta) => {
      for (const extension of delta.added) {
        for (const rawTool of extension.value) {
          ...
          if ((rawTool.name.startsWith("copilot_") || rawTool.name.startsWith("vscode_")) && !isProposedApiEnabled(
          ...
          if (rawTool.tags?.includes(CopilotAgentModeTag)) {
            if (!isProposedApiEnabled(
              ...
              logService.error(
                `Extension '${extension.description.identifier.value}' CANNOT register tool with tag "${CopilotAgentModeTag}" without enabling 'languageModelToolsForAgent' proposal`
              );
              continue;
            }
          }
          if (rawTool.tags?.some(
            (tag) => tag !== CopilotAgentModeTag && (tag.startsWith("copilot_") || tag.startsWith("vscode_"))
          ) && !isProposedApiEnabled(
            extension.description,
            "chatParticipantPrivate"
          )) {
      ...
      for (const extension of delta.removed) {
        for (const tool of extension.value) {
          this._registrationDisposables.deleteAndDispose(
            toToolKey(extension.description.identifier, tool.name)
          );
        }
      }
    });
