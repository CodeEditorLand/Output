...
const CONTEXT_GALLERY_SORT_CAPABILITIES = new RawContextKey(
  "gallerySortCapabilities",
  ""
);
const CONTEXT_GALLERY_FILTER_CAPABILITIES = new RawContextKey(
  "galleryFilterCapabilities",
  ""
);
const CONTEXT_GALLERY_ALL_REPOSITORY_SIGNED = new RawContextKey(
  "galleryAllRepositorySigned",
  false
);
const CONTEXT_GALLERY_HAS_EXTENSION_LINK = new RawContextKey(
...
let ExtensionsContributions = class extends Disposable {
    ...
    extensionGalleryManifestService.getExtensionGalleryManifest().then((extensionGalleryManifest) => {
      ...
      this._register(
        extensionGalleryManifestService.onDidChangeExtensionGalleryManifest(
          (extensionGalleryManifest2) => this.registerGalleryCapabilitiesContexts(
            extensionGalleryManifest2
          )
        )
      );
    });
  ...
  async registerGalleryCapabilitiesContexts(extensionGalleryManifest) {
    ...
    CONTEXT_GALLERY_ALL_REPOSITORY_SIGNED.bindTo(
      this.contextKeyService
    ).set(
      !!extensionGalleryManifest?.capabilities?.signing?.allRepositorySigned
    );
    CONTEXT_GALLERY_HAS_EXTENSION_LINK.bindTo(this.contextKeyService).set(
      !!(extensionGalleryManifest && getExtensionGalleryManifestResourceUri(
        extensionGalleryManifest,
    ...
    this.registerExtensionAction({
      ...
      menu: {
        ...
        when: ContextKeyExpr.and(
          ...
          CONTEXT_GALLERY_ALL_REPOSITORY_SIGNED
        ),
        order: 1
      },
      run: /* @__PURE__ */ __name(async (accessor, extensionId) => {
...
${publisher}${link ? "\n" + link : ""}`;
    ...
    this.registerExtensionAction({
      ...
      run: /* @__PURE__ */ __name((accessor, id) => accessor.get(ICommandService).executeCommand(
      ...
      for (const profile of userDataProfilesService.profiles) {
        extensionManagementService.getInstalled(
          ...
          for (const extension of extensions) {
            ...
            if (productService.trustedExtensionPublishers?.includes(
              publisher
            ) || extension.publisherDisplayName && productService.trustedExtensionPublishers?.includes(
