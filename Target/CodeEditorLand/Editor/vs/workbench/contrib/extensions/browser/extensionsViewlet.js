...
const ExtensionsSearchValueContext = new RawContextKey(
  "extensionsSearchValue",
  ""
);
...
let ExtensionsViewPaneContainer = class extends ViewPaneContainer {
  ...
  extensionsSearchValueContextKey;
  ...
  async checkForMaliciousExtensions() {
    try {
      ...
      if (maliciousExtensions.length) {
        await this.extensionsManagementService.uninstallExtensions(
        ...
        this.notificationService.prompt(
          ...
          localize(
            "malicious warning",
            "The following extensions were found to be problematic and have been uninstalled: {0}",
            maliciousExtensions.map((e) => e.identifier.id).join(", ")
          ),
          shouldRestartExtensions || shouldReloadWindow ? [
            {
              label: shouldRestartExtensions ? localize(
                "restartNow",
                "Restart Extensions"
