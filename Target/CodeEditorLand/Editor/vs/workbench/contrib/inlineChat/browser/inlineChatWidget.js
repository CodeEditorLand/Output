      ...
      void 0,
      {
        ...
        filter: /* @__PURE__ */ __name((item) => {
          if (isResponseVM(item) && item.isComplete && !item.errorDetails) {
            if (item.response.value.length > 0 && item.response.value.every((item2) => item2.kind === "textEditGroup" && _options.chatWidgetViewOptions?.rendererOptions?.renderTextEditsAsSummary?.(item2.uri))) {
              return false;
            }
