...
} from "../common/mcpConfiguration.js";
...
let McpAddConfigurationCommand = class {
  ...
  async getServerType() {
    ...
    try {
      aiSupported = await this._commandService.executeCommand(
        "github.copilot.chat.mcp.setup.check" /* IsSupported */
      );
    } catch {
    }
    if (aiSupported) {
      ...
      items.push(
        {
          type: "separator",
          label: localize("mcp.serverType.copilot", "Model-Assisted")
        },
        ...Object.entries(assistedTypes).map(
          ([type, { pickLabel, pickDescription }]) => ({
            kind: Number(type),
            label: pickLabel,
    ...
    if (!!this._environmentService.remoteAuthority) {
      options.push({
        ...
        description: localize(
          ...
          "Available on this remote machine"
        )
      });
    }
  ...
  async getAssistedConfig(type) {
    ...
    const loadingQuickPick = loadingQuickPickStore.add(
    ...
    const loadingAction = await new Promise(
      (resolve) => {
        loadingQuickPick.onDidAccept(
          () => resolve(loadingQuickPick.selectedItems[0]?.id)
        );
    ...
    const configWithName = await this._commandService.executeCommand("github.copilot.chat.mcp.setup.flow" /* StartFlow */, {
      name: packageName,
      type: packageType
    });
    if (!configWithName) {
  ...
  showOnceDiscovered(name) {
    ...
    store.add(
      autorun((reader) => {
        ...
        const match = mapFindFirst(
          colls,
          (collection) => mapFindFirst(
            collection.serverDefinitions.read(reader),
            (server) => server.label === name ? { server, collection } : void 0
          )
        );
        if (match) {
  ...
  async pickForUrlHandler(resource, showIsPrimary = false) {
    const name = decodeURIComponent(basename(resource)).replace(
      /\.json$/,
      ""
    );
    ...
    const items = [
      {
        ...
        description: localize(
          "install.description",
          "Install in your user settings"
        )
      },
      {
        id: "show",
        label: localize("install.show", "Show Configuration", name)
      },
      {
        id: "rename",
        label: localize("install.rename", 'Rename "{0}"', name)
      },
    ...
    const getEditors = /* @__PURE__ */ __name(() => this._editorService.getEditors(EditorsOrder.MOST_RECENTLY_ACTIVE).filter(
      (e) => e.editor.resource?.toString() === resource.toString()
    ), "getEditors");
    switch (pick?.id) {
      case "show":
      ...
      case "install":
        ...
        try {
          ...
          const {
            inputs,
            ...config
          } = parseJsonc(contents.value.toString());
          await this.writeToUserSetting(
            name,
            config,
