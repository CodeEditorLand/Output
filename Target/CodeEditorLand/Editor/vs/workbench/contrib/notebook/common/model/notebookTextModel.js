...
class NotebookOperationManager {
  ...
  appendPreviousOperation() {
    const previous = this._undoService.getLastElement(
      this._textModel.uri
    );
...
let NotebookTextModel = class extends Disposable {
  ...
  reset(cells, metadata, transientOptions) {
    ...
    const executions = this._notebookExecutionStateService.getCellExecutionsForNotebook(
      this.uri
    );
    const executingCellHandles = executions.filter((exe) => exe.state === NotebookCellExecutionState.Executing).map((exe) => exe.cellHandle);
    const edits = NotebookTextModel.computeEdits(
      this,
      cells,
      executingCellHandles
    );
    this.applyEdits(
  ...
  createSnapshot(options) {
    ...
    const data = {
      metadata: filter(
        this.metadata,
        (key) => !transientOptions.transientDocumentMetadata[key]
      ),
      cells: []
    };
    let outputSize = 0;
    for (const cell of this.cells) {
      ...
      cellData.metadata = filter(
        cell.metadata,
        (key) => !transientOptions.transientCellMetadata[key]
      );
  ...
  restoreSnapshot(snapshot, transientOptions) {
    this.reset(
      snapshot.cells,
      snapshot.metadata,
      transientOptions ?? this.transientOptions
    );
  }
  ...
  static _commonSuffix(a, aLen, aDelta, b, bLen, bDelta, isExecuting) {
    const maxResult = Math.min(aLen, bLen);
    let result = 0;
    for (let i = 0; i < maxResult && a[aDelta + aLen - i - 1].fastEqual(
      b[bDelta + bLen - i - 1],
  ...
  newCellsFromLastEdit = /* @__PURE__ */ new Set();
  ...
  findNextMatch(searchString, searchStart, isRegex, matchCase, wordSeparators, searchEnd) {
    ...
    while (cellIndex < searchEndCell) {
      ...
      const searchRange = new Range(
        ...
        wrapFlag ? searchEnd.position.column : cell.textBuffer.getLineMaxColumn(
