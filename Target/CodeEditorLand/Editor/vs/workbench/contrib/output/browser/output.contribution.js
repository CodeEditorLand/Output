      ...
      registerAction2(
        class extends Action2 {
          ...
          async run(accessor) {
            ...
            for (const log of extensionLogs.sort(
              (a, b) => a.label.localeCompare(b.label)
            )) {
              entries.push(log);
            }
            ...
            if (result?.length) {
              outputService.showChannel(
                outputService.registerCompoundLogChannel(
                  result
                )
              );
            }
        ...
        registerAction2(
          class extends Action2 {
            ...
            async run(accessor) {
              ...
              const channel = outputService.getActiveChannel();
              if (channel) {
                const channelDescriptor = outputService.getChannelDescriptor(
                  channel.id
                );
                if (channelDescriptor) {
                  outputService.setLogLevel(
                    channelDescriptor,
                    logLevel
                  );
                }
      ...
      registerAction2(
        class extends Action2 {
          ...
          async run(accessor) {
            ...
            const defaultLogLevelsService = accessor.get(
              IDefaultLogLevelsService
            );
            const channel = outputService.getActiveChannel();
            if (channel) {
              const channelDescriptor = outputService.getChannelDescriptor(channel.id);
              if (channelDescriptor && isSingleSourceOutputChannelDescriptor(
                channelDescriptor
              )) {
                const logLevel = loggerService.getLogLevel(
                  channelDescriptor.source.resource
                );
                return await defaultLogLevelsService.setDefaultLogLevel(
                  logLevel,
        ...
        registerAction2(
          class extends ViewAction {
            ...
            async runInView(serviceAccessor, view) {
              this.toggleLogLevelFilter(
                serviceAccessor.get(IOutputService),
                logLevel
              );
            }
        ...
        class extends Action2 {
          ...
          async run(accessor) {
            ...
            if (userLogs.length && (extensionLogs.length || logs.length)) {
              entries.push({
                type: "separator",
                label: nls.localize("userLogs", "User Logs")
              });
            }
            for (const log of userLogs.sort(
              (a, b) => a.label.localeCompare(b.label)
            )) {
              entries.push(log);
            }
          ...
          async run(accessor) {
            ...
            if (result?.length) {
              ...
              Registry.as(
                ...
                log: true,
                user: true,
                source: result.length === 1 ? { resource: result[0] } : result.map((resource) => ({
                  resource,
                  name: basename(resource).split(
                    "."
                  )[0]
                }))
              });
