...
class SCMAccessibilityHelp {
  ...
  when = ContextKeyExpr.or(
    ContextKeyExpr.and(
      ContextKeyExpr.equals("activeViewlet", "workbench.view.scm"),
      SidebarFocusContext
    ),
  ...
  getProvider(accessor) {
    ...
    return new SCMAccessibilityHelpContentProvider(
      commandService,
      scmViewService,
      viewsService
    );
    ...
    if (focusedRepository) {
      content.push(
        localize(
          "state-msg2",
          "Repository: {0}",
          focusedRepository.provider.name
        )
      );
