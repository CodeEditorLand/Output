  ...
  updateStyles() {
    ...
    for (const instance of this._terminalService.instances) {
      ...
      if (uri instanceof URI && iconClasses && iconClasses.length > 1) {
        css += `.monaco-workbench .terminal-tab.${iconClasses[0]}::before{content: ''; background-image: ${cssJs.asCSSUrl(uri)};}`;
      }
      if (ThemeIcon.isThemeIcon(icon)) {
        ...
        if (iconContribution) {
          ...
          if (def) {
            css += `.monaco-workbench .terminal-tab.codicon-${icon.id}::before{content: '${def.fontCharacter}' !important; font-family: ${cssJs.asCSSPropertyValue(def.font?.id ?? "codicon")} !important;}`;
          }
        }
      }
    }
