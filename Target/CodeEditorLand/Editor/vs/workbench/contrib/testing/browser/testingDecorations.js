...
let TestingDecorations = class extends Disposable {
  constructor(editor, codeEditorService, testService, decorations, uriIdentityService, results, configurationService, instantiationService) {
    ...
    this._register(
      Event.any(
        ...
        Event.filter(
          this.results.onTestChanged,
          (c) => c.reason === TestResultItemChangeReason.NewMessage
        ),
    ...
    this._register(
      Event.accumulate(
        ...
        for (const [message, deco] of this.loggedMessageDecorations) {
          ...
          if (invalidate) {
            changed = true;
            TestingDecorations.invalidatedTests.add(
              deco.resultItem || message
            );
          }
        }
        if (changed) {
  ...
  _currentUri;
  expectedWidget = this._register(
    new MutableDisposable()
  );
  actualWidget = this._register(
    new MutableDisposable()
  );
  errorContentWidgets = this._register(
    new DisposableMap()
  );
    ...
    for (const test of lastResult.tests) {
      ...
      for (let taskId = 0; taskId < test.tasks.length; taskId++) {
        ...
        for (let i = 0; i < state.messages.length; i++) {
          ...
          const line = m.location?.uri.toString() === uriStr ? m.location.range.startLineNumber : m.stackTrace && mapFindFirst(
            m.stackTrace,
            (f) => f.position && f.uri?.toString() === uriStr ? f.position.lineNumber : void 0
          );
  ...
  applyResultsLoggedMessages(uriStr, messageLines) {
    this.editor.changeDecorations((accessor) => {
      ...
      if (getTestingConfiguration(
        ...
        this.results.results.forEach(
          (r) => this.applyLoggedMessageFromResult(
            r,
            uriStr,
            seen,
            messageLines,
        ...
        this.applyLoggedMessageFromResult(
          this.results.results[0],
          uriStr,
          seen,
          messageLines,
...
let MultiRunTestDecoration = class extends RunTestDecoration {
  ...
  getContextMenuActions() {
    ...
    disposable.add(
      toDisposable(() => allActions.forEach((a) => a.dispose()))
    );
