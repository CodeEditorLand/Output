        ...
        steps: category.content.steps.map((step, index2) => {
          return {
            ...
            media: step.media.type === "image" ? {
              ...
              type: "video",
              path: convertRelativeMediaPathsToWebviewURIs(
              ...
              poster: step.media.poster ? convertRelativeMediaPathsToWebviewURIs(
                FileAccess.asFileUri(
                  "vs/workbench/contrib/welcomeGettingStarted/common/media/"
                ),
                step.media.poster
      ...
      this.extensionManagementService.onDidInstallExtensions((result) => {
        if (result.some(
          (e) => ExtensionIdentifier.equals(
            this.productService.defaultChatAgent?.extensionId,
            e.identifier.id
          ...
          const steps = (walkthrough.steps ?? []).map(
            (step, index2) => {
              ...
              if (step.media.image) {
                ...
                const baseURI = FileAccess.uriToFileUri(
                  extension.extensionLocation
                );
                media = {
                  type: "video",
