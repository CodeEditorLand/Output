...
let ConfigurationEditing = class {
  ...
  async updateConfiguration(operation, model, formattingOptions, options) {
    ...
    if (edit) {
      let disposable;
      try {
        disposable = this.filesConfigurationService.enableAutoSaveAfterShortDelay(
          model.uri
        );
  ...
  onInvalidConfigurationError(error, operation) {
    ...
    ) : operation.workspaceStandAloneConfigurationKey === MCP_CONFIGURATION_KEY ? nls.localize(
