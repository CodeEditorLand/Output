...
let WorkbenchExtensionGalleryManifestService = class extends ExtensionGalleryManifestService {
  constructor(productService, environmentService, fileService, telemetryService, storageService, remoteAgentService, sharedProcessService, configurationService, requestService, defaultAccountService, dialogService, hostService, logService) {
    ...
    const channels = [
      sharedProcessService.getChannel("extensionGalleryManifest")
    ];
    const remoteConnection = remoteAgentService.getConnection();
    if (remoteConnection) {
      channels.push(
        remoteConnection.getChannel("extensionGalleryManifest")
      );
    }
    this.getExtensionGalleryManifest().then((manifest) => {
      channels.forEach(
        (channel) => channel.call("setExtensionGalleryManifest", [manifest])
      );
      this._register(
        this.onDidChangeExtensionGalleryManifest(
          (manifest2) => channels.forEach(
            (channel) => channel.call("setExtensionGalleryManifest", [manifest2])
          )
        )
      );
    });
  ...
  async doGetExtensionGalleryManifest() {
    ...
    const configuredServiceUrl = this.configurationService.getValue(
      ExtensionGalleryServiceUrlConfigKey
    );
    if (configuredServiceUrl && this.checkAccess(
      await this.defaultAccountService.getDefaultAccount()
    )) {
      this.extensionGalleryManifest = [
        configuredServiceUrl,
        await this.getExtensionGalleryManifestFromServiceUrl(
          configuredServiceUrl
        )
      ];
    }
    ...
    this._register(
      this.configurationService.onDidChangeConfiguration((e) => {
        ...
        const configuredServiceUrl2 = this.configurationService.getValue(
          ExtensionGalleryServiceUrlConfigKey
        );
  ...
  checkAccess(account) {
    ...
    if (account.access_type_sku && this.productService.extensionsGallery?.accessSKUs?.includes(
      account.access_type_sku
    )) {
      this.logService.debug(
        "[Marketplace] Account has access to configured gallery"
      );
      return true;
    }
    this.logService.debug(
  ...
  async requestRestart() {
    const confirmation = await this.dialogService.confirm({
      ...
      primaryButton: localize(
        { key: "restart", comment: ["&& denotes a mnemonic"] },
        "&&Restart"
      )
    });
  ...
  async getExtensionGalleryManifestFromServiceUrl(url) {
    ...
    try {
      ...
      if (!extensionGalleryManifest) {
        throw new Error(
          "Unable to retrieve extension gallery manifest."
        );
      }
      return extensionGalleryManifest;
    } catch (error) {
      this.logService.error(
        "[Marketplace] Error retrieving extension gallery manifest",
        error
      );
      throw error;
    }
