...
import {
  TokenQuality,
  TokenUpdate
} from "../../../../editor/common/model/tokenStore.js";
...
let TreeSitterTokenizationFeature = class extends Disposable {
  ...
  _handleGrammarsExtPoint() {
    ...
    ].filter((languageId) => !this._getSetting(languageId));
...
let TreeSitterTokenizationSupport = class extends Disposable {
  constructor(_queries, Query, _languageId, _languageIdCodec, _treeSitterService, _themeService, _tokenizationStoreService, _instantiationService) {
    ...
    this._instantiationService = _instantiationService;
    this._codeEditors = this._instantiationService.createInstance(
      TreeSitterCodeEditors,
      this._languageId
    );
    this._register(
      this._codeEditors.onDidChangeViewport((e) => {
  ...
  get _encodedLanguageId() {
    if (!this._encodedLanguage) {
      this._encodedLanguage = this._languageIdCodec.encodeLanguageId(
        this._languageId
      );
    }
  ...
  _setInitialTokens(textModel) {
    ...
    this._tokenizationStoreService.setTokens(
      textModel,
      tokens,
      TokenQuality.None
    );
  ...
  _forceParseAndTokenizeContent(model, range, startOffsetOfRangeInDocument, endOffsetOfRangeInDocument, content, asUpdate) {
    ...
    const tree = this._treeSitterService.getTreeSync(
  ...
  async _parseAndTokenizeViewPort(model, viewportRanges) {
    ...
    for (const range of viewportRanges) {
      const startOffsetOfRangeInDocument = model.getOffsetAt(
        range.getStartPosition()
      );
      const endOffsetOfRangeInDocument = model.getOffsetAt(
        range.getEndPosition()
      );
      const version = model.getVersionId();
      if (this._tokenizationStoreService.rangeHasTokens(
        model,
        range,
      ...
      const tokenUpdates = await this._forceParseAndTokenizeContent(
        model,
        range,
        startOffsetOfRangeInDocument,
      ...
      if (!tokenUpdates || this._tokenizationStoreService.rangeHasTokens(
        model,
        range,
        TokenQuality.ViewportGuess
      )) {
  ...
  _setViewPortTokens(textModel, versionId, tree) {
    ...
    if (editor) {
      ...
      for (let i = 0; i < viewPort.length; i++) {
        ...
        const newRangeStartOffset = textModel.getOffsetAt(
          range.getStartPosition()
        );
        const newRangeEndOffset = textModel.getOffsetAt(
          range.getEndPosition()
        );
        rangeChanges[i] = {
          newRange: range,
      ...
      rangeChanges = [
        {
          newRange: new Range(
            ...
            1,
            maxLine,
            textModel.getLineMaxColumn(maxLine)
          ),
  ...
  _handleTreeUpdate(ranges, textModel, versionId, textModelTreeSitter) {
    ...
    for (let i = 0; i < ranges.length; i++) {
      ...
      if (rangeLinesLength > chunkSize) {
        ...
        do {
          ...
          const chunkEndPosition = new Position(
            chunkLineEnd,
            chunkEndColumn
          );
          ...
          rangeChanges.push({
            range: chunkRange,
            startOffset: textModel.getOffsetAt(
              chunkRange.getStartPosition()
            ),
            endOffset: textModel.getOffsetAt(
        ...
        if (i === 0 || rangeChanges[i - 1].endOffset < ranges[i].newRangeStartOffset) {
          ...
          const startPosition = textModel.getPositionAt(
    ...
    const captures = rangeChanges.map(
      (range) => this._getCaptures(range.range, textModelTreeSitter, tree)
    );
    return this._updateTreeForRanges(
      textModel,
      rangeChanges,
      versionId,
      tree,
  ...
  captureAtPosition(lineNumber, column, textModel) {
    ...
    const captures = this._captureAtRangeWithInjections(
      new Range(lineNumber, column, lineNumber, column + 1),
      textModelTreeSitter,
      textModelTreeSitter.parseResult.tree
    );
    return captures;
  }
  captureAtRangeTree(range, tree, textModelTreeSitter) {
    const captures = textModelTreeSitter ? this._captureAtRangeWithInjections(
      range,
      textModelTreeSitter,
      tree
  ...
  _captureAtRangeWithInjections(range, textModelTreeSitter, tree) {
    ...
    for (let i = 0; i < captures.length; i++) {
      ...
      const injection = this._getInjectionCaptures(
        textModelTreeSitter,
        capture,
        injectionRange
      );
  ...
  tokenizeEncoded(lineNumber, textModel) {
    ...
    const updates = this._rangeTokensAsUpdates(
      textModel.getOffsetAt({ lineNumber, column: 1 }),
      tokens.result
    );
    if (tokens.versionId === textModel.getVersionId()) {
      this._tokenizationStoreService.updateTokens(
        ...
        [
          {
            newTokens: updates,
            oldRangeLength: textModel.getLineLength(lineNumber)
          }
        ],
        TokenQuality.Accurate
      );
  ...
  _createTokensFromCaptures(tree, captures, rangeStartOffset, rangeEndOffset) {
    ...
    const endOffsetsAndScopes = Array(
      captures.length
    );
    ...
    const brackets = /* @__PURE__ */ __name((capture, startOffset) => {
      return capture.name.includes("punctuation") && capture.text ? Array.from(capture.text.matchAll(BRACKETS)).map(
  ...
  _getInjectionCaptures(textModelTreeSitter, parentCapture, range) {
    const injection = textModelTreeSitter.getInjection(
      parentCapture.node.startIndex,
      this._languageId
    );
    if (!injection?.tree || injection.versionId !== textModelTreeSitter.parseResult?.versionId) {
      return void 0;
    }
    const feature = TreeSitterTokenizationRegistry.get(
      injection.languageId
    );
    if (!feature) {
      return void 0;
    }
    return feature.captureAtRangeTree(
      range,
      injection.tree,
      textModelTreeSitter
    );
  ...
  _tokenizeEncoded(lineNumber, textModel) {
    const lineOffset = textModel.getOffsetAt({
      lineNumber,
      column: 1
    });
    const maxLine = textModel.getLineCount();
    const lineEndOffset = lineNumber + 1 <= maxLine ? textModel.getOffsetAt({
