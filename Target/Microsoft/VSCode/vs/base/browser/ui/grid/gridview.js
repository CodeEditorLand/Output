...
import { LayoutPriority, Sizing as Sizing2 } from "../splitview/splitview.js";
const defaultStyles = {
...
class BranchNode {
  ...
  get priority() {
    ...
    const priorities = this.children.map((c) => typeof c.priority === "undefined" ? 0 : c.priority);
    if (priorities.some(
      (p) => p === 2
      /* LayoutPriority.High */
    )) {
      return 2;
    } else if (priorities.some(
      (p) => p === 1
      /* LayoutPriority.Low */
    )) {
      return 1;
...
function createLatchedOnDidChangeViewEvent(view) {
  ...
  return Event.any(onDidSetViewSize, Event.map(Event.latch(Event.map(onDidChangeViewConstraints, (_) => [view.minimumWidth, view.maximumWidth, view.minimumHeight, view.maximumHeight]), arrayEquals), (_) => void 0));
}
...
class LeafNode {
  ...
  constructor(view, orientation, layoutController, orthogonalSize, size = 0) {
    ...
    this._onDidViewChange = Event.map(onDidChange, (e) => e && (this.orientation === 0 ? e.width : e.height), this.disposables);
...
class GridView {
  ...
  constructor(options = {}) {
    ...
    this.root = new BranchNode(0, this.layoutController, this.styles, this.proportionalLayout);
  }
  style(styles) {
    this.styles = styles;
