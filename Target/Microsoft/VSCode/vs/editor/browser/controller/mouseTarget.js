...
class MouseTarget {
  ...
  static toString(target) {
    return `${MouseTarget._typeToString(target.type)}: ${target.position} - ${target.range} - ${JSON.stringify(target.detail)}`;
  }
}
class ElementPath {
  static {
...
class MouseTargetFactory {
  ...
  static _hitTestViewLines(ctx, request) {
    ...
    if (ElementPath.isStrictChildOfViewLines(request.targetPath)) {
      ...
      if (ctx.viewLinesGpu) {
        ...
        const position = ctx.viewLinesGpu.getPositionAtCoordinate(
          lineNumber,
        ...
        if (position) {
          ...
          return request.fulfillContentText(
            position,
            EditorRange.fromPositions(position, position),
            detail
          );
        }
      }
  ...
  static _doHitTestWithCaretRangeFromPoint(ctx, request) {
    ...
    if (!isBelowLastLine) {
      ...
      const r = MouseTargetFactory._actualDoHitTestWithCaretRangeFromPoint(
        ctx,
...
function shadowCaretRangeFromPoint(shadowRoot, x, y) {
  ...
  if (el !== null) {
    while (el?.firstChild && el.firstChild.nodeType !== el.firstChild.TEXT_NODE && el.lastChild && el.lastChild.firstChild) {
      el = el.lastChild;
    }
