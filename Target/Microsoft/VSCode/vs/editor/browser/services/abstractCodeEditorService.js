...
let AbstractCodeEditorService = class extends Disposable {
  ...
  _transientWatchers = this._register(
    new DisposableMap()
  );
...
class RefCountedStyleSheet {
  ...
  removeRulesContainingSelector(ruleName) {
    domStylesheets.removeCSSRulesContainingSelector(
      ruleName,
      this._styleSheet
    );
  }
...
class DecorationTypeOptionsProvider {
  ...
  constructor(description, themeService, styleSheet, providerArgs) {
    ...
    const lightOverviewRulerColor = options.light?.overviewRulerColor || options.overviewRulerColor;
    const darkOverviewRulerColor = options.dark?.overviewRulerColor || options.overviewRulerColor;
      ...
      if (typeof opts.contentIconPath !== "undefined") {
        cssTextArr.push(
          strings.format(
            ...
            cssJs.asCSSUrl(URI.revive(opts.contentIconPath))
          )
        );
      }
    ...
    if (typeof opts.gutterIconPath !== "undefined") {
      cssTextArr.push(
        strings.format(
          ...
          cssJs.asCSSUrl(URI.revive(opts.gutterIconPath))
        )
      );
...
class CSSNameHelper {
  ...
  static getSelector(key, parentKey, ruleType) {
    let selector = `.monaco-editor .${CSSNameHelper.getClassName(key, ruleType)}`;
    if (parentKey) {
      selector = `${selector}.${CSSNameHelper.getClassName(parentKey, ruleType)}`;
    }
