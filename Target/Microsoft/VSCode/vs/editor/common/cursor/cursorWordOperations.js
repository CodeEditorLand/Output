...
class WordOperations {
  ...
  static _doFindPreviousWordOnLine(lineContent, wordSeparators, position) {
    ...
    for (let chIndex = position.column - 2; chIndex >= 0; chIndex--) {
      ...
      if (previousIntlWord && chIndex === previousIntlWord.index) {
        return WordOperations._createIntlWord(
          previousIntlWord,
          chClass
        );
      }
  ...
  static _doFindNextWordOnLine(lineContent, wordSeparators, position) {
    ...
    for (let chIndex = position.column - 1; chIndex < len; chIndex++) {
      ...
      if (nextIntlWord && chIndex === nextIntlWord.index) {
        return WordOperations._createIntlWord(nextIntlWord, chClass);
      }
      if (chClass === WordCharacterClass.Regular) {
        if (wordType === 2 /* Separator */) {
          return WordOperations._createWord(
  ...
  static _deleteInsideWordWhitespace(model, position) {
    ...
    while (leftIndex > 0 && WordOperations._charAtIsWhitespace(lineContent, leftIndex - 1)) {
      leftIndex--;
    }
    while (rightIndex + 1 < lineContentLength && WordOperations._charAtIsWhitespace(lineContent, rightIndex + 1)) {
      rightIndex++;
    }
    return new Range(
      position.lineNumber,
  ...
  static _deleteInsideWordDetermineDeleteRange(wordSeparators, model, position) {
    ...
    const deleteWordAndAdjacentWhitespace = /* @__PURE__ */ __name((word) => {
      ...
      while (endColumn - 1 < lineLength && WordOperations._charAtIsWhitespace(lineContent, endColumn - 1)) {
        expandedToTheRight = true;
        endColumn++;
      }
      if (!expandedToTheRight) {
        while (startColumn > 1 && WordOperations._charAtIsWhitespace(
          lineContent,
          startColumn - 2
        )) {
          startColumn--;
