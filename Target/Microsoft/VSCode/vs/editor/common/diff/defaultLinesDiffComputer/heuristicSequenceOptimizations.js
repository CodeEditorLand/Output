  ...
  do {
    ...
    for (let i = 1; i < diffs.length; i++) {
      let shouldJoinDiffs2 = function(before, after) {
        ...
        __name(cap, "cap");
        if ((cap(beforeLineCount1 * 40 + beforeSeq1Length) ** 1.5 + cap(beforeLineCount2 * 40 + beforeSeq2Length) ** 1.5) ** 1.5 + (cap(afterLineCount1 * 40 + afterSeq1Length) ** 1.5 + cap(afterLineCount2 * 40 + afterSeq2Length) ** 1.5) ** 1.5 > (max ** 1.5) ** 1.5 * 1.3) {
          return true;
        }
        return false;
      };
