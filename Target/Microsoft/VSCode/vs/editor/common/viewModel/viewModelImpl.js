...
class ViewModel extends Disposable {
        ...
        this.viewLayout.changeSpecialLineHeights((accessor) => {
          for (const change of filteredChanges) {
            ...
            if (lineHeightMultiplier !== null) {
              accessor.insertOrChangeCustomLineHeight(decorationId, viewRange.startLineNumber, viewRange.endLineNumber, lineHeightMultiplier * this._configuration.options.get(
                75
  ...
  */
  ...
  getViewRangeWithCursorPadding(viewRange) {
    const options = this._configuration.options;
    const cursorSurroundingLines = options.get(
      35
      /* EditorOption.cursorSurroundingLines */
    );
    const stickyScroll = options.get(
      131
