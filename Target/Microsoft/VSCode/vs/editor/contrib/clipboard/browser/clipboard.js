  ...
  target.addImplementation(
    ...
    (accessor, args) => {
      ...
      if (focusedEditor?.hasTextFocus()) {
        ...
        if (selection?.isEmpty() && !emptySelectionClipboard) {
          return true;
        }
...
if (PasteAction) {
  PasteAction.addImplementation(
    ...
    (accessor, args) => {
      ...
      if (focusedEditor?.hasModel() && focusedEditor.hasTextFocus()) {
        const experimentalEditContextEnabled = focusedEditor.getOption(
          EditorOption.effectiveExperimentalEditContextEnabled
        );
        if (experimentalEditContextEnabled) {
          const nativeEditContext = NativeEditContextRegistry.get(
            focusedEditor.getId()
          );
