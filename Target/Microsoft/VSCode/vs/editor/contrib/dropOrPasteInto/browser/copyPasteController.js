...
let CopyPasteController = class extends Disposable {
  ...
  handleCopy(e) {
    ...
    const operations = providers.map((provider) => {
      return {
        ...
        operation: createCancelablePromise(
          (token) => provider.prepareDocumentPaste?.(
            model,
            ranges,
            dataTransfer,
            token
    ...
    CopyPasteController._currentCopyOperation?.operations.forEach(
      (entry) => entry.operation.cancel()
    );
  ...
  async handlePaste(e) {
    ...
    const allProviders = this._languageFeaturesService.documentPasteEditProvider.ordered(model).filter((provider) => {
      ...
      if (preference) {
        if (!this.providerMatchesPreference(
          provider,
          preference
        )) {
          return false;
          ...
          editSession = {
            edits: editSession.edits.filter((edit) => {
              if ("only" in preference) {
                ...
                return preference.preferences.some(
                  (preference2) => preference2.contains(edit.kind)
                );
              } else {
  ...
  async mergeInDataFromCopy(allProviders, dataTransfer, metadata, token) {
    if (metadata?.id && CopyPasteController._currentCopyOperation?.handle === metadata.id) {
      const toResolve = CopyPasteController._currentCopyOperation.operations.filter(
        (op) => allProviders.some(
          (provider) => provider.pasteMimeTypes.some(
            (type) => matchesMimeType(type, op.providerMimeTypes)
          )
        )
      ).map((op) => op.operation);
  ...
  providerMatchesPreference(provider, preference) {
    if ("only" in preference) {
      return provider.providedPasteEditKinds.some(
        (providedKind) => preference.only.contains(providedKind)
      );
    } else if ("preferences" in preference) {
      return preference.preferences.some(
        (providedKind) => preference.preferences.some(
          (preferredKind) => preferredKind.contains(providedKind)
        )
      );
    } else {
  ...
  getInitialActiveEditIndex(model, edits) {
    ...
    for (const config of Array.isArray(preferredProviders) ? preferredProviders : []) {
      ...
      const editIndex = edits.findIndex(
        (edit) => desiredKind.contains(edit.kind)
      );
      if (editIndex >= 0) {
        return editIndex;
      }
