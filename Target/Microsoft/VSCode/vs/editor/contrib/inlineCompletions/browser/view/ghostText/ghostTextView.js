...
let GhostTextView = class GhostTextView2 extends Disposable {
  static {
  ...
  static {
    this.hot = createHotClass(GhostTextView_1);
  }
  constructor(_editor, _model, _options, _shouldKeepCursorStable, _isClickable, _languageService) {
    ...
    this.uiState = derived(this, (reader) => {
      ...
      const renderData = {
        cursorColumnDistance: cursorColumn !== void 0 ? Math.abs((inlineTextsWithTokens.length > 0 ? inlineTextsWithTokens[0].column : 1) - cursorColumn) : -1,
        ...
        sameShapeReplacements: inlineTextsWithTokens.length > 1 && inlineTextsWithTokens.length === 0 ? inlineTextsWithTokens.every((inline) => inline.text.length === inlineTextsWithTokens[0].text.length) : void 0
      };
