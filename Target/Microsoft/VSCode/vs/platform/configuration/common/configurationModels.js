...
class ConfigurationModel {
  ...
  merge(...others) {
    ...
    for (const other of others) {
      raws.push(
        ...other.raw ? Array.isArray(other.raw) ? other.raw : [other.raw] : [other]
      );
      if (other.isEmpty()) {
        continue;
      }
    ...
    return new ConfigurationModel(
      ...
      keys,
      overrides,
      !raws.length || raws.every((raw) => raw instanceof ConfigurationModel) ? void 0 : raws,
      this.logService
    );
  }
...
class Configuration {
  ...
  get userConfiguration() {
    if (!this._userConfiguration) {
      if (this._remoteUserConfiguration.isEmpty()) {
        ...
        const merged = this._localUserConfiguration.merge(
    ...
    if (!this._policyConfiguration.isEmpty() && this._policyConfiguration.getValue(section) !== void 0) {
      ...
      for (const key of this._policyConfiguration.keys) {
        configurationModel.setValue(
          key,
          this._policyConfiguration.getValue(key)
        );
      }
    }
  ...
  static parse(data, logService) {
    ...
    const userLocalConfiguration = Configuration.parseConfigurationModel(
      data.userLocal,
      logService
    );
    const userRemoteConfiguration = Configuration.parseConfigurationModel(
      data.userRemote,
      logService
    );
    ...
    const folders = data.folders.reduce(
      (result, value) => {
        result.set(
          URI.revive(value[0]),
          Configuration.parseConfigurationModel(value[1], logService)
        );
        return result;
      },
      new ResourceMap()
