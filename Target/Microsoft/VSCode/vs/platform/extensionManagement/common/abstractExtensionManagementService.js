...
let AbstractExtensionManagementService = class extends CommontExtensionManagementService {
  ...
  async installExtensions(extensions) {
    ...
    try {
      for (const { manifest, extension, options } of extensions) {
        ...
        const existingInstallExtensionTask = !URI.isUri(extension) ? this.installingExtensions.get(
          getInstallExtensionTaskKey(
            extension,
  ...
  async checkAndGetCompatibleVersion(extension, sameVersion, installPreRelease, productVersion) {
    ...
    if (deprecationInfo?.extension?.autoMigrate) {
      ...
      if (!compatibleExtension) {
        ...
        if (!installPreRelease && extension.hasPreReleaseVersion && extension.properties.isPreReleaseVersion && (await this.galleryService.getExtensions(
          [extension.identifier],
          CancellationToken.None
        ))[0]) {
          throw new ExtensionManagementError(
  ...
  async uninstallExtensions(extensions) {
    ...
    for (const { extension, options } of extensions) {
      ...
      if (uninstallOptions.remove) {
        ...
        for (const profile of this.userDataProfilesService.profiles) {
          ...
          const profileExtension = installed.find(
            (e) => areSameExtensions(e.identifier, extension.identifier)
          );
          if (profileExtension) {
            ...
            const uninstallExtensionTask2 = this.uninstallingExtensions.get(
              getUninstallExtensionTaskKey(
                profileExtension,
                uninstallOptionsWithProfile
              )
            );
            if (uninstallExtensionTask2) {
              this.logService.info(
                "Extensions is already requested to uninstall",
                profileExtension.identifier.id
              );
              alreadyRequestedUninstalls.push(
                uninstallExtensionTask2.waitUntilTaskIsFinished()
              );
            } else {
              allTasks.push(
                createUninstallExtensionTask(
    ...
    try {
      ...
      if (extensionsToRemove.length) {
        await this.joinAllSettled(
          extensionsToRemove.map(
            (extension) => this.removeExtension(extension)
          )
        );
      }
    } catch (e) {
  ...
  getDependents(extension, installed) {
    return installed.filter(
      (e) => e.manifest.extensionDependencies?.some(
