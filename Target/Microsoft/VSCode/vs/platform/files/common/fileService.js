...
let FileService = class extends Disposable {
  ...
  async writeFile(resource, bufferOrReadableOrStream, options) {
    ...
    try {
      ...
      if (!bufferOrReadableOrStreamOrBufferedStream) {
        bufferOrReadableOrStreamOrBufferedStream = await this.peekBufferForWriting(
          provider,
          bufferOrReadableOrStream
        );
      }
  ...
  async validateWriteFile(provider, resource, bufferOrReadableOrStream, options) {
    ...
    if (typeof options?.mtime === "number" && typeof options.etag === "string" && options.etag !== ETAG_DISABLED && typeof stat.mtime === "number" && typeof stat.size === "number" && options.mtime < stat.mtime && options.etag !== etag({
      ...
      buffer = await this.peekBufferForWriting(
        provider,
        bufferOrReadableOrStream
      );
