...
function getShellIntegrationInjection(shellLaunchConfig, options, env, logService, productService) {
  ...
  if (!shellLaunchConfig.executable) {
    return {
      type: "failure",
      reason: ShellIntegrationInjectionFailureReason.NoExecutable
    };
  }
  if (shellLaunchConfig.isFeatureTerminal && !shellLaunchConfig.forceShellIntegration) {
    return {
      type: "failure",
      reason: ShellIntegrationInjectionFailureReason.FeatureTerminal
    };
  }
  ...
  if (isWindows && (!options.windowsEnableConpty || getWindowsBuildNumber() < 18309)) {
    return {
      type: "failure",
      reason: ShellIntegrationInjectionFailureReason.Winpty
    };
  }
  ...
  if (isWindows) {
    if (shell === "pwsh.exe" || shell === "powershell.exe") {
      ...
      if (!newArgs) {
        return {
          type: "failure",
          reason: ShellIntegrationInjectionFailureReason.UnsupportedArgs
        };
      }
      newArgs = [...newArgs];
    ...
    return {
      type: "failure",
      reason: ShellIntegrationInjectionFailureReason.UnsupportedShell
    };
  }
  switch (shell) {
    case "bash": {
    ...
    case "fish": {
      if (!originalArgs || originalArgs.length === 0) {
        ...
        newArgs = shellIntegrationArgs.get(
          "fish-login" /* FishLogin */
        );
      } else if (originalArgs === shellIntegrationArgs.get("fish" /* Fish */) || originalArgs === shellIntegrationArgs.get(
        "fish-login" /* FishLogin */
      )) {
...
shellIntegrationArgs.set("fish" /* Fish */, [
  "--init-command",
  'source "{0}/out/vs/workbench/contrib/terminal/common/scripts/shellIntegration.fish"'
]);
shellIntegrationArgs.set("fish-login" /* FishLogin */, [
  "-l",
  "--init-command",
  'source "{0}/out/vs/workbench/contrib/terminal/common/scripts/shellIntegration.fish"'
]);
