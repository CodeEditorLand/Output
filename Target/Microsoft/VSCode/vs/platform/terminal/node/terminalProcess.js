...
let TerminalProcess = class extends Disposable {
  ...
  async start() {
    ...
    const injection = getShellIntegrationInjection(
    ...
    if (injection.type === "injection") {
      ...
      this._onDidChangeProperty.fire({
        type: ProcessPropertyType.ShellIntegrationInjectionFailureReason,
  ...
  async _validateExecutable() {
    ...
    const envPaths = slc.env?.PATH ? slc.env.PATH.split(path.delimiter) : void 0;
