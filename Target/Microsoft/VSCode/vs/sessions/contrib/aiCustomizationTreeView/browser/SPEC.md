...
The AI Customization Tree View is a sidebar tree that groups AI customization
files by type and storage. It is optimized for agent sessions and filters
worktree items to the active session repository.
...
- Refreshes on prompt service changes, workspace changes, and active session
  changes.
...
interfaces. The tree view should rely on the prompt discovery service rather
than scanning the file system directly.
...
- Skills display the frontmatter name when available, falling back to the folder
  name.
...
_This specification documents the AI Customization Tree View in
`src/vs/sessions/contrib/aiCustomizationTreeView/browser/`._
