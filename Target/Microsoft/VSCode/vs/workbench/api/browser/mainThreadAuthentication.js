...
let MainThreadAuthentication = class MainThreadAuthentication2 extends Disposable {
  ...
  async $sendDidChangeDynamicProviderInfo({ providerId, clientId, authorizationServer, label }) {
    ...
    this.dynamicAuthProviderStorageService.storeClientId(providerId || existing.providerId, authorizationServer ? URI.revive(authorizationServer).toString(true) : existing.authorizationServer, clientId || existing.clientId, label || existing.label);
  }
