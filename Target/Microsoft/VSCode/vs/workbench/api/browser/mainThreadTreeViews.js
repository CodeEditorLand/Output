  ...
  $refresh(treeViewId, itemsToRefreshByHandle) {
    ...
    if (viewer && dataProvider) {
      ...
      return viewer.refresh(
        itemsToRefresh.items.length ? itemsToRefresh.items : void 0,
        itemsToRefresh.checkboxes.length ? itemsToRefresh.checkboxes : void 0
      );
    }
...
class TreeViewDataProvider {
  ...
  getChildrenBatch(treeItems) {
    ...
    return this._proxy.$getChildren(
      this.treeViewId,
      treeItems ? treeItems.map((item) => item.handle) : void 0
    ).then(
      (children) => {
