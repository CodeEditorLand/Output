    ...
    for (let i = 0; i < commandsOrActions.length; i++) {
      ...
      if (CodeActionAdapter._isCommand(candidate) && !(candidate instanceof CodeAction)) {
        ...
        actions.push({
          ...
          command: toConvert.command && this._commands.toInternal(
            toConvert.command,
            disposables
          ),
          diagnostics: toConvert.diagnostics?.map(
            typeConvert.Diagnostic.from
          ),
          edit: toConvert.edit && typeConvert.WorkspaceEdit.from(
            toConvert.edit,
            void 0
          ),
          kind: toConvert.kind?.value,
...
class DocumentPasteEditProvider {
  ...
  _cachedPrepare;
  _editsCache = new Cache(
    "DocumentPasteEdit.edits"
  );
  async prepareDocumentPaste(resource, ranges, dataTransferDto, token) {
    ...
    const newEntries = Array.from(dataTransfer).filter(
    ...
    const items = await Promise.all(
      Array.from(newEntries, async ([mime, value]) => {
        ...
        return [
          mime,
          await typeConvert.DataTransferItem.from(mime, value, id)
        ];
      })
    );
  ...
  async providePasteEdits(requestId, resource, ranges, dataTransferDto, context, token) {
    ...
    const doc = this._documents.getDocument(resource);
    const vscodeRanges = ranges.map((range) => typeConvert.Range.to(range));
    const items = dataTransferDto.items.map(
      ([mime, value]) => {
  ...
  async resolvePasteEdit(id, token) {
    ...
    return {
      ...
      additionalEdit: resolvedItem.additionalEdit ? typeConvert.WorkspaceEdit.from(
...
class CompletionsAdapter {
  ...
  _convertCompletionItem(item, id, defaultInsertRange, defaultReplaceRange) {
    ...
    const result = {
      ...
      [extHostProtocol.ISuggestDataDtoField.additionalTextEdits]: item.additionalTextEdits?.map(typeConvert.TextEdit.from),
...
class InlineCompletionAdapter {
  ...
  get supportsHandleEvents() {
    return isProposedApiEnabled(
      this._extension,
      "inlineCompletionsAdditions"
    ) && (typeof this._provider.handleDidShowCompletionItem === "function" || typeof this._provider.handleDidPartiallyAcceptCompletionItem === "function" || typeof this._provider.handleDidRejectCompletionItem === "function");
  }
      ...
      items: normalizedResult.map(
        (item, idx) => {
          ...
          if (item.action) {
            ...
            action = this._commands.toInternal(
              item.action,
              disposableStore
            );
          }
          ...
          return {
            ...
            warning: item.warning && this._isAdditionsProposedApiEnabled ? {
              ...
              icon: item.warning.icon ? typeConvert.IconPath.fromThemeIcon(
                item.warning.icon
              ) : void 0
            } : void 0
          };
  ...
  async provideInlineEdits(uri, context, token) {
    ...
    const langResult = {
      ...
      commands: result.commands?.map(
        (c) => this._commands.toInternal(c, disposableStore)
      )
    };
    return langResult;
  }
...
class ExtHostLanguageFeatures {
  ...
  registerDocumentSymbolProvider(extension, selector, provider, metadata) {
    ...
    const displayName = metadata?.label || ExtHostLanguageFeatures._extLabel(extension);
  ...
  registerDocumentOnDropEditProvider(extension, selector, provider, metadata) {
    ...
    this._proxy.$registerDocumentOnDropEditProvider(
      ...
      metadata ? {
        ...
        providedDropKinds: metadata.providedDropEditKinds?.map(
          (x) => x.value
        )
      } : void 0
    );
