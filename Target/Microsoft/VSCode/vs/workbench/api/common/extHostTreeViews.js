...
class ExtHostTreeViews extends Disposable {
  constructor(_proxy, commands, logService) {
    ...
    function isTreeViewConvertableItem(arg) {
      return arg?.$treeViewId && (arg.$treeItemHandle || arg.$selectedTreeItems || arg.$focusedTreeItem);
    }
  ...
  getHandlesToRefresh(elements) {
    ...
    for (const elementNode of elementNodes) {
      if (elementNode && !elementsToUpdate.has(elementNode.item.handle)) {
        ...
        while (currentNode?.parent && elementNodes.findIndex(
          (node) => currentNode?.parent && node && node.item.handle === currentNode.parent.item.handle
        ) === -1) {
