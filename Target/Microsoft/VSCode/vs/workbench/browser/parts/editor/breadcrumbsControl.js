    ...
    if (element instanceof OutlineElement && outline.uri) {
      this._disposables.add(
        this._instantiationService.invokeFunction(
          (accessor) => createBreadcrumbDndObserver(
            accessor,
            container,
            element.symbol.name,
    ...
    onDragStart: /* @__PURE__ */ __name((event) => {
      ...
      instantiationService.invokeFunction((accessor2) => {
        if (URI.isUri(item)) {
          ...
          fillEditorsDragData(
            accessor2,
            [{ resource: item.uri, selection: item.symbol.range }],
            event
          );
          fillInSymbolsDragData(
            [
...
let BreadcrumbsControl = class {
  constructor(container, _options, _editorGroup, _contextKeyService, _contextViewService, _instantiationService, _quickInputService, _fileService, _editorService, _labelService, configurationService, breadcrumbsService) {
    ...
    this._ckBreadcrumbsPossible = BreadcrumbsControl.CK_BreadcrumbsPossible.bindTo(
      this._contextKeyService
    );
    this._ckBreadcrumbsVisible = BreadcrumbsControl.CK_BreadcrumbsVisible.bindTo(
      this._contextKeyService
    );
    this._ckBreadcrumbsActive = BreadcrumbsControl.CK_BreadcrumbsActive.bindTo(
      this._contextKeyService
    );
  ...
  update() {
    ...
    const updateBreadcrumbs = /* @__PURE__ */ __name(() => {
      ...
      const items = model.getElements().map(
        (element) => element instanceof FileElement ? this._instantiationService.createInstance(
          FileItem,
          model,
          element,
          options,
