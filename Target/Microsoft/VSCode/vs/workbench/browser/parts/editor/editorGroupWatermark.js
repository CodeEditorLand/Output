...
const showCopilot = ContextKeyExpr.or(
  ContextKeyExpr.equals("chatSetupHidden", false),
  ContextKeyExpr.equals("chatSetupInstalled", true)
);
const openChat = {
  text: localize("watermark.openChat", "Open Chat"),
  id: "workbench.action.chat.open",
  when: { native: showCopilot, web: showCopilot }
};
...
const workspaceEntries = [showCommands, gotoFile, openChat];
...
let EditorGroupWatermark = class extends Disposable {
  ...
  registerListeners() {
    this._register(
      this.configurationService.onDidChangeConfiguration((e) => {
        if (e.affectsConfiguration("workbench.tips.enabled") && this.enabled !== this.configurationService.getValue(
      ...
      this.storageService.onWillSaveState((e) => {
        if (e.reason === WillSaveStateReason.SHUTDOWN) {
          ...
          for (const entry of entries) {
            ...
            if (when) {
              this.cachedWhen[entry.id] = this.contextKeyService.contextMatchesRules(
                when
              );
            }
          }
  ...
  filterEntries(entries, shuffleEntries) {
    const filteredEntries = entries.filter(
      (entry) => isWeb && !entry.when?.web || !isWeb && !entry.when?.native || this.cachedWhen[entry.id]
