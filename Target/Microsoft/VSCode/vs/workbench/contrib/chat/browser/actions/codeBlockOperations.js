...
let ApplyCodeBlockOperation = class {
  ...
  async run(context) {
    ...
    const codemapperUri = await this.evaluateURIToUse(
    ...
    if (codemapperUri && !isEqual(activeEditorControl?.getModel().uri, codemapperUri)) {
      try {
        ...
        this.logService.info(
          "[ApplyCodeBlockOperation] error opening code mapper file",
          codemapperUri,
          e
        );
        return;
    ...
    let result = void 0;
    if (activeEditorControl) {
      result = await this.handleTextEditor(
        activeEditorControl,
        context.code
      );
    } else {
    ...
    const activeEditorOption = activeEditorControl?.getModel().uri ? {
      label: localize(
        ...
        this.labelService.getUriLabel(
          activeEditorControl.getModel().uri,
          { relative: true }
        )
      ),
      id: "activeEditor"
    ...
    if (selected) {
      switch (selected.id) {
        ...
        case "newUntitledFile":
          return URI.from({
            scheme: "untitled",
            path: resource ? resource.path : "Untitled-1"
          });
        case "activeEditor":
  ...
  async handleTextEditor(codeEditor, code) {
    ...
    const codeBlock = {
      code,
      resource: activeModel.uri,
      markdownBeforeBlock: void 0
    };
    ...
    try {
      const iterable = await this.progressService.withProgress(
        ...
        async (progress) => {
          ...
          const editsIterable = this.getEdits(
            codeBlock,
      ...
      editsProposed = await this.applyWithInlinePreview(
        iterable,
        codeEditor,
        cancellationTokenSource
      );
  ...
  tryToRevealCodeBlock(codeEditor, codeBlock) {
    ...
    if (match && match[1].length > 10) {
      const findMatch = codeEditor.getModel().findNextMatch(
        match[1],
...
function getEditableActiveCodeEditor(editorService) {
  ...
  if (activeCodeEditorInNotebook?.hasTextFocus() && activeCodeEditorInNotebook.hasModel()) {
