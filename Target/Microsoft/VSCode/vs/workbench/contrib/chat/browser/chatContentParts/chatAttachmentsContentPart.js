...
let ChatAttachmentsContentPart = class extends Disposable {
  constructor(variables, contentReferences, domNode, contextKeyService, instantiationService, openerService, hoverService, commandService, themeService, labelService) {
    super();
  ...
  _contextResourceLabels = this._register(
  ...
  initAttachedContext(container) {
    ...
    const hoverDelegate = this.attachedContextDisposables.add(
    ...
    this.variables.forEach(async (attachment) => {
      ...
      if (resource && (attachment.isFile || attachment.isDirectory)) {
        ...
        const hoverElement = this.customAttachment(
          widget,
          attachment.name,
          hoverDelegate,
          ariaLabel,
        ...
        this.createImageElements(
          ...
          widget,
          hoverElement,
          URI.isUri(ref) ? ref : void 0,
          omissionType
        );
        this.attachedContextDisposables.add(
          this.hoverService.setupDelayedHover(widget, {
        ...
        const classNames = [
          "file-icon",
          `${attachment.language}-lang-file-icon`
        ];
        if (attachment.copiedFrom) {
          resource = attachment.copiedFrom.uri;
          range = attachment.copiedFrom.range;
          const filename = basename(resource.path);
          label.setLabel(filename, void 0, {
            extraClasses: classNames
          });
        } else {
          label.setLabel(attachment.fileName, void 0, {
            extraClasses: classNames
          });
        }
        widget.appendChild(
          dom.$(
            "span.attachment-additional-info",
            {},
            `Pasted ${attachment.pastedLines}`
          )
        );
        widget.style.position = "relative";
        const hoverContent = {
...
\`\`\``
          ...
          if (resource2) {
            this.attachedContextDisposables.add(
              this.instantiationService.invokeFunction(
                (accessor) => hookUpResourceAttachmentDragAndContextMenu(
                  accessor,
                  widget,
                  resource2
                )
      ...
      if (attachment.kind === "symbol") {
        const scopedContextKeyService = this.attachedContextDisposables.add(
          this.contextKeyService.createScoped(widget)
        );
        this.attachedContextDisposables.add(
          this.instantiationService.invokeFunction(
            (accessor) => hookUpSymbolAttachmentDragAndContextMenu(
              ...
              { ...attachment, kind: attachment.symbolKind },
  ...
  customAttachment(widget, friendlyName, hoverDelegate, ariaLabel, isAttachmentOmitted, isImage, isURL, value) {
    const pillIcon = dom.$(
      ...
      {},
      dom.$(
        isAttachmentOmitted ? "span.codicon.codicon-warning" : "span.codicon.codicon-file-media"
      )
    );
    const textLabel = dom.$(
    ...
    if (isURL && !isAttachmentOmitted && value) {
      hoverElement.textContent = localize(
        ...
        "{0}",
        convertUint8ArrayToString(value)
      );
      this.attachedContextDisposables.add(
        this.hoverService.setupDelayedHover(widget, {
          content: hoverElement,
  ...
  createImageElements(buffer, widget, hoverElement, reference, omittedState) {
    ...
    const pillImg = dom.$("img.chat-attached-context-pill-image", {
      src: url,
      alt: ""
    });
    const pill = dom.$("div.chat-attached-context-pill", {}, pillImg);
    const existingPill = widget.querySelector(
      ".chat-attached-context-pill"
    );
    if (existingPill) {
      existingPill.replaceWith(pill);
    }
    const hoverImage = dom.$("img.chat-attached-context-image", {
      src: url,
      alt: ""
    });
    const imageContainer = dom.$(
      "div.chat-attached-context-image-container",
      {},
      hoverImage
    );
    hoverElement.appendChild(imageContainer);
    if (reference) {
      const urlContainer = dom.$(
        "a.chat-attached-context-url",
        {},
        omittedState === OmittedState.Partial ? localize(
          "chat.imageAttachmentWarning",
          "This GIF was partially omitted - current frame was be sent."
        ) : reference.toString()
      );
      ...
      this._register(
        dom.addDisposableListener(
          urlContainer,
          "click",
          () => this.openResource(reference, false, void 0)
        )
      );
    ...
    hoverImage.onerror = () => {
      const pillIcon = dom.$(
        "div.chat-attached-context-pill",
        {},
        dom.$("span.codicon.codicon-file-media")
      );
      const pill2 = dom.$("div.chat-attached-context-pill", {}, pillIcon);
      const existingPill2 = widget.querySelector(
        ".chat-attached-context-pill"
      );
      if (existingPill2) {
...
function hookUpResourceAttachmentDragAndContextMenu(accessor, widget, resource) {
  ...
  const scopedContextKeyService = store.add(
    contextKeyService.createScoped(widget)
  );
  store.add(setResourceContext(accessor, scopedContextKeyService, resource));
  widget.draggable = true;
  store.add(
    dom.addDisposableListener(widget, "dragstart", (e) => {
      instantiationService.invokeFunction(
        (accessor2) => fillEditorsDragData(accessor2, [resource], e)
      );
...
function hookUpSymbolAttachmentDragAndContextMenu(accessor, widget, scopedContextKeyService, attachment, contextMenuId) {
  ...
  const chatResourceContext = chatAttachmentResourceContextKey.bindTo(
    scopedContextKeyService
  );
  ...
  const providerContexts = [
    [
      EditorContextKeys.hasDefinitionProvider.bindTo(
        scopedContextKeyService
      ),
      languageFeaturesService.definitionProvider
    ],
    [
      EditorContextKeys.hasReferenceProvider.bindTo(
        scopedContextKeyService
      ),
  ...
  const updateContextKeys = /* @__PURE__ */ __name(async () => {
    const modelRef = await textModelService.createModelReference(
      attachment.value.uri
    );
    try {
