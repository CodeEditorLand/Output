    ...
    buttons.forEach((buttonData) => {
      ...
      if (buttonData.moreActions) {
        button = new ButtonWithDropdown(elements.buttonsContainer, {
          ...
          actions: buttonData.moreActions.map((action) => this._register(new Action(action.label, action.label, void 0, true, () => {
...
let ChatConfirmationWidget = class ChatConfirmationWidget2 extends BaseChatConfirmationWidget {
  ...
  constructor(title, subtitle, message, buttons, instantiationService, contextMenuService, configurationService, hostService) {
    super(title, subtitle, buttons, instantiationService, contextMenuService, configurationService, hostService);
    this.message = message;
    const renderedMessage = this._register(this.markdownRenderer.render(typeof this.message === "string" ? new MarkdownString(this.message) : this.message, { asyncRenderCallback: /* @__PURE__ */ __name(() => this._onDidChangeHeight.fire(), "asyncRenderCallback") }));
...
let ChatCustomConfirmationWidget = class ChatCustomConfirmationWidget2 extends BaseChatConfirmationWidget {
  ...
  constructor(title, subtitle, messageElement, buttons, instantiationService, contextMenuService, configurationService, hostService) {
    super(title, subtitle, buttons, instantiationService, contextMenuService, configurationService, hostService);
