...
let ChatMarkdownContentPart = class extends Disposable {
  constructor(markdown, context, editorPool, fillInIncompleteTokens, codeBlockStartIndex, renderer, currentWidth, codeBlockModelCollection, rendererOptions, contextKeyService, textModelService, instantiationService) {
    super();
        ...
        markdown.content,
        {
          ...
          codeBlockRendererSync: /* @__PURE__ */ __name((languageId, text, raw) => {
            ...
            if (equalsIgnoreCase(languageId, localFileLanguageId)) {
              ...
              const fastUpdateModelEntry = this.codeBlockModelCollection.updateSync(
                sessionId,
                element,
                globalIndex,
                {
  ...
  renderCodeBlock(data, text, isComplete, currentWidth) {
    ...
    if (isResponseVM(data.element)) {
      this.codeBlockModelCollection.update(
        ...
        data.element,
        data.codeBlockIndex,
        { text, languageId: data.languageId, isComplete }
      ).then((e) => {
...
let CollapsedCodeBlock = class extends Disposable {
  ...
  render(uri, isStreaming) {
    ...
    this.updateTooltip(
      this.labelService.getUriLabel(uri, { relative: false })
    );
    const renderDiff = /* @__PURE__ */ __name((changes) => {
      ...
      if (changes && !changes?.identical && !changes?.quitEarly) {
        ...
        const deletionsFragment = changes.removed === 1 ? localize("chat.codeblock.deletions.one", "1 deletion") : localize(
    ...
    this._progressStore.add(
      autorun((r) => {
        ...
        if (!isStreaming && !isComplete2) {
          ...
          labelDetail.textContent = value === 0 || !value ? localize(
            "chat.codeblock.generating",
            "Generating edits..."
          ) : localize(
        ...
        if (!diffBetweenStops) {
          diffBetweenStops = modifiedEntry && editSession ? editSession.getEntryDiffBetweenStops(
