  ...
  class RemoveFileFromWorkingSet extends WorkingSetAction {
    ...
    async runWorkingSetAction(accessor, currentEditingSession, chatWidget, ...uris) {
      ...
      const pendingEntries = currentEditingSession.entries.get().filter(
        (entry) => uris.includes(entry.modifiedURI) && entry.state.get() === ModifiedFileEntryState.Modified
      );
...
class ViewPreviousEditsAction extends EditingSessionAction {
  static {
    __name(this, "ViewPreviousEditsAction");
  }
  static Id = "chatEditing.viewPreviousEdits";
  static Label = localize(
