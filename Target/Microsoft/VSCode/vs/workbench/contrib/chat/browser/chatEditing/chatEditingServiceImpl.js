...
let ChatEditingService = class extends Disposable {
  constructor(_instantiationService, multiDiffSourceResolverService, textModelService, contextKeyService, _chatService, _editorService, decorationsService, _fileService, lifecycleService, storageService, logService, extensionService, productService, notebookService) {
    ...
    const readonlyEnabledContextKey = chatEditingAgentSupportsReadonlyReferencesContextKey.bindTo(
      contextKeyService
    );
    const setReadonlyFilesEnabled = /* @__PURE__ */ __name(() => {
      const enabled = productService.quality !== "stable" && extensionService.extensions.some(
        (e) => e.enabledApiProposals?.includes(
    ...
    setReadonlyFilesEnabled();
    this._register(
      extensionService.onDidRegisterExtensions(setReadonlyFilesEnabled)
    );
    this._register(
      extensionService.onDidChangeExtensions(setReadonlyFilesEnabled)
    );
    let storageTask;
    this._register(
  ...
  async createEditingSession(chatModel, global = false) {
    assertType(
      this.getEditingSession(chatModel.sessionId) === void 0,
  ...
  installAutoApplyObserver(session, chatModel) {
    ...
    observerDisposables.add(
      chatModel.onDidDispose(() => observerDisposables.dispose())
    );
  ...
  observerEditsInResponse(requestId, responseModel, session, observerDisposables) {
    ...
    const ensureEditorOpen = /* @__PURE__ */ __name((partUri) => {
      ...
      editedFilesExist.set(
        ...
        fileExists.then((e) => {
          ...
          const inactive = this._editorService.activeEditorPane?.input instanceof ChatEditorInput && this._editorService.activeEditorPane.input.sessionId === session.chatSessionId || Boolean(
            activeUri && session.entries.get().find(
              (entry) => isEqual(activeUri, entry.modifiedURI)
            )
          );
    ...
    const handleResponseParts = /* @__PURE__ */ __name(async () => {
      ...
      for (let i = 0; i < responseModel.response.value.length; i++) {
        ...
        if (!entry) {
          entry = {
            seen: 0,
            streaming: session.startStreamingEdits(
              CellUri.parse(part.uri)?.notebook ?? part.uri,
              responseModel,
              undoStop
            )
          };
          ...
          if (part.kind === "notebookEditGroup") {
            newEdits.forEach((edit) => {
              if (TextEdit.isTextEdit(edit)) {
                ...
                entry.streaming.pushNotebookCellText(edit.uri, [
                  edit.edit
                ]);
              } else {
    ...
    if (responseModel.isComplete) {
      ...
      const disposable = observerDisposables.add(
        responseModel.onDidChange((e2) => {
  ...
  async getRelatedFiles(chatSessionId, prompt, files, token) {
    ...
    const result = await Promise.all(
      providers.map(async (provider) => {
        try {
          const relatedFiles = await provider.provideRelatedFiles(
            { prompt, files },
            token
          );
...
let ChatDecorationsProvider = class extends Disposable {
  ...
  _modifiedUris = derived(this, (r) => {
    const uri = this._currentEntries.read(r);
    return uri.filter(
      (entry) => !entry.isCurrentlyBeingModifiedBy.read(r) && entry.state.read(r) === ModifiedFileEntryState.Modified
    ).map((entry) => entry.modifiedURI);
  });
  onDidChange = Event.any(
    observeArrayChanges(
      this._currentlyEditingUris,
      compareBy((uri) => uri.toString(), compare),
      this._store
    ),
    observeArrayChanges(
      this._modifiedUris,
    ...
    if (isModified) {
      ...
      return {
        ...
        tooltip: defaultAgentName ? localize(
          "chatEditing.modified",
          "Pending changes from {0}",
          defaultAgentName
        ) : localize(
...
let ChatEditingMultiDiffSourceResolver = class {
  ...
  async resolveDiffSource(uri) {
    ...
    const thisSession = derived(this, (r) => {
      return this._editingSessionsObs.read(r).find(
        (candidate) => candidate.chatSessionId === parsed.chatSessionId
      );
    });
    return this._instantiationService.createInstance(
      ChatEditingMultiDiffSource,
      thisSession,
      parsed.showPreviousChanges
    );
  }
};
    ...
    (reader) => {
      ...
      return entries.map((entry) => {
        if (this._showPreviousChanges) {
          const entryDiffObs = currentSession.getEntryDiffBetweenStops(
            entry.modifiedURI,
            void 0,
            void 0
          );
