...
let InlineAnchorWidget = class extends Disposable {
  constructor(element, inlineReference, originalContextKeyService, contextMenuService, fileService, hoverService, instantiationService, labelService, languageService, menuService, modelService, telemetryService, themeService) {
    ...
    if (this.data.kind === "symbol") {
      ...
      const recomputeIconClasses = /* @__PURE__ */ __name(() => getIconClasses(
        modelService,
        languageService,
        location.uri,
        fileKind,
        fileKind === FileKind.FOLDER && !themeService.getFileIconTheme().hasFolderIcons ? FolderThemeIcon : void 0
    ...
    if (this.data.kind !== "symbol") {
      ...
      this._register(
        dom.addDisposableListener(element, "dragstart", (e) => {
          ...
          instantiationService.invokeFunction(
            (accessor) => fillEditorsDragData(accessor, [stat], e)
          );
  ...
  class GoToDefinitionAction extends Action2 {
    ...
    async run(accessor, location) {
      ...
      return instantiationService.invokeFunction(
        (accessor2) => action.run(accessor2)
      );
    }
  }
);
