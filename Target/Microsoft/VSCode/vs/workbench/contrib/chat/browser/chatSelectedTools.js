...
let ChatSelectedTools = class extends Disposable {
  ...
  constructor(toolsService, instaService, storageService) {
    super();
    this._selectedTools = this._register(
      storedTools(
    ...
    const allTools = observableFromEvent(
      ...
      () => Array.from(toolsService.getTools()).filter(
        (t) => t.supportsToolPicker
      )
    );
            ...
            updateLabel() {
              this._store.add(
                autorun((r) => {
                  ...
                  const message = count === 0 ? "$(tools)" : enabled !== count ? localize(
                    "tool.1",
                    "{0} {1} of {2}",
                    "$(tools)",
                    enabled,
