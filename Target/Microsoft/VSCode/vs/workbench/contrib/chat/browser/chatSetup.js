...
let ChatSetup = class ChatSetup2 {
  ...
  static getInstance(instantiationService, context, controller) {
    ...
    if (!instance) {
      instance = ChatSetup_1.instance = instantiationService.invokeFunction((accessor) => {
        return new ChatSetup_1(context, controller, instantiationService, accessor.get(ITelemetryService), accessor.get(IWorkbenchLayoutService), accessor.get(IKeybindingService), accessor.get(IChatEntitlementService), accessor.get(ILogService), accessor.get(IConfigurationService), accessor.get(IViewsService), accessor.get(IProductService), accessor.get(IOpenerService));
      });
    }
    return instance;
  }
  constructor(context, controller, instantiationService, telemetryService, layoutService, keybindingService, chatEntitlementService, logService, configurationService, viewsService, productService, openerService) {
    this.context = context;
    this.controller = controller;
    this.instantiationService = instantiationService;
    this.telemetryService = telemetryService;
  ...
  getButtons(variant) {
    ...
    if (this.context.state.entitlement === ChatEntitlement.Unknown) {
      ...
      const alternateProviderSetting = this.configurationService.getValue("chat.setup.signInWithAlternateProvider");
