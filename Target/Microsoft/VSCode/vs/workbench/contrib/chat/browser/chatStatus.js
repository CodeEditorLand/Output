...
let ChatStatusDashboard = class ChatStatusDashboard2 extends Disposable {
  ...
  show(token) {
    ...
    if (chatQuota || completionsQuota || premiumChatQuota) {
      ...
      const completionsQuotaIndicator = completionsQuota ? this.createQuotaIndicator(this.element, disposables, completionsQuota, localize("completionsLabel", "Code completions"), false) : void 0;
      const chatQuotaIndicator = chatQuota ? this.createQuotaIndicator(this.element, disposables, chatQuota, localize("chatsLabel", "Chat messages"), false) : void 0;
      const premiumChatQuotaIndicator = premiumChatQuota ? this.createQuotaIndicator(this.element, disposables, premiumChatQuota, localize("premiumChatsLabel", "Premium requests"), true) : void 0;
      if (resetDate) {
  ...
  createQuotaIndicator(container, disposables, quota, label, supportsOverage) {
    ...
    const update = /* @__PURE__ */ __name((quota2) => {
      ...
      if (quota2.unlimited) {
        ...
        quotaValue.textContent = localize("quotaDisplayWithOverage", "+{0} requests", quota2.overageCount);
      } else {
