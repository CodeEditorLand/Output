...
let ChatWidget = class extends Disposable {
  constructor(location, _viewContext, viewOptions, styles, codeEditorService, configurationService, contextKeyService, instantiationService, chatService, chatAgentService, chatWidgetService, contextMenuService, chatAccessibilityService, logService, themeService, chatSlashCommandService, chatEditingService, storageService, telemetryService) {
    ...
    const viewModelObs = observableFromEvent(
      this,
      ...
      bindContextKey(
        ...
        (reader) => {
          ...
          const decidedEntries = entries.filter(
            (entry) => entry.state.read(reader) !== ModifiedFileEntryState.Modified
          );
    ...
    this._register(
      autorunWithStore((r, store) => {
        ...
        const session = sessions.find(
          (candidate) => candidate.chatSessionId === viewModel?.sessionId
        );
    ...
    const loadedWelcomeContent = storageService.getObject(
      `${PersistWelcomeMessageContentKey}.${this.location}`,
    ...
    this._register(
      this.onDidChangeParsedInput(() => this.updateChatInputContext())
    );
  }
  static {
  ...
  render(parent) {
    ...
    const scrollDownButton = this._register(
      new Button(this.listContainer, {
        supportIcons: true,
    ...
    scrollDownButton.setTitle(
      localize("scrollDownButtonLabel", "Scroll down")
    );
    this._register(
      scrollDownButton.onDidClick(() => {
    ...
    this._register(
      this.chatWidgetService.register(this)
    );
  }
  scrollToEnd() {
    if (this.lastItem) {
      const offset = Math.max(
        this.lastItem.currentRenderedHeight ?? 0,
        1e6
      );
      ...
      this.tree.setChildren(null, treeItems, {
        diffIdentityProvider: {
          getId: /* @__PURE__ */ __name((element) => {
            return `${element.dataId}${isRequestVM(element)}${isResponseVM(element) && element.renderData ? `_${this.visibleChangeCount}` : ""}${isResponseVM(element) ? `_${element.contentReferences.length}` : ""}_${element.shouldBeRemovedOnSend ? `${element.shouldBeRemovedOnSend.afterUndoStop || "1"}` : "0"}${isRequestVM(element) && element.contentReferences ? `_${element.contentReferences?.length}` : ""}${isResponseVM(element) && element.model.isPaused.get() ? "_paused" : ""}`;
          }, "getId")
        }
      });
  ...
  renderWelcomeViewContentIfNeeded() {
    ...
    if (welcomeContent && !numItems && this.welcomeMessageContainer.children.length === 0) {
      ...
      const tips = this.viewOptions.supportsAdditionalParticipants ? new MarkdownString(
        localize(
          "chatWidget.tips",
        ...
        localize(
          "chatWidget.tips.withoutParticipants",
          "{0} or type {1} to attach context",
          "$(attach)",
          "#"
        ),
      ...
      dom.append(
        ...
        this.welcomePart.value.element
      );
    }
    if (this.viewModel) {
  ...
  async renderChatEditingSessionState() {
    ...
    this.inputPart.renderChatEditingSessionState(
      this._editingSession.get() ?? null
    );
  ...
  renderSampleQuestions() {
    if (this.viewModel?.getItems().length === 0) {
      this.renderFollowups(
        this.input.currentMode === ChatMode.Ask ? this.viewModel.model.sampleQuestions : void 0
      );
    }
  }
    ...
    this._register(
      this.renderer.onDidClickRerunWithAgentOrCommandDetection((item) => {
        ...
        if (request) {
          ...
          this.chatService.resendRequest(request, options2).catch(
    ...
    this._register(
      this.tree.onDidScroll(() => {
        ...
        this.container.classList.toggle(
          "show-scroll-down",
          !isScrolledDown && !this.scrollLock
        );
      })
    );
  }
  ...
  createInput(container, options) {
    ...
    this._register(
      this.inputPart.onDidAcceptFollowup((e) => {
        ...
        let msg = "";
        if (e.followup.agentId && e.followup.agentId !== this.chatAgentService.getDefaultAgent(
          this.location,
          this.input.currentMode
        )?.id) {
    ...
    this._register(
      this.inputPart.attachmentModel.onDidChangeContext(() => {
    ...
    this._register(
      this.input.onDidChangeCurrentChatMode(() => {
        this.renderSampleQuestions();
        this.renderWelcomeViewContentIfNeeded();
        this.refreshParsedInput();
      })
    ...
    this.viewModelDisposables.add(
      Event.accumulate(
        ...
        this.isRequestPaused.set(
          this.viewModel.requestPausibility === ChatPauseState.Paused
        );
        this.canRequestBePaused.set(
          this.viewModel.requestPausibility !== ChatPauseState.NotPausable
        );
        this.onDidChangeItems();
  ...
  async _acceptInput(query, options) {
    ...
    if (this.viewModel) {
      ...
      this.scrollLock = !!checkModeOption(
        this.input.currentMode,
        this.viewOptions.autoScroll
      );
      ...
      if (instructionsEnabled) {
        ...
        this.logService.trace(
          `[\u23F1] instructions tree resolved in ${performance.now() - instructionsStarted}ms`
        );
      }
      ...
      this.chatService.cancelCurrentRequestForSession(
        this.viewModel.sessionId
      );
  ...
  layout(height, width) {
    ...
    this.inputPart.layout(
      this._dynamicMessageLayoutData?.enabled ? this._dynamicMessageLayoutData.maxHeight : height,
      width
    );
    ...
    if (this.viewOptions.renderFollowups) {
      welcomeOffset = Math.max(
        welcomeOffset - this.inputPart.followupsHeight,
        0
      );
    }
    if (this.viewOptions.enableWorkingSet) {
      welcomeOffset = Math.max(
        welcomeOffset - this.inputPart.editSessionWidgetHeight,
        0
      );
    }
    welcomeOffset = Math.max(
      welcomeOffset - this.inputPart.attachmentsHeight,
      0
    );
    ...
    const lastItem = this.viewModel?.getItems().at(-1);
    const lastResponseIsRendering = isResponseVM(lastItem) && lastItem.renderData;
    if (lastElementVisible && (!lastResponseIsRendering || checkModeOption(
      this.input.currentMode,
      this.viewOptions.autoScroll
    ))) {
      this.scrollToEnd();
  ...
  updateChatInputContext() {
    const currentAgent = this.parsedInput.parts.find(
...
class ChatWidgetService extends Disposable {
  ...
  _onDidAddWidget = this._register(
    new Emitter()
  );
