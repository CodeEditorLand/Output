...
let ChatImplicitContextContribution = class ChatImplicitContextContribution2 extends Disposable {
  ...
  constructor(codeEditorService, editorService, chatWidgetService, chatService, chatEditingService, configurationService, ignoredFilesService) {
    ...
    this._register(Event.runAndSubscribe(editorService.onDidActiveEditorChange, () => {
      ...
      if (codeEditor) {
        activeEditorDisposables.add(Event.debounce(Event.any(codeEditor.onDidChangeModel, codeEditor.onDidChangeCursorSelection, codeEditor.onDidScrollChange, codeEditor.onDidChangeModelLanguage), () => void 0, 500)(() => this.updateImplicitContext()));
      }
...
let ChatImplicitContext = class ChatImplicitContext2 extends Disposable {
  static {
