...
let SlashCommandCompletions = class SlashCommandCompletions2 extends Disposable {
  ...
  constructor(languageFeaturesService, chatWidgetService, chatSlashCommandService, promptsService) {
    super();
...
let BuiltinDynamicCompletions = class BuiltinDynamicCompletions2 extends Disposable {
  ...
  constructor(historyService, workspaceContextService, searchService, labelService, languageFeaturesService, chatWidgetService, _chatEditingService, outlineService, editorService, configurationService) {
    super();
  ...
  constructor(languageFeaturesService, chatWidgetService) {
    ...
    this._register(this.languageFeaturesService.completionProvider.register({ scheme: ChatInputPart.INPUT_SCHEME, hasAccessToAllModels: true }, {
      ...
      provideCompletionItems: /* @__PURE__ */ __name(async (model, position, _context, _token) => {
        ...
        const toolItems = [];
        toolItems.push(...widget.input.selectedToolsModel.tools.get().filter((t) => t.canBeReferencedInPrompt).filter((t) => !usedToolNames.has(t.toolReferenceName ?? "")).map((t) => {
          const source = t.source;
          const detail = source.type === "mcp" ? localize("desc", "MCP Server: {0}", source.label) : source.type === "extension" ? source.label : void 0;
