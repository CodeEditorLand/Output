  ...
  constructor(widget) {
    ...
    this._register(
      this.widget.onDidChangeAgent((e) => {
        if (e.slashCommand?.isSticky || !e.slashCommand && e.agent.metadata.isSticky) {
        ...
        if (!change.text && this.widget.viewModel) {
          const previousParsedValue = parser.parseChatRequest(
            ...
            widget.location,
            {
              ...
              mode: this.widget.input.currentMode
            }
          );
          const deletableTokens = previousParsedValue.parts.filter(
            (p) => p instanceof ChatRequestAgentPart || p instanceof ChatRequestAgentSubcommandPart || p instanceof ChatRequestSlashCommandPart || p instanceof ChatRequestToolPart
          );
