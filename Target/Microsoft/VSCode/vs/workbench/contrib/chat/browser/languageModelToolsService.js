...
${toErrorMessage(err, true)}`);
  ...
  async prepareToolInvocation(tool, dto, token) {
    const prepared = tool.impl.prepareToolInvocation ? await tool.impl.prepareToolInvocation({
  ...
  toToolAndToolSetEnablementMap(toolOrToolSetNames) {
    ...
    for (const tool of this._tools.values()) {
      result.set(tool.data, tool.data.toolReferenceName !== void 0 && toolOrToolSetNames.has(tool.data.toolReferenceName));
    }
    for (const toolSet of this._toolSets) {
