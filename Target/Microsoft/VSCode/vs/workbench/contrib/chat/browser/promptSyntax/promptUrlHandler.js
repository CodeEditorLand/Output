...
let PromptUrlHandler = class PromptUrlHandler2 extends Disposable {
  ...
  static {
    PromptUrlHandler_1 = this;
  }
  static {
    this.ID = "workbench.contrib.promptUrlHandler";
  }
  static {
    this.CONFIRM_INSTALL_STORAGE_KEY = "security.promptForPromptProtocolHandling";
  }
  constructor(urlService, notificationService, requestService, instantiationService, fileService, openerService, logService, dialogService, storageService) {
    super();
  ...
  async shouldBlockInstall(promptType, url) {
    const location = url.with({ path: url.path.substring(0, url.path.indexOf("/", 1) + 1), query: void 0, fragment: void 0 }).toString();
    const key = PromptUrlHandler_1.CONFIRM_INSTALL_STORAGE_KEY + "-" + location;
    if (this.storageService.getBoolean(key, -1, false)) {
      return false;
    }
...
PromptUrlHandler = PromptUrlHandler_1 = __decorate([
