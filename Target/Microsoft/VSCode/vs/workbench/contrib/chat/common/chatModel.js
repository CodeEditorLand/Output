...
const nonHistoryKinds = /* @__PURE__ */ new Set(["toolInvocation", "toolInvocationSerialized", "undoStop", "prepareToolInvocation"]);
...
class ChatResponseModel extends Disposable {
  ...
  constructor(params) {
    ...
    this.isInProgress = signal.map((_value, r) => {
      signal.read(r);
      return !this.isPendingConfirmation.read(r) && !this.shouldBeRemovedOnSend && !this._isComplete;
    });
...
(function(ChatResponseResource2) {
  ...
  function createUri(sessionId, requestId, toolCallId, index, basename2) {
    return URI.from({
      scheme: ChatResponseResource2.scheme,
      authority: sessionId,
      path: `/tool/${requestId}/${toolCallId}/${index}` + (basename2 ? `/${basename2}` : "")
    });
  }
