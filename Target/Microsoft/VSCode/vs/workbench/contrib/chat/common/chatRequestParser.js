...
import { ChatRequestAgentPart, ChatRequestAgentSubcommandPart, ChatRequestDynamicVariablePart, ChatRequestSlashCommandPart, ChatRequestSlashPromptPart, ChatRequestTextPart, ChatRequestToolPart, chatAgentLeader, chatSubcommandLeader, chatVariableLeader } from "./chatParserTypes.js";
...
let ChatRequestParser = class ChatRequestParser2 {
  ...
  parseChatRequest(sessionId, message, location = ChatAgentLocation.Panel, context) {
    ...
    const toolsByName = new Map(this.variableService.getSelectedTools(sessionId).filter((t) => t.canBeReferencedInPrompt && t.toolReferenceName).map((t) => [t.toolReferenceName, t]));
    let lineNumber = 1;
    let column = 1;
  ...
  tryToParseVariable(message, offset, position, parts, toolsByName) {
