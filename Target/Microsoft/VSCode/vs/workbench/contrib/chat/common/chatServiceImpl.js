...
let ChatService = class extends Disposable {
  constructor(storageService, logService, extensionService, instantiationService, telemetryService, workspaceContextService, chatSlashCommandService, chatVariablesService, chatAgentService, configurationService, experimentService, chatTransferService) {
    super();
  ...
  async activateDefaultAgent(location) {
    ...
    const defaultAgentData = this.chatAgentService.getContributedDefaultAgent(location) ?? this.chatAgentService.getContributedDefaultAgent(
...
ChatService = __decorateClass([
  ...
  __decorateParam(11, IChatTransferService)
], ChatService);
