...
let ChatLifecycleHandler = class ChatLifecycleHandler2 extends Disposable {
  ...
  hasNonCloudSessionInProgress() {
    return this.agentSessionsService.model.sessions.some((session) => isSessionInProgressStatus(session.status) && session.providerType !== AgentSessionProviders.Cloud);
  }
