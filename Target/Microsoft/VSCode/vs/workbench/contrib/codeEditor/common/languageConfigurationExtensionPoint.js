...
let LanguageConfigurationFileHandler = class extends Disposable {
  ...
  static _extractValidOnEnterRules(languageId, configuration) {
    ...
    for (let i = 0, len = source.length; i < len; i++) {
      ...
      const beforeText = LanguageConfigurationFileHandler._parseRegex(
        languageId,
      ...
      if (onEnterRule.afterText) {
        const afterText = LanguageConfigurationFileHandler._parseRegex(
          languageId,
      ...
      if (onEnterRule.previousLineText) {
        const previousLineText = LanguageConfigurationFileHandler._parseRegex(
          languageId,
  ...
  static extractValidConfig(languageId, configuration) {
    const comments = LanguageConfigurationFileHandler._extractValidCommentRule(
      languageId,
      configuration
    );
    const brackets = LanguageConfigurationFileHandler._extractValidBrackets(
      languageId,
      configuration
    );
    const autoClosingPairs = LanguageConfigurationFileHandler._extractValidAutoClosingPairs(
      languageId,
      configuration
    );
    const surroundingPairs = LanguageConfigurationFileHandler._extractValidSurroundingPairs(
      languageId,
      configuration
    );
    const colorizedBracketPairs = LanguageConfigurationFileHandler._extractValidColorizedBracketPairs(
      languageId,
      configuration
    );
    const autoCloseBefore = typeof configuration.autoCloseBefore === "string" ? configuration.autoCloseBefore : void 0;
    const wordPattern = configuration.wordPattern ? LanguageConfigurationFileHandler._parseRegex(
      languageId,
      "wordPattern",
      configuration.wordPattern
    ) : void 0;
    const indentationRules = configuration.indentationRules ? LanguageConfigurationFileHandler._mapIndentationRules(
      languageId,
    ...
    if (configuration.folding) {
      ...
      const startMarker = rawMarkers?.start ? LanguageConfigurationFileHandler._parseRegex(
        languageId,
        "folding.markers.start",
        rawMarkers.start
      ) : void 0;
      const endMarker = rawMarkers?.end ? LanguageConfigurationFileHandler._parseRegex(
        languageId,
        "folding.markers.end",
        rawMarkers.end
      ) : void 0;
    ...
    const onEnterRules = LanguageConfigurationFileHandler._extractValidOnEnterRules(
      languageId,
      configuration
    );
  ...
  static _mapIndentationRules(languageId, indentationRules) {
    const increaseIndentPattern = LanguageConfigurationFileHandler._parseRegex(
      languageId,
    ...
    const decreaseIndentPattern = LanguageConfigurationFileHandler._parseRegex(
      languageId,
    ...
    if (indentationRules.indentNextLinePattern) {
      result.indentNextLinePattern = LanguageConfigurationFileHandler._parseRegex(
        languageId,
    ...
    if (indentationRules.unIndentedLinePattern) {
      result.unIndentedLinePattern = LanguageConfigurationFileHandler._parseRegex(
        languageId,
