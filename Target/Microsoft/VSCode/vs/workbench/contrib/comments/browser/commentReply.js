  ...
  createTextModelListener(commentEditor, commentForm) {
    this._commentThreadDisposables.push(
      commentEditor.onDidFocusEditorWidget(() => {
        const model2 = commentEditor.getModel();
        if (model2) {
    ...
    if (model) {
      this._commentThreadDisposables.push(
        model.onDidChangeContent(() => {
          ...
          if (this._commentThread.input && this._commentThread.input.uri === model.uri && this._commentThread.input.value !== modelContent) {
    ...
    this._commentThreadDisposables.push(
      this._commentThread.onDidChangeInput((input) => {
        ...
        const model2 = commentEditor.getModel();
        if (thread.input && model2 && thread.input.uri !== model2.uri) {
          return;
        }
        if (!input) {
          return;
        ...
        if (commentEditor.getValue() !== input.value) {
          ...
          if (input.value === "") {
            ...
            const domNode = commentEditor.getDomNode();
            if (domNode) {
