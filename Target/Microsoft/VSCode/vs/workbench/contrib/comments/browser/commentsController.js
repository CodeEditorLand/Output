...
function moveToNextCommentInThread(commentInfo, type) {
  ...
  const currentIndex = commentInfo.thread.comments?.indexOf(
    commentInfo.comment
  );
...
let CommentController = class {
  constructor(editor, commentService, instantiationService, codeEditorService, contextMenuService, quickInputService, viewsService, configurationService, contextKeyService, editorService, keybindingService, accessibilityService, notificationService) {
    ...
    this.globalToDispose.add(
      this.commentService.onDidSetResourceCommentInfos(async (e) => {
        const editorURI = this.editor?.hasModel() && this.editor.getModel().uri;
  ...
  onEditorChangeCursorPosition(e) {
    ...
    this._activeCursorHasComment.set(
      this.getCommentsAtLine(range).length > 0
    );
  }
  ...
  async handleCommentAdded(editorId, uniqueOwner, thread) {
    ...
    const pendingCommentText = this._pendingNewCommentCache[uniqueOwner]?.[thread.threadId] ?? continueOnCommentText;
    const pendingEdits = this._pendingEditsCache[uniqueOwner]?.[thread.threadId];
