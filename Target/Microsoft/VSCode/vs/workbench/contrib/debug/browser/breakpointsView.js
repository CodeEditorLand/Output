...
let BreakpointsView = class extends ViewPane {
    ...
    this._register(
      this.list.onMouseMiddleClick(async ({ element }) => {
        if (element instanceof Breakpoint) {
          ...
          await this.debugService.removeDataBreakpoints(
            element.getId()
          );
  ...
  renderHeaderTitle(container, title) {
    ...
    this.hintContainer = this._register(
      new IconLabel(iconLabelContainer, {
        supportIcons: true,
        hoverDelegate: {
          showHover: /* @__PURE__ */ __name((options, focus) => this.hoverService.showInstantHover(
            {
  ...
  onListContextMenu(e) {
    ...
    this.breakpointHasMultipleModes.set(
      this.debugService.getModel().getBreakpointModes(
        getModeKindForBreakpoint(element)
      ).length > 1
    );
  ...
  onStateChange() {
    if (this.isBodyVisible()) {
      ...
      let found = false;
      if (thread?.stoppedDetails?.hitBreakpointIds && thread.stoppedDetails.hitBreakpointIds.length > 0) {
