  ...
  renderBody(container) {
    ...
    this._register(
      this.tree.onDidOpen(async (e) => {
        ...
        if (Array.isArray(element)) {
          element.forEach(
            (sf) => this.dataSource.deemphasizedStackFramesToShow.add(sf)
          );
...
class ShowMoreRenderer {
  ...
  renderElement(element, index, data) {
    ...
    if (stackFrames.every(
      (sf) => !!(sf.source?.origin && sf.source.origin === stackFrames[0].source.origin)
    )) {
...
class CallStackDataSource {
  ...
  deemphasizedStackFramesToShow = /* @__PURE__ */ new WeakSet();
  hasChildren(element) {
    if (isDebugSession(element)) {
