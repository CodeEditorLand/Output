...
let DebugEditorContribution = class {
  ...
  onEditorMouseMove(mouseEvent) {
    ...
    if (target.type === MouseTargetType.CONTENT_WIDGET && target.detail === DebugHoverWidget.ID || this.hoverWidget.isInSafeTriangle(
  ...
  async toggleExceptionWidget() {
    ...
    const exceptionSf = callStack.find(
      (sf) => !!(sf?.source?.available && sf.source.presentationHint !== "deemphasize")
    );
