...
let DebugService = class {
  ...
  async startDebugging(launch, configOrName, options, saveBeforeStart = !options?.parentSession) {
    const message = options?.noDebug ? nls.localize(
      "runTrust",
      ...
      if (activeEditor?.resource) {
        ...
        if (chosen) {
          ...
          if (chosen.dynamicLabel) {
            const dyn = await this.configurationManager.getDynamicConfigurationsByType(
              chosen.type
            );
            const found = dyn.find(
              (d) => d.label === chosen.dynamicLabel
            );
            if (found) {
              launch = found.launch;
            ...
            if (lineNumber >= 1 && lineNumber <= model.getLineCount()) {
              ...
              aria.alert(
                nls.localize(
                  ...
                  lineContent,
                  thread?.stoppedDetails ? `, reason ${thread.stoppedDetails.reason}` : "",
...
function getStackFrameThreadAndSessionToFocus(model, stackFrame, thread, session, avoidSession) {
  ...
  if (!thread) {
    if (stackFrame) {
      ...
      thread = stoppedThread || (threads?.length ? threads[0] : void 0);
    }
  }
  if (!stackFrame && thread) {
