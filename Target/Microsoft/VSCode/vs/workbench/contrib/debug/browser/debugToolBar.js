...
let DebugToolBar = class extends Themable {
    ...
    this.updateScheduler = this._register(
      new RunOnceScheduler(() => {
        ...
        const actions = getFlatActionBarActions(
          this.debugToolBarMenu.getActions({
  ...
  getCurrentYPosition() {
    return Number.parseInt(this.$el.style.getPropertyValue("--y-position"));
  }
  storePosition() {
  ...
  getStoredXPosition() {
    ...
    const storedPercentage = isMainWindow ? Number(
      this.storageService.get(
        DEBUG_TOOLBAR_POSITION_KEY,
        StorageScope.PROFILE
      )
    ) : this.auxWindowCoordinates.get(currentWindow)?.x;
    return storedPercentage !== void 0 && !Number.isNaN(storedPercentage) ? storedPercentage : 0.5;
  }
  getStoredYPosition() {
    const currentWindow = dom.getWindow(this.layoutService.activeContainer);
    const isMainWindow = currentWindow === mainWindow;
