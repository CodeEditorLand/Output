...
import {
  COPY_ADDRESS_ID,
  COPY_ADDRESS_LABEL
} from "../../../../workbench/contrib/debug/browser/debugCommands.js";
...
let DisassemblyView = class extends EditorPane {
  constructor(group, telemetryService, themeService, storageService, _configurationService, _instantiationService, _debugService, _contextMenuService, menuService, contextKeyService) {
    ...
    this.menu = menuService.createMenu(
      MenuId.DebugDisassemblyContext,
      contextKeyService
    );
  ...
  createEditor(parent) {
    ...
    this._register(
      this._disassembledInstructions.onContextMenu(
        ...
        if (bpEvent && this._disassembledInstructions) {
          ...
          bpEvent.changed?.forEach((bp) => {
            if (bp instanceof InstructionBreakpoint) {
              ...
              if (index >= 0) {
                if (this._disassembledInstructions) {
                  if (this._disassembledInstructions.row(
                    index
                  ).isBreakpointEnabled !== bp.enabled) {
                    this._disassembledInstructions.row(
                      index
