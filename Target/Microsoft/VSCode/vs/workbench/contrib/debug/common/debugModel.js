...
class Thread {
  ...
  getTopStackFrame() {
    ...
    const firstAvailableStackFrame = callStack.find(
      (sf) => !!((stopReason === "instruction breakpoint" || stopReason === "step" && this.lastSteppingGranularity === "instruction") && sf.instructionPointerReference || sf.source?.available && (KEEP_SUBTLE_FRAME_AT_TOP_REASONS.includes(
        stopReason
...
class Breakpoint extends BaseBreakpoint {
  ...
  get adapterData() {
    return this.data?.source?.adapterData ? this.data.source.adapterData : this._adapterData;
  }
  get endLineNumber() {
