...
let InstallAction = class extends ExtensionAction {
  ...
  async run() {
    ...
    if (extension?.local) {
      ...
      if (runningExtension && !runningExtension.activationEvents?.some(
...
async function getContextMenuActionsGroups(extension, contextKeyService, instantiationService) {
  return instantiationService.invokeFunction(async (accessor) => {
    ...
    const allowedExtensionsService = accessor.get(
      IAllowedExtensionsService
    );
    const cksOverlay = [];
    if (extension) {
...
let EnableForWorkspaceAction = class extends ExtensionAction {
  ...
  update() {
    this.enabled = false;
    if (this.extension?.local && !this.extension.isWorkspaceScoped) {
...
let DisableForWorkspaceAction = class extends ExtensionAction {
  ...
  update() {
    this.enabled = false;
    if (this.extension?.local && !this.extension.isWorkspaceScoped && this.extensionService.extensions.some(
      (e) => areSameExtensions(
...
let ExtensionRuntimeStateAction = class extends ExtensionAction {
  ...
  update() {
    ...
    if (this.extension.local?.manifest?.contributes?.localizations && this.extension.local.manifest.contributes.localizations.length > 0) {
      return;
    }
...
let AbstractConfigureRecommendedExtensionsAction = class extends Action {
  ...
  getSelectionPosition(content, resource, path) {
    ...
    if (node?.parent?.children) {
      ...
      const lastExtensionNode = recommendationsValueNode.children?.length ? recommendationsValueNode.children[recommendationsValueNode.children.length - 1] : null;
      ...
      [
        new Action(
          ...
          this.extensionsWorkbenchService.isExtensionIgnoredToSync(
            this.extension
          ) ? localize("sync", "Sync this extension") : localize("do not sync", "Do not sync this extension"),
          void 0,
          true,
...
let InstallSpecificVersionOfExtensionAction = class extends Action {
  constructor(id, label, extensionsWorkbenchService, quickInputService, instantiationService, extensionEnablementService) {
    super(id, label);
    this.extensionsWorkbenchService = extensionsWorkbenchService;
    this.quickInputService = quickInputService;
    this.instantiationService = instantiationService;
