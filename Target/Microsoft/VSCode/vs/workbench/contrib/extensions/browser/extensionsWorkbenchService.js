...
let ExtensionsWorkbenchService = class ExtensionsWorkbenchService2 extends Disposable {
  ...
  reportInstalledExtensionsTelemetry() {
    const extensionIds = this.installed.filter((extension) => !extension.isBuiltin && (extension.enablementState === 12 || extension.enablementState === 11)).map((extension) => ExtensionIdentifier.toKey(extension.identifier.id));
  ...
  shouldAutoUpdateExtension(extension) {
    ...
    if (autoUpdateValue === "onlyEnabledExtensions") {
      return extension.enablementState !== 9 && extension.enablementState !== 10;
    }
    return false;
  }
  ...
  async install(arg, installOptions = {}, progressLocation) {
    ...
    if (installOptions.enable) {
      if (extension.enablementState === 10 || extension.enablementState === 9) {
        if (installOptions.justification) {
          const result = await this.dialogService.confirm({
        ...
        await this.setEnablement(
          extension,
          extension.enablementState === 10 ? 12 : 11
  ...
  promptAndSetEnablement(extensions, enablementState) {
    const enable = enablementState === 11 || enablementState === 12;
    if (enable) {
  ...
  getExtensionsRecursively(extensions, installed, enablementState, options, checked = []) {
    ...
    if (toCheck.length) {
      ...
      const extensionsToEanbleOrDisable = installed.filter((i) => {
        ...
        const enable = enablementState === 11 || enablementState === 12;
        const isExtensionEnabled = i.enablementState === 11 || i.enablementState === 12;
