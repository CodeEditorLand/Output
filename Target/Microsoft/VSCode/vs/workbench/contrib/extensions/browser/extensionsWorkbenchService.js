...
let Extension = class {
  ...
  get repository() {
    return this.gallery?.assets.repository ? this.gallery.assets.repository.uri : void 0;
  }
  get licenseUrl() {
    return this.gallery?.assets.license ? this.gallery.assets.license.uri : void 0;
  }
  get supportUrl() {
    return this.gallery?.supportLink ? this.gallery.supportLink : void 0;
  }
  get state() {
...
let Extensions = class extends Disposable {
  ...
  async matchInstalledExtensionsWithGallery(extensions) {
    const toMatch = extensions.filter(
      (e) => e.local && !e.gallery && e.local.source !== "resource"
    );
    if (!toMatch.length) {
      return;
    }
    ...
    for (const extension of extensions) {
      const compatible = galleryExtensions.find(
        (e) => areSameExtensions(e.identifier, extension.identifier)
      );
      if (compatible) {
...
let ExtensionsWorkbenchService = class extends Disposable {
  ...
  updateExtensionsNotificaiton() {
    ...
    if (computedNotificiations.length) {
      for (const dismissedNotification of this.getDismissedNotifications()) {
        if (computedNotificiations.some(
  ...
  computeExtensionsNotifications() {
    ...
    const disallowedExtensions = this.local.filter(
      (e) => e.enablementState === EnablementState.DisabledByAllowlist
    );
    if (disallowedExtensions.length) {
      computedNotificiations.push({
        message: this.configurationService.inspect(
          AllowedExtensionsConfigKey
        ).policy ? nls.localize(
        ...
        key: `disallowedExtensions:${disallowedExtensions.sort(
          (a, b) => a.identifier.id.localeCompare(b.identifier.id)
        ).map((e) => e.identifier.id.toLowerCase()).join("-")}`
      });
    }
    ...
    if (invalidExtensions.length) {
      if (invalidExtensions.some(
        (e) => e.local?.manifest.engines?.vscode && (!isEngineValid(
        ...
        computedNotificiations.push({
          ...
          key: `incompatibleExtensions:${invalidExtensions.sort(
            (a, b) => a.identifier.id.localeCompare(b.identifier.id)
          ).map(
            (e) => `${e.identifier.id.toLowerCase()}@${e.local?.manifest.version}`
          ).join("-")}`
        });
      } else {
        computedNotificiations.push({
          ...
          severity: Severity.Warning,
          extensions: invalidExtensions,
          key: `invalidExtensions:${invalidExtensions.sort(
            (a, b) => a.identifier.id.localeCompare(b.identifier.id)
          ).map(
            (e) => `${e.identifier.id.toLowerCase()}@${e.local?.manifest.version}`
          ).join("-")}`
    ...
    if (deprecatedExtensions.length) {
      computedNotificiations.push({
        ...
        key: `deprecatedExtensions:${deprecatedExtensions.sort(
          (a, b) => a.identifier.id.localeCompare(b.identifier.id)
        ).map((e) => e.identifier.id.toLowerCase()).join("-")}`
      });
    }
  ...
  async updateRunningExtensions(auto = false) {
    ...
    if (toAdd.length || toRemove.length) {
      if (await this.extensionService.stopExtensionHosts(
        nls.localize("restart", "Changing extension enablement"),
        auto
      )) {
    ...
    if (extension.local) {
      ...
      if (runningExtension) {
        ...
        if (otherServer && extension.enablementState === EnablementState.DisabledByExtensionKind) {
          ...
          if (extensionInOtherServer?.local && this.extensionEnablementService.isEnabled(
  ...
  getPrimaryExtension(extensions) {
    ...
    const manifest = extensionsToChoose.find((e) => e.local?.manifest)?.local?.manifest;
    if (!manifest) {
  ...
  async checkForUpdates(reason, onlyBuiltin) {
    if (reason) {
      this.logService.info(
        `[Extensions]: Checking for updates. Reason: ${reason}`
      );
    } else {
    ...
    if (infos.length) {
      ...
      this.logService.trace(
        "Checking updates for extensions",
        infos.map((e) => e.id).join(", ")
      );
      const galleryExtensions = await this.galleryService.getExtensions(
        infos,
        {
  ...
  async downloadVSIX(extensionId, preRelease) {
    ...
    const options = [];
    for (const targetPlatform2 of galleryExtension.allTargetPlatforms) {
    ...
    if (options.length) {
      ...
      const option = await this.quickInputService.pick(
    ...
    if (targetPlatform !== galleryExtension.properties.targetPlatform) {
      [galleryExtension] = await this.galleryService.getExtensions(
        [{ id: extensionId, preRelease }],
        { compatible: true, targetPlatform },
        CancellationToken.None
      );
    }
  ...
  async syncInstalledExtensionsWithGallery(gallery) {
    ...
    if (this.outdated.length) {
      this.logService.info(
        ...
        this.outdated.map((e) => e.identifier.id).join(", ")
      );
      this.eventuallyAutoUpdateExtensions();
    }
  }
  ...
  async autoUpdateExtensions() {
    ...
    for (const extension of this.outdated) {
      if (!this.shouldAutoUpdateExtension(extension)) {
        this.logService.info(
          "Auto update disabled for extension",
      ...
      if (await this.shouldRequireConsentToUpdate(extension)) {
        this.logService.info(
          "Auto update consent required for extension",
          extension.identifier.id
        );
        continue;
      }
      ...
      if (isString(extensionOrPublisher)) {
        ...
        if (this.isAutoUpdateEnabledFor(extensionOrPublisher) !== enable) {
          if (enable) {
            ...
            if (enabledAutoUpdateExtensions.includes(
  ...
  async canInstall(extension) {
    ...
    if (extension.gallery) {
      if (!extension.gallery.isSigned && (await this.extensionGalleryManifestService.getExtensionGalleryManifest())?.capabilities.signing?.allRepositorySigned) {
        return new MarkdownString().appendText(
          nls.localize("not signed", "This extension is not signed.")
        );
      }
      ...
      return localResult ?? remoteResult ?? webResult ?? new MarkdownString().appendText(
        nls.localize(
          "cannot be installed",
    ...
    if (arg instanceof URI) {
      ...
      if (gallery) {
        if (installOptions.installEverywhere) {
          servers = [];
          const installableServers = await this.extensionManagementService.getInstallableServers(
            gallery
          );
          for (const extensionsServer of this.extensionsServers) {
            if (installableServers.includes(
              extensionsServer.server
            ) && !extensionsServer.local.find(
              (e) => areSameExtensions(
                e.identifier,
                gallery.identifier
              )
          ...
          if (extension.enablementState === EnablementState.DisabledByExtensionKind) {
            const [installableServer] = await this.extensionManagementService.getInstallableServers(
              gallery
            );
        ...
        if (!installable) {
          if (!gallery) {
            ...
            const reportIssueUri = manifest ? getExtensionGalleryManifestResourceUri(
              manifest,
              ExtensionGalleryResourceType.ReportIssueUri
            ) : void 0;
            const reportIssueMessage = reportIssueUri ? nls.localize(
              "report issue",
  ...
  async doSetEnablement(extensions, enablementState) {
    return await this.extensionEnablementService.setEnablement(
...
ExtensionsWorkbenchService = __decorateClass([
  ...
  __decorateParam(25, IStorageService),
  __decorateParam(26, IDialogService),
  __decorateParam(27, IUserDataSyncEnablementService),
  __decorateParam(28, IUpdateService),
  __decorateParam(29, IUriIdentityService),
  __decorateParam(30, IWorkspaceContextService),
  __decorateParam(31, IViewsService),
  __decorateParam(32, IFileDialogService),
  __decorateParam(33, IQuickInputService),
  __decorateParam(34, IAllowedExtensionsService)
