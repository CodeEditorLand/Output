...
class InlineChatRunOptions {
  ...
  static isInlineChatRunOptions(options) {
    const { initialSelection, initialRange, message, autoSend, position, existingSession, attachments } = options;
    if (typeof message !== "undefined" && typeof message !== "string" || typeof autoSend !== "undefined" && typeof autoSend !== "boolean" || typeof initialRange !== "undefined" && !Range.isIRange(initialRange) || typeof initialSelection !== "undefined" && !Selection.isISelection(initialSelection) || typeof position !== "undefined" && !Position.isIPosition(position) || typeof existingSession !== "undefined" && !(existingSession instanceof Session) || typeof attachments !== "undefined" && (!Array.isArray(attachments) || !attachments.every((item) => item instanceof URI))) {
      return false;
    }
    return true;
  }
...
let InlineChatController = class InlineChatController2 {
  ...
  constructor(editor, configurationService) {
    const inlineChat2 = observableConfigValue("inlineChat.enableV2", false, configurationService);
    this._delegate = derived((r) => {
      if (inlineChat2.read(r)) {
        return InlineChatController22.get(editor);
      } else {
...
let InlineChatController1 = InlineChatController1_1 = class InlineChatController12 {
  static {
  ...
  dispose() {
    if (this._currentRun) {
      this._messages.fire(
        this._session?.chatModel.hasRequests ? 4 : 2
  ...
  async [
    ...
    const origDeco = this._editor.getOption(
      103
  ...
  _showWidget(headless = false, initialRender = false, position) {
    ...
    if (initialRender && this._editor.getOption(
      120
...
let InlineChatController22 = class InlineChatController23 {
  static {
    __name(this, "InlineChatController2");
  }
  static {
    InlineChatController2_1 = this;
  }
  static {
    this.ID = "editor.contrib.inlineChatController2";
  }
  static get(editor) {
    return editor.getContribution(InlineChatController2_1.ID) ?? void 0;
  }
  get widget() {
...
InlineChatController22 = InlineChatController2_1 = __decorate([
  __param(1, IInstantiationService),
...
export {
  InlineChatController,
  InlineChatController1,
  InlineChatController22 as InlineChatController2,
