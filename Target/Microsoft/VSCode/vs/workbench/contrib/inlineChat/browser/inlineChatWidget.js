...
let InlineChatWidget = class {
  constructor(location, _options, _instantiationService, _contextKeyService, _keybindingService, _accessibilityService, _configurationService, _accessibleViewService, _textModelResolverService, _chatService, _hoverService) {
      ...
      void 0,
      {
        ...
        filter: /* @__PURE__ */ __name((item) => {
          ...
          if (item.response.value.every(
            (item2) => item2.kind === "textEditGroup" && _options.chatWidgetViewOptions?.rendererOptions?.renderTextEditsAsSummary?.(
              item2.uri
            )
          )) {
            return false;
    ...
    const ctxResponseSupportIssues = ChatContextKeys.responseSupportsIssueReporting.bindTo(
      this.scopedContextKeyService
    );
    const ctxResponseError = ChatContextKeys.responseHasError.bindTo(
      this.scopedContextKeyService
    );
    const ctxResponseErrorFiltered = ChatContextKeys.responseIsFiltered.bindTo(
      this.scopedContextKeyService
    );
  ...
  selectAll() {
    this._chatWidget.inputEditor.setSelection(
      new Selection(1, 1, Number.MAX_SAFE_INTEGER, 1)
    );
  }
  ...
  updateStatus(message, ops = {}) {
    ...
    if (isTempMessage) {
      ...
      this._elements.statusLabel.dataset["state"] = void 0;
    }
    if (ops.title) {
      this._elements.statusLabel.dataset["title"] = ops.title;
    } else {
      this._elements.statusLabel.dataset["title"] = void 0;
    }
    this._onDidChangeHeight.fire();
  }
  reset() {
