...
let BaseIssueReporterService = class extends Disposable {
  constructor(disableExtensions, data, os, product, window, isWeb, issueFormService, themeService, fileService, fileDialogService) {
    ...
    if (issueReporterElement) {
      this.previewButton = this._register(
        new Button(issueReporterElement, unthemedButtonStyles)
      );
  ...
  updateAcknowledgementState() {
    const acknowledgementCheckbox = this.getElementById(
  ...
  async renderBlocks() {
    ...
    if (extensionData && extensionData.length > MAX_EXTENSION_DATA_LENGTH) {
      ...
      downloadExtensionDataLink.addEventListener(
        "click",
        handleLinkClick
      );
      this._register({
        dispose: /* @__PURE__ */ __name(() => downloadExtensionDataLink.removeEventListener(
          "click",
          handleLinkClick
        ), "dispose")
      });
