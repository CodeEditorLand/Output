...
let McpAddConfigurationCommand = class McpAddConfigurationCommand2 {
  ...
  async getConfigurationTarget() {
    const options = [
      { target: 2, label: localize("mcp.target.user", "User Settings"), description: localize("mcp.target.user.description", "Available in all workspaces") }
    ];
    if (!!this._environmentService.remoteAuthority) {
      options.push({ target: 4, label: localize("mcp.target.remote", "Remote Settings"), description: localize("mcp.target..remote.description", "Available on this remote machine") });
    }
    if (this._workspaceService.getWorkspace().folders.length > 0) {
      options.push({ target: 5, label: localize("mcp.target.workspace", "Workspace Settings"), description: localize("mcp.target.workspace.description", "Available in this workspace") });
    }
    if (options.length === 1) {
  ...
  async pickForUrlHandler(resource, showIsPrimary = false) {
    ...
    const getEditors = /* @__PURE__ */ __name(() => this._editorService.getEditors(
      0
      /* EditorsOrder.MOST_RECENTLY_ACTIVE */
    ).filter((e) => e.editor.resource?.toString() === resource.toString()), "getEditors");
    switch (pick?.id) {
      case "show":
