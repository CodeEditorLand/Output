...
let ConfigMcpDiscovery = class extends Disposable {
  ...
  start() {
    ...
    const addPath = /* @__PURE__ */ __name((path) => {
      this.configSources.push({
        ...
        getServerToLocationMapping: /* @__PURE__ */ __name((uri) => this._getServerIdMapping(
          uri,
          path.section ? [...path.section, "servers"] : ["servers"]
        ), "getServerToLocationMapping")
      });
    }, "addPath");
    this._register(
  ...
  async sync() {
    const configurationKey = this._configurationService.inspect(
      mcpConfigurationSection
    );
    ...
    for (const [index, src] of this.configSources.entries()) {
      ...
      let value = src.path.workspaceFolder ? this._configurationService.inspect(
        mcpConfigurationSection,
        { resource: src.path.workspaceFolder.uri }
      )[src.path.key] : configurationKey[src.path.key];
      if (value?.mcpServers) {
        value = {
          ...value,
