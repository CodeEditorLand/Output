...
let McpSamplingService = class McpSamplingService2 extends Disposable {
  ...
  async sample(opts, token = CancellationToken.None) {
    const messages = opts.params.messages.map((message) => {
      const content = message.content.type === "text" ? { type: "text", value: message.content.text } : message.content.type === "image" || message.content.type === "audio" ? { type: "image_url", value: { mimeType: message.content.mimeType, data: decodeBase64(message.content.data) } } : void 0;
      if (!content) {
        return void 0;
      }
      return {
  ...
  _getConfig(server) {
    ...
    return { value: void 0, mapping: void 0, key, target: leastSpecificConfig, resource };
  }
