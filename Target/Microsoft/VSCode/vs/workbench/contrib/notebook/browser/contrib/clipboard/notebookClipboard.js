...
function runCutCells(accessor, editor, targetCell) {
  ...
  clipboardService.writeText(selectedCells.map((cell) => cell.getText()).join("\n"));
  const edits = selectionRanges.map((range) => ({ editType: 1, index: range.start, count: range.end - range.start, cells: [] }));
...
let NotebookClipboardContribution = class NotebookClipboardContribution2 extends Disposable {
  static {
