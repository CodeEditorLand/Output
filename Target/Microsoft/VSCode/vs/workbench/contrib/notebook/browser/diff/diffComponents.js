...
class AbstractElementRenderer extends Disposable {
  ...
  _showOutputsRaw() {
    if (this._outputEditorContainer) {
      ...
      this.cell.rawOutputHeight = this._outputEditor?.getContentHeight() ?? 0;
    }
  }
  ...
  _applySanitizedMetadataChanges(currentMetadata, newMetadata) {
    ...
    try {
      ...
      const modifiedTextModel = this.cell.modified?.textModel;
      if (!modifiedTextModel) {
        return;
      }
      const index = this.notebookEditor.textModel?.cells.indexOf(modifiedTextModel);
  ...
  _buildOutputEditor() {
    ...
    if ((this.cell.type === "modified" || this.cell.type === "unchanged") && !this.notebookEditor.textModel?.transientOptions.transientOutputs) {
      ...
      if (originalOutputsSource !== modifiedOutputsSource) {
        ...
        const disposable = this.cell.modified?.textModel.onDidChangeOutputs(() => {
        ...
        if (disposable) {
          this._outputEditorDisposeStore.add(disposable);
        }
        return;
      }
    }
  ...
  dispose() {
    if (this._outputEditor && this._outputEditor.saveViewState()) {
  ...
  async _initializeSourceDiffEditor() {
    ...
    if (this._editor) {
      ...
      if (originalEditor) {
        ...
        this._register(
          originalEditor.onDidScrollChange(handleScrollChange)
        );
      }
      if (modifiedEditor) {
        this._register(
        ...
        this._register(
          modifiedEditor.onDidScrollChange(handleScrollChange)
        );
      }
    }
    const editorViewState = this.cell.getSourceEditorViewState();
    ...
    const contentHeight = this._editor?.getContentHeight();
    this.cell.editorHeight = contentHeight ?? 0;
  }
  updateEditorOptionsForWhitespace() {
    const editor = this._editor;
    if (!editor) {
