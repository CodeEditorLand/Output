...
let NotebookInlineDiffDecorationContribution = class extends Disposable {
  constructor(notebookEditor, notebookEditorWorkerService, instantiationService, logService) {
    ...
    this._register(
      autorun((reader) => {
        this.previous = this.notebookEditor.notebookOptions.previousModelToCompare.read(
          reader
        );
        if (this.previous) {
  ...
  clear() {
    ...
    this.logService.debug(
      "inlineDiff",
      "Cleared decorations and listeners"
    );
  }
  dispose() {
  ...
  initialize() {
    ...
    this.insertedCellDecorator = this.instantiationService.createInstance(
      NotebookInsertedCellDecorator,
      this.notebookEditor
    );
    this.deletedCellDecorator = this.instantiationService.createInstance(
    ...
    this.listeners.push(
      this.notebookEditor.onDidChangeModel(() => this._update())
    );
  ...
  async _update() {
    ...
    if (!this.previous || !current) {
      this.logService.debug(
        "inlineDiff",
        "Update skipped - no original or current document"
      );
      return;
    }
...
${e}`
      ...
      this.deletedCellDecorator?.apply(
        diffInfo.cellDiffInfo,
        this.previous
      );
    }
    await this.updateCells(
      this.previous,
      current,
      this.cachedNotebookDiff.cellDiffInfo
    );
  }
