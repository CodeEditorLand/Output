...
let NotebookAccessibilityProvider = class extends Disposable {
      ...
      Event.debounce(
        ...
        if (viewModel2) {
          for (const update of updates) {
            const cellModel = viewModel2.getCellByHandle(
              update.cellHandle
            );
            if (cellModel) {
          ...
          if (this.shouldReadCellOutputs(lastUpdate.state)) {
            const cell = viewModel2.getCellByHandle(
              lastUpdate.cellHandle
            );
            if (cell?.outputsViewModels.length) {
              const text = getAllOutputsText(
                viewModel2.notebookDocument,
                cell,
                true
              );
              alert(text);
  ...
  shouldReadCellOutputs(state) {
    return state === void 0 && // execution completed
    this.isReplHistory && this.accessibilityService.isScreenReaderOptimized() && this.configurationService.getValue(
