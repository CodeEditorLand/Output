...
let NotebookEditorWidget = class extends Disposable {
  ...
  getSelectionViewModels() {
    ...
    return this.viewModel.getSelections().map(
      (range) => (
        // Ensure `b` in reduce is always an array
      ...
      cell.onCellDecorationsChanged((e) => {
        e.added.forEach((options) => {
          if (options.className) {
            this.deltaCellContainerClassNames(
              cell.id,
              [options.className],
              [],
              cell.cellKind
    ...
    if (this._list) {
      ...
      if (this.viewModel) {
        ...
        if (element) {
          ...
          const editorFocused = element.getEditState() === CellEditState.Editing && !!(itemDOM?.ownerDocument.activeElement && itemDOM.contains(
  ...
  hasOutputTextSelection() {
    ...
    while (container && container !== this._body) {
      if (container.classList?.contains("output")) {
        return true;
      }
  ...
  deltaCellContainerClassNames(cellId, added, removed, cellkind) {
    if (cellkind === CellKind.Markup) {
      ...
      this._webview?.deltaCellOutputContainerClassNames(
        cellId,
        added,
        removed
      );
  ...
  _layoutDisposables = /* @__PURE__ */ new Set();
  ...
  async focusNotebookCell(cell, focusItem, options) {
    ...
    if (focusItem === "editor") {
      ...
      if (itemDOM?.ownerDocument.activeElement && itemDOM.contains(itemDOM.ownerDocument.activeElement)) {
  ...
  cellIsHidden(cell) {
    ...
    return foldedRanges?.some(
      (range) => modelIndex !== void 0 && modelIndex >= range.start && modelIndex <= range.end
    ) ?? false;
  }
