...
function matchCellBasedOnSimilarties(modifiedCells, originalCells) {
  ...
  for (let i = 0; i < modifiedCells.length; i++) {
    ...
    if (index >= 0 && dist === 0) {
      ...
      originalIndexWithMostEdits.set(index, {
        dist,
        modifiedIndex: i
      });
      results.push({
  ...
  results.forEach((result, i) => {
    ...
    const modifiedCell = modifiedCells[i];
    if (result.original === -1 && result.possibleOriginal >= 0 && !unavailableIndexes.has(result.possibleOriginal) && canOriginalIndexBeMappedToModifiedIndex(result.possibleOriginal, {
      ...
      (originalIndex, originalValue) => {
        ...
        if (nextMatchedModifiedIndex > 0 || previousMatchedOriginalIndex > 0) {
          ...
          if (matchesForThisOriginalIndex && previousMatchedOriginalIndex < originalIndex) {
            const betterMatch = Array.from(
              matchesForThisOriginalIndex
            ).find(([modifiedIndex, value]) => {
              if (modifiedIndex === i) {
                return false;
              }
...
function computeClosestCell({ cell, index: cellIndex }, arr, ignoreEmptyCells, cache, canOriginalIndexBeMappedToModifiedIndex) {
  let min_edits = Number.POSITIVE_INFINITY;
  let min_index = -1;
  const internalId = cell.internalMetadata?.internalId;
  if (internalId) {
    const internalIdIndex = arr.findIndex(
      (cell2) => cell2.internalMetadata?.internalId === internalId
    );
