...
let KeybindingsEditor = class extends EditorPane {
  ...
  setInput(input, options, context, token) {
    ...
    return super.setInput(input, options, context, token).then(() => this.render(!!options?.preserveFocus));
  }
  clearInput() {
    super.clearInput();
  ...
  createOverlayContainer(parent) {
    ...
    this._register(
      this.defineKeybindingWidget.onDidChange((keybindingStr) => {
        if (this.keybindingsEditorModel !== null) {
    ...
    templateData.disposables.add(
      this.keybindingsEditor.onDefineWhenExpression((e) => {
        if (keybindingItemEntry === e) {
          ...
          if (templateData.element.parentElement) {
            inputWidget.layout(
              new DOM.Dimension(
