...
let SettingsEditor2 = class extends EditorPane {
    ...
    this._register(
      workspaceTrustManagementService.onDidChangeTrust(() => {
        ...
        if (this.settingsTreeModel.value) {
          this.settingsTreeModel.value.updateWorkspaceTrust(
            workspaceTrustManagementService.isWorkspaceTrusted()
          );
          this.renderTree();
        }
  ...
  settingRenderers;
  tocTreeModel;
  settingsTreeModel = this._register(
  ...
  viewState;
  _searchResultModel = this._register(
    new MutableDisposable()
  );
  ...
  removeLanguageFilters() {
    if (this.searchWidget?.getValue().includes(`@${LANGUAGE_SETTING_TAG}`)) {
  ...
  createSettingsTree(container) {
    ...
    this._register(
      this.settingsTree.onDidFocus(() => {
        ...
        if (classList?.contains("monaco-list") && classList.contains("settings-editor-tree")) {
  ...
  updateElementsByKey(keys) {
    if (keys.size) {
      ...
      if (this.settingsTreeModel.value) {
        keys.forEach(
          (key) => this.settingsTreeModel.value?.updateElementsByName(key)
        );
      }
  ...
  updateModifiedLabelForKey(key) {
    ...
    const isModified = dataElements?.[0]?.isConfigured;
  ...
  async searchWithProvider(type, searchProvider, token) {
    const result = await this._searchPreferencesModel(
