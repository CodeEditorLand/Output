...
class SettingsTreeElement extends Disposable {
  ...
  parent;
  _tabbable = false;
  _onDidChangeTabbable = this._register(
    new Emitter()
  );
...
let SettingsTreeModel = class {
  ...
  createSettingsTreeGroupElement(tocEntry, parent) {
    ...
    if (tocEntry.settings) {
      const settingChildren = tocEntry.settings.map(
        (s) => this.createSettingsTreeSettingElement(s, element)
      );
  ...
  sortResults(filterMatches) {
    ...
    filterMatches.sort((a, b) => {
      if (a.matchType !== b.matchType) {
        ...
        return b.keyMatchScore - a.keyMatchScore || compareTwoNullableNumbers(
          a.setting.internalOrder,
          b.setting.internalOrder
        );
