...
let QuickDiffModel = class QuickDiffModel2 extends Disposable {
  ...
  getQuickDiffResults() {
    return this._quickDiffs.map((quickDiff) => {
      const changes = this.changes.filter((change) => change.providerId === quickDiff.id);
      return {
        ...
        if (diff.changes && diff.changes2 && diff.changes.length === diff.changes2.length) {
          for (let index = 0; index < diff.changes.length; index++) {
            ...
            if (quickDiffPrimary && quickDiff.kind === "secondary") {
              const primaryQuickDiffChange = allDiffs.find((d) => d.change2.modified.equals(change2.modified) && d.change2.original.length === change2.original.length);
  ...
  findNextClosestChange(lineNumber, inclusive = true, provider) {
    const visibleQuickDiffLabels = this.quickDiffs.filter((quickDiff) => (!provider || quickDiff.label === provider) && this.quickDiffService.isQuickDiffProviderVisible(quickDiff.id)).map((quickDiff) => quickDiff.label);
    if (!inclusive) {
      const nextChange = this.changes.findIndex((change) => visibleQuickDiffLabels.includes(change.label) && change.change.modifiedStartLineNumber > lineNumber);
    ...
    const inclusiveChangeIndex = this.changes.findIndex((change) => visibleQuickDiffLabels.includes(change.label) && change.change.modifiedStartLineNumber <= lineNumber && getModifiedEndLineNumber(change.change) >= lineNumber);
