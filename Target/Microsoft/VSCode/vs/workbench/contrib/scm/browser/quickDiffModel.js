...
const IQuickDiffModelService = createDecorator(
...
let QuickDiffModel = class extends Disposable {
  constructor(textFileModel, options, scmService, quickDiffService, editorWorkerService, configurationService, textModelResolverService, _chatEditingService, progressService) {
    ...
    this._register(
      autorunWithStore((r, store) => {
        for (const session of this._chatEditingService.editingSessionsObs.read(
          r
        )) {
          store.add(
            autorun((r2) => {
  ...
  _originalEditorModelsDisposables = this._register(
    new DisposableStore()
  );
  ...
  getQuickDiffResults() {
    return this._quickDiffs.map((quickDiff) => {
      const changes = this.changes.filter(
        (change) => change.label === quickDiff.label
      );
      return {
        ...
        return (await Promise.all(
          quickDiffs.map(async (quickDiff) => {
            try {
              ...
              this._originalEditorModels.set(
                quickDiff.originalResource,
                ref.object
              );
  ...
  async getOriginalResource() {
    ...
    const isBeingModifiedByChatEdits = this._chatEditingService.editingSessionsObs.get().some(
      (session) => session.getEntry(uri)?.state.get() === ModifiedFileEntryState.Modified
    );
  ...
  findNextClosestChange(lineNumber, inclusive = true, provider) {
    ...
    for (let i = 0; i < this.changes.length; i++) {
      ...
      if (!this.quickDiffs.find(
        (quickDiff) => quickDiff.label === this.changes[i].label
      )?.visible) {
        continue;
      }
