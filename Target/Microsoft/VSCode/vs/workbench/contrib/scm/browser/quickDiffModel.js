...
let QuickDiffModel = class QuickDiffModel2 extends Disposable {
  ...
  findNextClosestChange(lineNumber, inclusive = true, providerId) {
    const visibleQuickDiffIds = this.quickDiffs.filter((quickDiff) => (!providerId || quickDiff.id === providerId) && this.quickDiffService.isQuickDiffProviderVisible(quickDiff.id)).map((quickDiff) => quickDiff.id);
    if (!inclusive) {
      let nextChangeIndex2 = this.changes.findIndex((change) => visibleQuickDiffIds.includes(change.providerId) && change.change.modifiedStartLineNumber > lineNumber);
      if (nextChangeIndex2 !== -1) {
        return nextChangeIndex2;
      }
      nextChangeIndex2 = this.changes.findIndex((change) => visibleQuickDiffIds.includes(change.providerId));
      return nextChangeIndex2 !== -1 ? nextChangeIndex2 : 0;
    }
    const primaryQuickDiffId = this.quickDiffs.find((quickDiff) => quickDiff.kind === "primary")?.id;
    const primaryInclusiveChangeIndex = this.changes.findIndex((change) => change.providerId === primaryQuickDiffId && change.change.modifiedStartLineNumber <= lineNumber && getModifiedEndLineNumber(change.change) >= lineNumber);
    if (primaryInclusiveChangeIndex !== -1) {
      return primaryInclusiveChangeIndex;
    }
    let nextChangeIndex = this.changes.findIndex((change) => visibleQuickDiffIds.includes(change.providerId) && change.change.modifiedStartLineNumber <= lineNumber && getModifiedEndLineNumber(change.change) >= lineNumber);
    if (nextChangeIndex !== -1) {
      return nextChangeIndex;
    }
    nextChangeIndex = this.changes.findIndex((change) => visibleQuickDiffIds.includes(change.providerId));
  ...
  findPreviousClosestChange(lineNumber, inclusive = true, providerId) {
    for (let i = this.changes.length - 1; i >= 0; i--) {
      ...
      const quickDiff = this.quickDiffs.find((quickDiff2) => quickDiff2.id === this.changes[i].providerId);
