...
let SearchView = class SearchView2 extends ViewPane {
  static {
  ...
  constructor(options, fileService, editorService, codeEditorService, progressService, notificationService, dialogService, commandService, contextViewService, instantiationService, viewDescriptorService, configurationService, contextService, searchViewModelWorkbenchService, contextKeyService, replaceService, textFileService, preferencesService, themeService, searchHistoryService, contextMenuService, accessibilityService, keybindingService, storageService, searchService, openerService, hoverService, notebookService, logService, accessibilitySignalService, telemetryService) {
    ...
    this._register(this.storageService.onDidChangeValue(1, SearchHistoryService.SEARCH_HISTORY_KEY, this._store)(() => {
  ...
  async onSearchComplete(progressComplete, excludePatternText, includePatternText, completed, shouldDoFinalRefresh = true) {
    ...
    if (this.shouldShowAIResults() && !allResults) {
      ...
      if (aiName) {
        const searchWithAIButtonTooltip = appendKeyBindingLabel(nls.localize("triggerAISearch.tooltip", "Search with {0}", aiName), this.keybindingService.lookupKeybinding(
  ...
  updateSearchResultCount(disregardExcludesAndIgnores, onlyOpenEditors, clear = false) {
    ...
    if (fileCount > 0) {
      ...
      const openInEditorTooltip = appendKeyBindingLabel(nls.localize("openInEditor.tooltip", "Copy current search results to an editor"), this.keybindingService.lookupKeybinding(
  ...
  handleKeywordClick(keyword, index, maxKeywords) {
  ...
  updateKeywordSuggestion(keywords) {
    const messageEl = this.clearMessage();
    ...
    topKeywords.forEach((keyword, index) => {
      ...
      const button = this.messageDisposables.add(new SearchLinkButton(keyword.keyword, () => this.handleKeywordClick(keyword.keyword, index, topKeywords.length), this.hoverService));
  ...
  showSearchWithoutFolderMessage() {
    ...
    const textEl = dom.append(this.searchWithoutFolderMessageElement, $("p", void 0, nls.localize("searchWithoutFolder", "You have not opened or specified a folder. Only open files are currently searched - ")));
  ...
  onFilesChanged(e) {
    ...
    if (e.gotDeleted()) {
      const deletedMatches = matches.filter((m) => e.contains(
        m.resource,
        2
...
let SearchViewDataSource = class SearchViewDataSource2 {
  static {
...
let RefreshTreeController = class RefreshTreeController2 extends Disposable {
  static {
  ...
  async refreshTree(event) {
    ...
    if (!event || event.added || event.removed) {
      if (searchConfig.sortOrder === "modified") {
      ...
      if (searchConfig.sortOrder === "countAscending" || searchConfig.sortOrder === "countDescending") {
        await this.searchView.getControl().updateChildren(void 0);
      } else {
