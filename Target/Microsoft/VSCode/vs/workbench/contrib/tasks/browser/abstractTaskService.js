...
let AbstractTaskService = class extends Disposable {
  ...
  async _activateTaskProviders(type) {
    ...
    const hasLoggedActivation = this._activatedTaskProviders.has(
      type ?? "all"
    );
  ...
  _getTaskSystemInfo(key) {
    const infos = this._taskSystemInfos.get(key);
    return infos?.length ? infos[0] : void 0;
  }
  ...
  async getTask(folder, identifier, compareId = false, type = void 0) {
    ...
    const key = !Types.isString(
      identifier
    ) ? TaskDefinition.createTaskIdentifier(identifier, console) : identifier;
    if (key === void 0) {
  ...
  async run(task, options, runSource = TaskRunSource.System) {
    ...
    let executeTaskResult;
    try {
      if (options?.attachProblemMatcher && this._shouldAttachProblemMatcher(task) && !InMemoryTask.is(task)) {
  ...
  _getResourceForKind(kind) {
    ...
    switch (kind) {
      ...
      case TaskSourceKind.WorkspaceFile: {
        if (this._workspace?.configuration) {
  ...
  async _handleExecuteResult(executeResult, runSource) {
    ...
    if (executeResult.kind === TaskExecuteKind.Active) {
      ...
      if (active?.same && runSource === TaskRunSource.FolderOpen || runSource === TaskRunSource.Reconnect) {
...
`);
  ...
  _getLegacyTaskConfigurations(workspaceTasks) {
    ...
    for (const task of workspaceTasks.tasks) {
      if (CustomTask.is(task)) {
        const commandName = task.command?.name;
...
`);
  ...
  async _runTaskCommand(filter) {
    ...
    const exactMatchTask = !taskName ? void 0 : tasks.find(
    ...
    if (this.schemaVersion === JsonSchemaVersion.V2_0_0) {
      this.tasks().then((tasks) => {
        ...
        for (const task of tasks) {
          ...
          if (taskGroup?.isDefault && taskGroup._id === TaskGroup.Test._id) {
            selectedTask = task;
            break;
          }
        }
