...
let TestingExplorerView = class extends ViewPane {
  ...
  getTreeIncludeExclude(profileOrBitset, withinItems, filterToType = "visible") {
    ...
    const isRunnableWithProfileOrBitset = /* @__PURE__ */ __name((item) => {
      ...
      if (value === void 0) {
        value = typeof profileOrBitset === "number" ? !!this.testProfileService.getDefaultProfileForTest(
          profileOrBitset,
          item
    ...
    const attempt = /* @__PURE__ */ __name((element, alreadyIncluded) => {
      ...
      if (
        ...
        isRunnableWithProfileOrBitset(element.test) && // And either it's a leaf node or most children are included, the  include it.
        (visibleRunnableChildren === 0 || visibleRunnableChildren * 2 >= inTree.children.length) && // And not if we're only showing a single of its children, since it
  ...
  getContinuousRunDropdown(defaultAction, options) {
    const allProfiles = [
      ...Iterable.flatMap(
        ...
        (cr) => {
          if (this.testService.collection.getNodeById(
            cr.controller.id
          )?.children.size) {
            return Iterable.filter(
              cr.profiles,
              (p) => p.supportsContinuousRun
            );
          }
...
let TestingExplorerViewModel = class extends Disposable {
  ...
  tryPeekError(item) {
    ...
    return lookup?.[1].tasks.some((s) => isFailedState(s.state)) ? this.peekOpener.tryPeekFirstError(lookup[0], lookup[1], {
      preserveFocus: true
    }) : false;
  }
  onContextMenu(evt) {
...
const getActionableElementActions = /* @__PURE__ */ __name((contextKeyService, menuService, testService, crService, profiles, element) => {
  ...
  if (test) {
    ...
    const supportsCr = !!ctrl && profiles.getControllerProfiles(ctrl.id).some(
      (p) => p.supportsContinuousRun && canUseProfileWithTest(p, test)
    );
    contextKeys.push(
      [
