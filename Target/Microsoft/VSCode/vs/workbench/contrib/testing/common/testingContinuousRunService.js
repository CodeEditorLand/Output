...
let TestingContinuousRunService = class extends Disposable {
  ...
  isEnabledForProfile({
    ...
    for (const node of this.running.values()) {
      if (node.profiles.get().some(
        (p) => p.profileId === profileId && p.controllerId === controllerId
      )) {
        return true;
      }
    }
  ...
  stop(testId) {
    const cancellations = [
      ...this.running.deleteRecursive(
        testId ? TestId.fromString(testId).path : []
      )
    ];
