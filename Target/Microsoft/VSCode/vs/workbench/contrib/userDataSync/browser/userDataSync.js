...
let UserDataSyncWorkbenchContribution = class extends Disposable {
      ...
      actions: {
        primary: [
          toAction({
            ...
            run: /* @__PURE__ */ __name(() => resource === SyncResource.Settings ? this.preferencesService.openUserSettings({
              jsonEditor: true
      ...
      actions: {
        primary: [
          toAction({
            ...
            run: /* @__PURE__ */ __name(() => source === SyncResource.Settings ? this.preferencesService.openUserSettings({
              jsonEditor: true
            }) : this.preferencesService.openGlobalKeybindingSettings(
              true
            ), "run")
  ...
  async askToConfigure() {
    return new Promise((c, e) => {
      ...
      quickPick.selectedItems = items.filter(
        (item) => this.userDataSyncEnablementService.isResourceEnabled(
          item.id,
          true
        )
      );
  ...
  registerManageSyncAction() {
    ...
    const when = ContextKeyExpr.and(
      CONTEXT_SYNC_ENABLEMENT,
      CONTEXT_ACCOUNT_STATE.notEqualsTo(AccountStatus.Unavailable),
      CONTEXT_SYNC_STATE.notEqualsTo(SyncStatus.Uninitialized)
    );
    this._register(
      registerAction2(
