...
const apiMenus = [
  ...
  },
  {
    ...
    description: localize(
      "menus.historyItemHover",
      "The Source Control history item hover menu"
    ),
    proposed: "contribSourceControlHistoryItemMenu"
  },
  {
  ...
  },
  {
    ...
    description: localize(
      "menus.chatModelPicker",
      "The chat model picker dropdown menu"
    ),
    supportsSubmenus: false,
    ...
    const rows = commands.sort((a, b) => a.id.localeCompare(b.id)).map((command) => {
      return [
        ...
        new MarkdownString().appendMarkdown(
          `${command.menus.sort((a, b) => a.localeCompare(b)).map((menu) => `\`${menu}\``).join("&nbsp;")}`
        )
      ];
    });
    return {
