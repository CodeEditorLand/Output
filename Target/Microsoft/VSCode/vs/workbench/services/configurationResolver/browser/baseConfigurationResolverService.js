...
import {
  VariableError,
  VariableKind
} from "../common/configurationResolver.js";
import {
  ConfigurationResolverExpression
} from "../common/configurationResolverExpression.js";
...
class BaseConfigurationResolverService extends AbstractVariableResolverService {
  ...
  async resolveWithInteractionReplace(folder, config, section, variables, target) {
    ...
    await this.resolveWithInteraction(
      folder,
      parsed,
      section,
      variables,
  ...
  async resolveWithInteraction(folder, config, section, variableToCommandMap, target) {
    ...
    for (const variable of expr.unresolved()) {
      ...
      if (variable.name === "command") {
        ...
        const value = await this.commandService.executeCommand(
          commandId,
          expr.toObject()
        );
        ...
        result = {
          value: await this._contributedVariables.get(
            variable.inner
          )?.()
        };
      } else {
    ...
    return new Map(
      Iterable.map(expr.resolved(), ([key, value]) => [
        key.inner,
        value.value
      ])
    );
  ...
  async resolveInputs(folder, section, target) {
    ...
    inputs ??= this.configurationService.getValue(
      section,
      overrides
    )?.inputs;
    return inputs;
