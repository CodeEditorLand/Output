...
let ExtensionManagementService = class ExtensionManagementService2 extends CommontExtensionManagementService {
  static {
  ...
  async installGalleryExtensions(extensions) {
    ...
    await Promise.all(extensions.map(async ({ extension, options }) => {
      try {
        ...
        if (options?.context?.[EXTENSION_INSTALL_SOURCE_CONTEXT] !== "settingsSync") {
          await this.checkForWorkspaceTrust(manifest, false);
  ...
  async installFromGallery(gallery, installOptions, servers) {
    ...
    if (installOptions?.context?.[EXTENSION_INSTALL_SOURCE_CONTEXT] !== "settingsSync") {
      await this.checkForWorkspaceTrust(manifest, false);
  ...
  async getExtensions(locations) {
    const scannedExtensions = await this.extensionsScannerService.scanMultipleExtensions(locations, 1, { includeInvalid: true });
    const result = [];
...
` : ""}${localize("non web extensions detail", "Contains extensions which are not supported.")}`;
  ...
  getTrustedPublishersFromStorage() {
    const trustedPublishers = this.storageService.getObject(TrustedPublishersStorageKey, -1, {});
...
let WorkspaceExtensionsManagementService = class WorkspaceExtensionsManagementService2 extends Disposable {
  static {
    __name(this, "WorkspaceExtensionsManagementService");
  }
  static {
    WorkspaceExtensionsManagementService_1 = this;
  }
  static {
    this.WORKSPACE_EXTENSIONS_KEY = "workspaceExtensions.locations";
  }
  ...
  getInstalledWorkspaceExtensionsLocations() {
    const locations = [];
    try {
      const parsed = JSON.parse(this.storageService.get(WorkspaceExtensionsManagementService_1.WORKSPACE_EXTENSIONS_KEY, 1, "[]"));
      if (Array.isArray(locations)) {
        for (const location of parsed) {
          if (isString(location)) {
            if (this.workspaceService.getWorkbenchState() === 2) {
  ...
  saveWorkspaceExtensions() {
    ...
    if (this.workspaceService.getWorkbenchState() === 2) {
      this.storageService.store(
        WorkspaceExtensionsManagementService_1.WORKSPACE_EXTENSIONS_KEY,
