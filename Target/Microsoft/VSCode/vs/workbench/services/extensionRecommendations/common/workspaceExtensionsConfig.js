...
let WorkspaceExtensionsConfigService = class extends Disposable {
  ...
  async toggleRecommendation(extensionId) {
    ...
    const isWorkspaceRecommended = workspaceExtensionsConfigContent?.recommendations?.some(
  ...
  async toggleUnwantedRecommendation(extensionId) {
    ...
    const isWorkspaceUnwanted = workspaceExtensionsConfigContent?.unwantedRecommendations?.some(
      (r) => r === extensionId
    );
