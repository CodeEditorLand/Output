...
let AbstractExtensionService = class extends Disposable {
      ...
      this._extensionManagementService.onDidInstallExtensions(
        (result) => {
          ...
          for (const { local, operation } of result) {
            if (local?.isValid && operation !== InstallOperation.Migrate && this._safeInvokeIsEnabled(local)) {
  ...
  async _resolveAndProcessExtensions(lock) {
    ...
    if (resolverExtensions.length) {
      toAdd = allExtensions.filter(
        (extension) => !resolverExtensions.some(
      ...
      if (allExtensions.length < toAdd.length + resolverExtensions.length) {
        const toRemove = resolverExtensions.filter(
          (registered) => !allExtensions.some(
            (e) => ExtensionIdentifier.equals(
              e.identifier,
        ...
        if (toRemove.length) {
          this._registry.deltaExtensions(
            lock,
            [],
            toRemove.map((e) => e.identifier)
          );
...
function isResolverExtension(extension) {
  return !!extension.activationEvents?.some(
    (activationEvent) => activationEvent.startsWith("onResolveRemoteAuthority:")
  );
}
