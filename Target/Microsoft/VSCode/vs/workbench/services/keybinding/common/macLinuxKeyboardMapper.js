...
class ScanCodeKeyCodeMapper {
  ...
  lookupKeyCodeCombo(keyCodeCombo) {
    ...
    for (let i = 0, len = scanCodeCombosEncoded.length; i < len; i++) {
      ...
      const ctrlKey = !!(scanCodeComboEncoded & 1);
      const shiftKey = !!(scanCodeComboEncoded & 2);
      const altKey = !!(scanCodeComboEncoded & 4);
  ...
  lookupScanCodeCombo(scanCodeCombo) {
    ...
    for (let i = 0, len = keyCodeCombosEncoded.length; i < len; i++) {
      ...
      const ctrlKey = !!(keyCodeComboEncoded & 1);
      const shiftKey = !!(keyCodeComboEncoded & 2);
      const altKey = !!(keyCodeComboEncoded & 4);
...
class MacLinuxKeyboardMapper {
  ...
  dumpDebugInfo() {
    ...
    for (let scanCode = ScanCode.None; scanCode < ScanCode.MAX_VALUE; scanCode++) {
      ...
      for (let mod = 0; mod < 8; mod++) {
        const hwCtrlKey = !!(mod & 1);
        const hwShiftKey = !!(mod & 2);
        const hwAltKey = !!(mod & 4);
