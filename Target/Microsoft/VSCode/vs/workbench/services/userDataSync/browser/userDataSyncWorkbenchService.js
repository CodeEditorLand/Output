...
let UserDataSyncWorkbenchService = class extends Disposable {
  ...
  _accountStatus = AccountStatus.Uninitialized;
  get accountStatus() {
  ...
  _onDidTurnOnSync = this._register(new Emitter());
  onDidTurnOnSync = this._onDidTurnOnSync.event;
  _current;
  get current() {
    ...
    this._register(
      this.authenticationService.onDidChangeDeclaredProviders(() => {
        if (this.updateAuthenticationProviders()) {
          initPromise.finally(
            () => this.update(
              "declared authentication providers changed"
            )
          );
        }
      })
